...
def _extract_publish_time_from_id(note_id: str) -> int:
    """从小红书 note ID（MongoDB ObjectId 格式）提取精确发布时间戳。

    MongoDB ObjectId 的前 4 字节（8 个 hex 字符）是 Unix 秒时间戳，
    这是最精确的发布时间来源，无需依赖 cornerTagInfo 文字解析。

    Args:
        note_id: 小红书帖子 ID，如 "69a56e1b000000000d00ac3e"。
...
class NoteCard:
    ...
    publish_time: int = 0  # Unix 秒时间戳，0 表示未知

    @classmethod
    def from_dict(cls, d: dict, note_id: str = "") -> NoteCard:
        video_data = d.get("video")

        # 优先从 note ID（MongoDB ObjectId 前4字节）提取精确时间戳
        publish_time_sec = _extract_publish_time_from_id(note_id)

        # 兜底：从 publishTime / time 字段读取（单位可能是毫秒或秒）
        if publish_time_sec == 0:
            raw_ts = d.get("publishTime") or d.get("time") or 0
            if raw_ts:
                raw_ts = int(raw_ts)
                # 毫秒级时间戳（13位）转秒，秒级（10位）直接使用
                publish_time_sec = raw_ts // 1000 if raw_ts > 1e10 else raw_ts
...
class Feed:
    ...
    def from_dict(cls, d: dict) -> Feed:
        ...
        return cls(
            ...
            note_card=NoteCard.from_dict(d.get("noteCard", {}), note_id=note_id),
            index=d.get("index", 0),
        )
    ...
    def to_dict(self) -> dict:
        ...
        result: dict = {
            ...
            "publishTime": self.note_card.publish_time,
            "user": {
...
class PublishImageContent:
    ...
    is_ai_assisted: bool = False  # 是否标注"AI 辅助创作"

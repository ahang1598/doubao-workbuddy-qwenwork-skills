...
description: 证据驱动论文写作参考。当用户写作或修改引言、相关工作、研究背景、文献综述、讨论、结论，或任何需要引用、实验、图表、事实材料支撑的段落时，使用此参考；重点防止编造引用、候选文献堆砌、强结论无证据、正文混入提示词和 AI 草稿痕迹。
---
...
涉及文献、实验、方法对比或领域判断的正文，不要直接生成。先在对话中建立两个轻量产物：

1. **Evidence Map（证据图）**：哪些材料能支持哪些具体判断。
2. **Paragraph Blueprint（段落蓝图）**：每段承担什么论证角色、使用哪些证据、边界在哪里。

豆包沙箱可能回收，因此这些产物不要求写入 `refs/` 或 `plan/` 文件；它们必须在对话中可见，必要时汇总进 `paper-writer-state`。
...
结果、讨论、贡献声明或摘要中涉及实验效果时，先在对话中明确实验协议。协议不要求写入持久文件，但必须让当前证据链可见，避免把计划、模拟数据或未完成实验写成真实结果。直接生成论文时默认 Draft Mode：可以用规划数据和 mock 表格搭完整初稿，但必须让读者一眼看出这些内容等待真实实验替换。
...
- 在对话产物、表格标题或状态块中明确标注为 `PLANNING DATA`；不要依赖本地文件名前缀来区分。
...
- 证据 ID：
- 边界或限定：
- 禁止写入：提示词、过程说明、AI 来源说明、待替换说明、无依据强结论
```
...
- 待替换说明：如“用户应替换此处”“fill later”“此处插图”。

<!-- Source: HKUSTDial/Supervisor-Skills. Adapted for doubao-academic-polish. -->
...
- **Cross-domain Technique Paper**: a Technique Paper whose
  ...
  Key Idea carries the narrative but the cross-domain framing is
...
Ask the following in order (aligned with intro-drafter's paper-types.md). First yes decides:
...
3. Is the paper's main contribution a new benchmark? If yes, use
   `benchmark-paper-template` (separate plugin).
4. Is the technique transplanted from another field? If yes, Mixed
   (Technique with cross-domain framing).
5. Default: Technique Paper.

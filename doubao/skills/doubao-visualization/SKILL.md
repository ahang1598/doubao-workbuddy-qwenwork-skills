...
description: 回答中，只要内容包含可被图形更清晰表达的结构化关系，即使用户未明确要求，也应主动读取并使用ke shi h Skill。覆盖流程与顺序、多对象或多维比较、构成与分布、机制与因果、架构与关系网络、空间或设备结构、趋势与时间序列、预测、量化及多因素分析，并继承多轮中的可视化、重算、比较和修改意图；同时图表、图解、配图、信息图、知识图谱、流程图、时间线、结构图、原图标注与圈选连线、路径轨迹、动态或交互演示、动画讲解、参数变化、知识科普支持默认可视化。根据任务选择 ECharts、HTML/SVG、原图叠加、基于原图二次生成或生成式知识配图。仅原子事实或确认、翻译改写、用户明确只要文字、纯图片检索或编辑、地图导航，或独立 PPT、Sheet、App、附件及工程任务中内嵌可视化无实际价值时不使用。
---

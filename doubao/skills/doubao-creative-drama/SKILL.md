...
- **【最高优先级】唯一正式交付方式为 `present_files`**：
 - 所有用户可见产物都必须先保存为正式文件，再调用 `present_files` 交付。适用范围包括但不限于规划报告、参数确认表、剧情梗概、人物小传、场景清单、剧本、分镜表、资产验收表、图片、关键帧、视频、提示词包及阶段汇总。
 - 禁止使用裸链接、本地路径、临时 URL、聊天正文、Markdown 图片/链接、HTML 媒体标签、截图、附件链接、其它展示工具或口头说明替代 `present_files`。
 - 聊天正文只能发送简短的交付状态、确认问题和下一步请求，不得粘贴或复述产物正文，不得暴露产物的 URL 或本地路径。
 - 调用 `present_files` 前，按 `ep_N` 命名空间和本 skill 的命名规范整理文件名；文本与表格产物优先保存为 Markdown 文件，图片和视频保留可直接查看的原始格式。
 - 只有 `present_files` 返回成功后，才可声明产物已交付并请求用户确认。工具不可用或交付失败时，不得切换为任何其它交付格式；应保留正式文件并重试 `present_files`，仍失败则仅说明正式交付未完成，禁止声称阶段完成。
 - 本规则覆盖本文件及所有 `references/*.md` 中历史遗留的“展示”“返回”“输出格式”等表述；下游 reference 不得另行指定其它用户可见交付方式。
 - 本 skill 中出现的“格式”“模板”“示例”仅定义正式文件内部的内容结构，不构成新的交付方式；这些内容仍必须写入文件并通过 `present_files` 交付。
- 当用户要求生成视频时，**必须先澄清并确认以下基础参数再动手**，不得跳过参数确认直接调用视频生成工具：
 ...
 - **即时正式交付原则**：在资产设定和视频生成阶段，每生成完一张图或一个视频，必须立即保存为正式文件并调用 `present_files` 交付，不等待全部产物生成完毕后再统一返回。
 ...
 - **【强制】图片和视频统一通过 `present_files` 交付**：所有阶段的人设图、场景图、道具图、关键帧图和视频生成结果都必须先落为正式文件，再调用 `present_files`；禁止使用 Markdown、HTML、URL、路径、截图或其它展示工具替代。
...
| scriptwriter | 故事主题、题材、单集总时长/长篇总集数、主角/配角/反派人物设定、核心冲突、结局方向、情绪基调、禁用内容、剧作基调偏好。**建议交付"剧情梗概（约 1500 字）→ 人物小传（主角/配角/反派）→ 主要故事场景（2-4 / 4-8 个）→ 完整剧本（分集分场专业格式）"四段产物**；长篇建议附分集大纲，且完整剧本按"首批 3-5 集 → `present_files` 交付 → 用户确认 → 下一批"的模式推进，不建议一次性生成全部剧集；完整剧本的场景推荐从产物三清单中选取。剧情设计和剧本设定缺关键参数时先澄清通常更稳。 |
...
启动或接续项目时，先将以下简短规划报告保存为正式 Markdown 文件并通过 `present_files` 交付，再读取下一阶段 ref。禁止把报告正文直接贴入聊天：
...
- 长篇总集数 >10 集：已将"剧情/人设漂移"幻觉声明及主动建议写入正式文件，并通过 `present_files` 交付：是/否/不适用。
...
在剧本、分镜、资产、关键帧的任何一个阶段完成正式文件生成并通过 `present_files` 成功交付后，**必须停下来**，主动询问用户是否满意、是否需要修改；**必须**收到"确认/没问题/继续/yes/ok"等明确许可后，才可以读取下一个 reference 文件推进。**禁止**在没有获得用户明确许可的情况下，擅自读取下一个 reference 文件并自动推进到下一阶段。
...
- **【强制】所有用户可见产物均已保存为正式文件并通过 `present_files` 成功交付**，未使用裸链接、本地路径、临时 URL、聊天正文、Markdown/HTML 媒体标签、截图、附件链接、其它展示工具或口头说明替代。
...
- **【强制】单批生成视频数量不超过 2 个**。
- 资产和视频遵循"即时正式交付"原则，生成一个文件即调用一次 `present_files`，不囤积。
- **【强制】只有确认 `present_files` 已成功交付对应文件后，才可以声明该产物已生成或该阶段已完成**；失败时使用同一正式文件重试，不得改用任何其它交付格式。

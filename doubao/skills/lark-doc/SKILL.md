...
- **Windows 兼容性**：SystemPrompt 出现 `Computer OS: Windows` 或为 windows os 时遵循以下指令
  - Bash 工具实际按 PowerShell 语法执行：每次 Bash 调用只执行一条外部命令；禁止使用 `&&` / `||` 串联命令，多步操作拆成多次 Bash 调用。禁止使用 PowerShell `Get-Content` 读取待传给 `lark-cli` 的本地文件，也不得通过变量或管道中转文件内容；
  - 参数支持文件输入时，必须直接使用上述 `@file` 形式（`@` 前必须加双引号），避免文本解码或重编码导致内容损坏。常见问题：在 PowerShell 5.X 上，双引号会被直接移除，导致内容解码失败。
  - 文件读写走工具本身或 Python**：不要用 `>` / `Out-File` / `Get-Content ... | cli --xxx -`，PowerShell 5 默认非 UTF-8、加 BOM、走 stdin 时非 ASCII 字节会被重编码。读写文件形式时使用统一编码，比如 utf-8
- **身份：文档操作推荐显式指定 `--as user`。**
- **最终给用户的回复不要过长**：最重要的产物是你最终生成的文档产物，而不是给用户的最终回复。最终回复 **不能超过300字，也不能超过 8 行**，只需要简单介绍一下产物即可

...
> Runs as the authorized end user (`--as user`). The caller must be in the target chat, and must belong to the same tenant for internal chats.
...
| Permission denied | missing `im:chat.members:read` |   | 让 agent 平台为当前用户补开 `im:chat.members:read` scope |

...
`im +messages-search` is user-only. For a named group with history/listing intent, resolve the group with `im +chat-search`, then list messages with `im +chat-messages-list --chat-id <chat_id>`.

...
- 唯一例外——用户需求里**明确说要由你（Agent）发布为 HTML 应用**时（如「你直接帮我发布成应用」「用命令发布上线」）：读取 [`reference/html-publish.md`](reference/html-publish.md)，按其规则**直接执行 html-publish 敏捷链路**发布（用户此话即授权，无需再次向用户确认）——发布状态查询等规则以该文档为准。

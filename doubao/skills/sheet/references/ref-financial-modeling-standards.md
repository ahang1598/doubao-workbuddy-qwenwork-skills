...
本文是**财务/金融场景的操作与视觉规范**，与执行引擎无关——无论产物是 Python（openpyxl）生成的本地 `.xlsx`，还是飞书表格，只要表格里承载的是财务性质的数据，就必须遵守本文。
...
- **输入已是一张成型的模型表**（给了飞书表格或附件文件，要求"美化 / 规范化 / 保留计算逻辑 / 更新到原表"）→ 这是**美化已有表，不是重建**：只在原表上就地调整样式属性（字体 / 填充 / 边框 / 对齐 / 数字格式），本文关于报表结构、公式、Sensitivity 与布局、多 sheet 拆分的规则**只用于你新增的内容，不得用来改动原表的既有结构、公式和布局**。此分支下必须守住两条：
...
> **公式优先是财务建模的基础纪律：用 Python 算好数值再写进单元格，交付的是改输入不重算的死表。**
...
ws['E7'] = 644                          # 直接写死 Python 算出来的结果 → 改输入不重算
...
写完横向公式后，必须用 `load_workbook(..., data_only=False)` 回读 2-3 个相邻年份列的公式字符串，确认年份引用跟随列移动、被锁定的假设保持不变；再按 `references/ref-xlsx-workflow.md` 的交付门禁跑一次 `excel_formula_verify.py`——openpyxl 以 `data_only=True` 直接读产物只会拿到 `None`，缓存值要靠重算生成。

### 先立结构、时点与口径三类不变量，再写公式

模型算错常常不在公式语法，而在结构缺一行、时点错一期、口径混用。下面五条与具体模型无关，动手前就落实：

- **两侧必须相等的结构，表内要有校验行**：来源与用途、资产与负债权益、期初到期末的滚动，各写一行 `= 左侧合计 - 右侧合计` 的公式并显示在表内。自己配平但不留校验行，读表的人无从确认它真的平。
- **先画时间轴，再写第一期**：交割日 / 起始日、首个完整期间、退出或结束日各自落格。交割落在期末时当期不计息、不摊还、不分配，首期从下一期起算；默认「当年即整期」会让利息、偿还与回报整链错一期。
- **资格与豁免判定先于费率档**：涉及减免、优惠、起征点、分档累进时，先用一张小表判定是否适用（条件、阈值、判定结论、依据各一列），判定为不适用就不再套档。先套费率再补判定，多算的金额会一路传导到最终结果。
- **同名指标的多个口径分列**：含税与不含税、全口径与剔除某类、期间累计与全年预计，各自单独取数并标注用途，不复用同一个总额当所有分母。同一事实存在多个口径时并列展示并注明期间与状态，不静默二选一。
- **首期、末期与最终结果写成检查点**：每条滚动链的首末期、以及税后利润 / 回报倍数 / 内部收益率 / 净现值这类最终结果，各取一个可手算的数，写进门禁的 `--checkpoints`（格址 + 期望值 + 容差，写法见 `references/ref-xlsx-workflow.md`）由工具逐条判。自己复算一遍再声明核对过，等于没核。
...
零值显示为 `-`，负数使用括号法 `(123)`。如采用窄货币符号列，符号列单独放 `$` / `¥`，数值列不再带货币符号。用户点名"保留 N 位小数"时，用固定 N 位的 `number_format`（如 `0.00`）落格、尾随零必须显示——不要让货币 / 千分位 / 默认显示盖掉要求的位数，也不要把数值与文本标记混进同一格。
...
- **公式零错误（一次诊断）**：财务模型公式密集、`$` 锁定与跨表引用极易写错，是 `#REF!` / `#DIV/0!` / `#VALUE!` / `#N/A` / `#NAME?` / `#NUM!` / `#NULL!` 的高发区。生成 `.xlsx` 后按 `references/ref-xlsx-workflow.md` 的交付门禁跑 `python3 scripts/excel_formula_verify.py <产物> 60`（基于已有文件编辑时另加 `--baseline <源文件.xlsx> --scope '<允许改动的范围>'`）。退出 3 是确定性错误（错误值、缺 `_xlfn.` 前缀、区域不同形、源表被改写），修完重跑；退出 4 是重算未完成或硬编码嫌疑，能公式化且成本可控时改写，不能时在交付说明声明静态值原因。
...
#   cp model.xlsx .recalc.xlsx && python3 scripts/excel_formula_verify.py .recalc.xlsx --in-place
...
金融模型若用户或附件给了抽检值 / 目标值 / covenant / breakeven，建议把它们做成交付前诊断：Debt roll-forward、Exit Net Debt、MOIC/IRR、税务调增合计、关键百分比哨兵等任一不符时，在交付说明中列出。

## 落地提示（按引擎）

本规范的逻辑与视觉要求与引擎无关；具体写值/公式/样式时按产物载体选实现方式：

- **本地 `.xlsx`（Python 引擎）**：用 openpyxl 实现颜色、数字格式、边框、列宽、网格线、勾稽校验（见上方各 Python 示例）；批量样式可用 `python3 scripts/excel_format_range.py`，公式诊断可用 `python3 scripts/excel_formula_verify.py`（含公式重算）。所有颜色用不带 `#` 的 6 位 hex（如 `'0000FF'`）。

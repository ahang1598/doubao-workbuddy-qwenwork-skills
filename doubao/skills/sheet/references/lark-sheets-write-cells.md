...
1. **写完建议读回**：`+cells-set` 后紧跟 `+csv-get`（或 `+cells-get`）读目标范围前几行，检查是否出现 `#VALUE!` / `#NAME?` / `#REF!` / `#N/A` / `#DIV/0!` / `#NUM!`；AI 公式跳过这一步，第一校验入口是 `+formula-verify --ai-only --range`（禁止先用 `+cells-get` / `+csv-get` 轮询 AI 结果）
...
💡 **公式写入后的补充诊断可用 `+formula-verify`**：`+csv-get` / `+cells-get` 的抽样回读只能帮你快速发现明显错误，但它覆盖不到整列中段、隐藏行、被条件格式遮蔽的错误，也看不到 `partial` 截断。如果这次 `+cells-set` / `--copy-to-range` / `+csv-put` 实际写入了公式，可转到 `lark-sheets-formula-verify` 跑 `+formula-verify` 做诊断。普通公式跑到 `status='success'`；AI 公式先用 `+formula-verify --ai-only --range <代表性范围>` 抽检，没有明确失败 / 不支持状态时允许带 pending 交付并告知用户后台仍在计算。不要等用户补一句"再验证下公式"才想到这个工具。

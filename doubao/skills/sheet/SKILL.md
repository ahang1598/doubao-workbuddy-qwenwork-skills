...
---
> ⚠️ **强制前置条件**：本文档共 369 行，指令说明和约束分散在各部分。必须先读到末行「全文完」，再执行任何其他操作；即使已找到当前任务所需的指令，也不得提前停止。未见该标记时，下一步必须继续调用 Read，在读完之前不得调用 Bash、`--help` 或任何其他工具。读取工具能一次取全文就一次取全，被截断则调整偏移量（`offset`）续读。本技能所有文档末行均有该标记。
...
Canvas 模式打开一个本地文件时，文件会被解码为底层产物放入工作目录，其解码后的路径被 `token` 唯一标识，豆包客户端（`canvas`）通过实时渲染底层产物，让用户实时感受到你对此产物的任何变更，一切变更均发生于工作目录中的底层产物，不改变用户本地文件，仅当用户要求保存时才会打包成同类型文件覆盖用户提供的真实本地路径。
...
- 在Canvas 模式下，若 `lark-cli` 长期出现超时问题，应当考虑当前 `canvas` 可能被关闭，不要继续使用 `lark-cli` 指令。`file_status` 为 "close" 时，`lark-cli` 一定会执行失败，此时可用 Python 处理。文件从未被打开（不带 `token`）时同理。
- Canvas 模式下编辑的本地文件只能由用户自行保存，无法通过 `lark-cli` 导出。

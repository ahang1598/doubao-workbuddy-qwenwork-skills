...
医学领域相关的文献检索与调研需求优先使用 `doubao-medical-literature-search`。本 Skill 覆盖指南查新、研究调研、限定范围的一次性资讯查新、综述和课题方向调研，将医学主题或课题转化为可检索问题，综合指南/共识、系统综述、关键研究、最新进展、权威医学资讯和必要监管来源，输出可追溯的文献梳理与证据脉络。
...
- 查询 PubMed 等文献数据库中的一组论文、指南或证据脉络，继续本 Skill；只查基因/变异、临床试验、药械注册等公开数据库的具体记录、标识或字段，改用 `doubao-biomedical-data-query`。数据库记录仅作为文献调研材料时，不改变本 Skill 归属。
- 主要查询医保目录、限定支付、药品品规编码或地方待遇政策，改用 `doubao-medical-insurance-assistant`；只把医保条件作为文献比较维度时，继续本 Skill。
- 开放式查询领域最新进展或持续跟踪、订阅更新，改用 `doubao-medical-literature-monitoring`；限定主题、时间或来源的一次性文献查新继续本 Skill。
- 用户给定文献并要求来源内解读或翻译，分别改用 `doubao-medical-literature-interpretation`、`doubao-medical-literature-translation`；需要主动检索、比较或扩展多篇证据时，继续本 Skill。
- 推荐课题方向、梳理研究基础与证据空白，继续本 Skill；需要形成可执行的研究方案、终点或方法设计时，改用 `doubao-medical-protocol-design`。病历文书、临床汇报或患者科普写作也不属于本 Skill。
...
`quick_answer` 仍是有来源的循证短答，不是无检索、无引用的普通回答：完成必要的高价值检索后，引用的指南、共识、论文、试验、说明书或监管来源必须可追溯。`tosv.byted.org` 域内资料不展示 URL，必要时只以纯文本写来源名称。聊天引用作为冒号后的内容出现时，无论来源工具均使用 Markdown；只有紧邻相关句子、段落或内容的批注式引用，且 URL 确由 `general_search` 返回时，才使用 `<RichMediaReference>`。其他工具、其他位置或来源不明均使用 Markdown。可见链接只使用正式发布主体、期刊/出版社官网、PubMed/PMC、DOI、监管机构、专业学会或正式说明书等权威可追溯入口。
...
- `seed_medical_search` 已直接存在于当前可用工具列表中；需要检索指南、共识或药品说明书时直接调用，不得先用 `tool_search` 搜索、发现、确认或转接该工具。
- 非文档聊天回答的引用格式同时按引用位置和 URL 的实际来源工具判断。`tosv.byted.org` 域内资料的 URL 一律不展示，不生成 Markdown 超链接，也不放入 `<RichMediaReference>`；必要时只以纯文本写来源名称。链接或引用作为冒号后的内容出现时，无论来自何种工具，都禁止使用 `<RichMediaReference>`，必须改用 Markdown 超链接。只有引用位于相关句子、段落或内容旁，作为批注式参考，且 URL 确由 `general_search` 返回时，才使用 `<RichMediaReference>`；其他位置、其他工具或来源无法确认时均使用 Markdown。输出前逐项核对组件位置、URL 来源和域名，不得根据域名、页面类型或内容来源猜测工具来源。
...
- `quick_answer` 必须保留循证来源：进行必要的高价值检索，引用具名外部证据时提供可追溯来源；`tosv.byted.org` URL 不展示。冒号后的引用无论来源均用 Markdown；仅相关句子、段落或内容旁的批注式引用，且 URL 确由 `general_search` 返回时，使用 `<RichMediaReference>`。其余 URL 使用 Markdown，锚文本使用完整来源标题，不得过度简写；段末或回复末写成 `[完整标题｜host](URL)`，同一位置有多个 Markdown 链接时，用中文顿号 `、` 或分行分隔。不得只给编号或裸 URL。
...
- 聊天 response（包括 `quick_answer` 和 `full_report` 的最终入口回复）直接引用外部证据时，来源链接只取检索结果结构化返回的 `url/link` 字段，不从摘要或展示文本手抄。先排除 `tosv.byted.org` 域内 URL：回答中不得展示其超链接，必要时只以纯文本写来源名称。若链接或引用出现在中文或英文冒号后，例如“来源：”“参考：”“证据：”“链接：”，无论 URL 来自什么工具，都使用 Markdown 超链接，不得使用 `<RichMediaReference>`。仅当引用紧邻相关句子、段落或内容，起批注参考作用，且 URL 确由 `general_search` 返回时，才使用 `<RichMediaReference>["https://..."]</RichMediaReference>`；同一批注位置的多个 `general_search` 来源可写入一个纯 URL JSON 数组。其他工具、其他位置或来源无法确认的 URL 均使用 Markdown，可在正文相关位置用完整来源标题作锚文本，也可在段末或回复末以 `[完整标题｜host](URL)` 集中列出；不得过度简写标题，同一位置有多个 Markdown 链接时，用中文顿号 `、` 或分行分隔。发送前逐项核对每个组件的位置、URL 来源和域名；不满足“批注式位置 + general_search 来源 + 非 tosv.byted.org”任一条件时，不得使用组件。渲染前将缺少协议的完整域名与路径补成 `https://...`，不得输出无协议裸短链。该规则不改变飞书、Word、PPT 等文档的既有超链接写法。
...
- 任何不创建或不交付飞书文档、PPT、Word 等产物的回答，引用外部证据时都必须提供可追溯来源；`tosv.byted.org` URL 不展示。冒号后的引用一律使用 Markdown；只有位于相关内容旁的批注式引用且 URL 确由 `general_search` 返回时，才使用 `<RichMediaReference>`。非 `general_search`、非批注式位置或来源不明均使用 Markdown，锚文本使用完整来源标题，段末/回复末写成 `[完整标题｜host](URL)`，同一位置有多个 Markdown 链接时，用中文顿号 `、` 或分行分隔。每项核心检索结论仍须可追溯；无法取得 `url/link` 的候选不得作为具名依据写入回答，若无可核验来源则明确说明检索限制并降低结论确定性。
...
- `full_report` 中，每轮先筛掉明显无关、重复和低质量结果，再将拟纳入正文、图表、参考文献或保留为辅助参考的证据及时写入内部文献记录。完成计划检索批次后运行 `node scripts/match-journal-rank.js --csv <literature-log.csv> --in-place`，再精读或写报告；后续新增论文时重新运行。临床指南/共识和药品说明书使用 `seed_medical_search`：每次指南 Query 必须包含 `filetype:guideline`，每次说明书 Query 必须包含 `filetype:drug`；可按主题或资料类型多次查询，同时需要两类资料时分别查询。结果不足时再用 `general_search` 补充官方发布页、监管页或其他权威正式入口。纳入说明书时必须记录原标题、机构、级别、纳入理由和可访问链接。
...
- 证据角色按研究质量、来源权威性、主题匹配度和时效性判断，不按是否取得全文机械分组。利用关键研究时，将“问题匹配度 × 研究设计适切性 × 信息量与精确性 × 偏倚风险 × endpoint 质量 × 结果一致性与可重复性”作为方法层判断：不计算分数，不要求逐项展示；用“可支持的具体判断 + 关键依据 + 主要边界”说明研究对当前检索问题的贡献。默认不展示阅读状态；仅对确实阅读过全文的重要论文，可简要标注 `已阅读全文`。
...
- 不使用高程度或绝对化结论措辞；若原文有推荐等级，只描述原文等级。研究价值短评只使用非排序表述，不做最高级或名次式评价；样本量、随访、期刊和发表时间只用于解释精确性、适用性或时效性。确需横向排序时，另在有明确范围和数据依据的比较段中说明。
- 除医学资讯跟踪按专用结构可在图表不能增加信息时省略外，纳入正文或参考文献的相关文献达到 5 篇及以上时，除非用户明确要求不要图表，正式文档必须同时生成证据脉络 Mermaid 思维导图和关键研究里程碑 Mermaid 时间线图。相关文献不足 5 篇时，若问题范围很窄或图表不能增加信息，可省略其中一张或全部白板图。思维导图按“证据分类 -> 具体文献 -> 核心结论”组织；时间线按年份或研究阶段呈现；图表与正文、参考文献统一使用 CSV 编号。系统综述和关键研究默认逐篇解读并写明期刊名称和年份；与用户问题、核心结论或关键边界直接相关的指南和研究应适度展开其关键条文、设计、结果及实际意义，外围或重复证据保持简洁，不平均分配篇幅。
- 输出定位为循证文献检索与学术、临床学习支持，不替代临床医嘱、伦理审查、正式系统综述注册、统计分析或人工专家审稿。

## 能力路由

- `seed_medical_search` 检索临床指南/共识和药品说明书；每次指南 Query 必须包含 `filetype:guideline`，每次说明书 Query 必须包含 `filetype:drug`。可按主题或资料类型多次查询，同时需要两类资料时分别查询。
- `scholar_search` 优先检索论文、系统综述、Meta 分析、RCT、队列、诊断研究、里程碑研究和最新高影响力研究。
- `general_search` 用于最新论文检索补充，以及补充学会官网、政府/监管机构、期刊原文页、数据库页面、临床试验注册和其他权威正式入口。
- 用户明确强调“最新”“近期”“刚发布”或“最新进展”时，指南/共识仍先用带 `filetype:guideline` 的 `seed_medical_search`，随后适度前移 `general_search`，补充可能尚未被医学或学术检索及时收录的最新正式发布信息；同时核验来源权威性，并继续用 `scholar_search` 覆盖相关研究证据。
- 用户明确要求本 Skill 完成限定范围的一次性医学资讯查新时，优先用 `general_search` 检索近期权威机构、学会、监管部门、期刊在线优先、会议官网和试验注册动态，再用 `scholar_search` 补充或定位对应论文、指南及既有证据；可少量补充与主题高度相关的 `arXiv`、`medRxiv` 或 `bioRxiv` 预印本，必须标明“预印本/未同行评议”。按日期、来源类型和证据状态去重整合，不把新闻稿或预印本单独写成已确立的疗效或安全性结论。

...
本文件覆盖**截图后的人工视觉核对**，与 [validation-xml.md](validation-xml.md) 中的服务端 lint 和 XML 内容核对共同用于验收。按任务工作流完成必要截图；截图不可用时，该页保留未完成视觉验收状态，不要声称做过视觉核对。

截图视觉校验能发现 XML 难以判断的问题（文字截断、图片裁切、图表压盖、弱对比）。截图时假设页面有问题，主动找出并报告所有风险。

截图保存在当前工作目录 `<CWD>` 下，命令显式传 `--output-dir`；使用响应中的实际图片路径核对，保留来源截图用于前后比较。

截图命令（逐页传 `--slide-id`，单批最多 8 页，超出按批次串行）：
...
- 文本框过窄，导致不必要的频繁换行。
- 残留占位符、模板默认文字或未替换内容。
- 流程线或反馈线：核对起止节点、箭头方向和折线路径，避免端点悬空或穿过文字；表达闭环时，反馈线应明确回到对应节点。抽查截图时应覆盖这类页面。
- 树状图：核对节点、连线、箭头方向、分组边界和标签可读性；视觉正常不等于满足可编辑要求，仍须在回读 XML 中确认树状图主体没有 `<embed>`。
...
只有逐页实际看过并记录 `pass`/`fix` 的页，才能写“该页已截图确认”；截图落盘、批量成功或只看关键页都不算。对 lint 返回或被豁免的每条发现，按元素 ID 和坐标定位到截图中的具体区域，并结合回读 XML 核对，不能只写“整体正常”。截图不能替代对已确认真实问题的修复；修复后开启 lint 重提并重新查看截图。

必要截图因限流、超时或读取失败而未完成时，按错误提示恢复后重试；仍不可用则保留该页未验收状态，说明缺失页及原因，不得凭 `ok: true`、无 `issues`、静态检查或导出成功改记为 `pass`，也不得把整稿交付为已完成验收。

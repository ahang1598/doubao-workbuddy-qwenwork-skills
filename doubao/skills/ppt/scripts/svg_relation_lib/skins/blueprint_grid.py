"""
Skin · blueprint_grid · Blueprint Grid · CAD Cyan

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: dimension arrows + measurement text.
"""
...
FONT_SERIF = 'Courier New, JetBrains Mono, monospace'
FONT_SANS = 'JetBrains Mono, Menlo, monospace'
...
PALETTE_OBJ = Palette(
    name='Blueprint Grid · CAD Cyan',
    bg='#1E3A5F',
    bg_alt='#254670',
    bg_dim='#254670',
    ink='#F5F9FF',
    gray='rgba(245,249,255,0.55)',
    hair='#F5F9FF',
    primary=HUE["rust"],
    ...
    signature_note="BLUEPRINT GRID",
)

# alias for registry contract
BLUEPRINT_GRID = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class BlueprintGridSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # CAD blueprint: dimension arrows + measurement text
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="none" stroke="{primary}" stroke-width="1"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-6}" fill="none" stroke="{accent}" stroke-width="0.4" opacity="0.6" stroke-dasharray="3 2"/>')
        # dimension arrow bottom
        parts.append(f'<line x1="{cx-r-14}" y1="{cy+r+16}" x2="{cx+r+14}" y2="{cy+r+16}" stroke="{primary}" stroke-width="0.6"/>')
        parts.append(f'<line x1="{cx-r}" y1="{cy+r+12}" x2="{cx-r}" y2="{cy+r+20}" stroke="{primary}" stroke-width="0.6"/>')
        parts.append(f'<line x1="{cx+r}" y1="{cy+r+12}" x2="{cx+r}" y2="{cy+r+20}" stroke="{primary}" stroke-width="0.6"/>')
        parts.append(f'<text x="{cx}" y="{cy+r+13}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{primary}" opacity="0.7">{r*2:.0f} px</text>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8.5" font-weight="700" fill="{accent}" letter-spacing="0.20em">SPEC · {esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SANS}" font-size="20" font-weight="700" fill="{primary}">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+26}" text-anchor="middle" font-family="{FONT_SANS}" font-size="15" font-weight="700" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{accent}" opacity="0.8" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = BlueprintGridSkin()


__all__ = [
    "BLUEPRINT_GRID", "PALETTE",
    "BlueprintGridSkin", "SKIN_INSTANCE",

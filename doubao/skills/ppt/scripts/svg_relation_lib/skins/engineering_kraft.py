"""
Skin · engineering_kraft · Engineering Kraft · Sketch

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: hand-drawn double stroke + blueprint dashes.
"""
...
FONT_SERIF = 'Baskerville, Georgia, serif'
FONT_SANS = 'Architects Daughter, Kalam, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Engineering Kraft · Sketch',
    bg='#D6C7A8',
    bg_alt='#CCBE9F',
    bg_dim='#CCBE9F',
    ink='#1A3C6C',
    gray='rgba(26,60,108,0.55)',
    hair='#1A3C6C',
    primary=HUE["rust"],
    ...
    signature_note="ENGINEERING KRAFT",
)

# alias for registry contract
ENGINEERING_KRAFT = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class EngineeringKraftSkin:
    name = 'engineering_kraft'
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # hand-drawn double stroke + blueprint dashes
        parts.append(f'<circle cx="{cx-1}" cy="{cy+1}" r="{r}" fill="{pal.bg_alt}" stroke="{primary}" stroke-width="1.6"/>')
        parts.append(f'<circle cx="{cx+1}" cy="{cy-1}" r="{r-2}" fill="none" stroke="{primary}" stroke-width="0.6" opacity="0.5" stroke-dasharray="3 2"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{primary}" letter-spacing="0.14em">{esc(kicker.upper())}</text>')
        if name:
        ...
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.gray}" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = EngineeringKraftSkin()


__all__ = [
    "ENGINEERING_KRAFT", "PALETTE",
    "EngineeringKraftSkin", "SKIN_INSTANCE",

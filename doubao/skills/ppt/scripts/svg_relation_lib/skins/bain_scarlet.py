"""
Skin · bain_scarlet · Bain Scarlet · Trade Gothic

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: thick top+bottom rule bar hero.
"""
...
FONT_SANS = 'Trade Gothic, Franklin Gothic, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Bain Scarlet · Trade Gothic',
    bg='#FFFFFF',
    bg_alt='#F8F5F5',
    bg_dim='#F8F5F5',
    ink='#1A1A1A',
    ...
    signature_note="BAIN SCARLET",
)

# alias for registry contract
BAIN_SCARLET = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class BainScarletSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # top+bottom heavy rule bar (Bain scarlet style)
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg}" stroke="{primary}" stroke-width="1.2"/>')
        parts.append(f'<rect x="{cx-r}" y="{cy-r+2}" width="{r*2}" height="4" fill="{primary}"/>')
        parts.append(f'<rect x="{cx-r}" y="{cy+r-6}" width="{r*2}" height="4" fill="{primary}"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-26}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{primary}" letter-spacing="0.26em">{esc(kicker.upper())}</text>')
        if name:
        ...
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+28}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
...
SKIN_INSTANCE = BainScarletSkin()


__all__ = [
    "BAIN_SCARLET", "PALETTE",
    "BainScarletSkin", "SKIN_INSTANCE",

"""
Skin · swiss_grid · Swiss Grid · International Style

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: pure black type set on grid + accent stripe.
"""
...
PALETTE_OBJ = Palette(
    name='Swiss Grid · International Style',
    bg='#FFFFFF',
    bg_alt='#F5F5F5',
    bg_dim='#F5F5F5',
    ink='#000000',
    ...
    signature_note="SWISS GRID",
)

# alias for registry contract
SWISS_GRID = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class SwissGridSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # Swiss International Style: pure black type on grid + red stripe
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg}" stroke="{pal.ink}" stroke-width="0.5"/>')
        parts.append(f'<rect x="{cx-r}" y="{cy-2}" width="{r*2}" height="4" fill="{accent}"/>')
        if kicker:
            parts.append(f'<text x="{cx-r+6}" y="{cy-r+16}" font-family="{FONT_SANS}" font-size="8" font-weight="700" fill="{pal.ink}" letter-spacing="0.18em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx-r+6}" y="{cy-16}" font-family="{FONT_SANS}" font-size="20" font-weight="700" fill="{pal.ink}" letter-spacing="-0.02em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx-r+6}" y="{cy+22}" font-family="{FONT_SANS}" font-size="14" font-weight="700" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx-r+6}" y="{cy+r-6}" font-family="{FONT_SANS}" font-size="7" fill="{pal.gray}" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = SwissGridSkin()


__all__ = [
    "SWISS_GRID", "PALETTE",
    "SwissGridSkin", "SKIN_INSTANCE",

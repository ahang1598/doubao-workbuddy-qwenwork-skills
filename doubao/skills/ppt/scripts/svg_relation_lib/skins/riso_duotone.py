"""
Skin · riso_duotone · Riso Duotone · Overprint

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: two overprinted circles + riso pink+cyan.
"""
...
FONT_SERIF = 'GT Alpina, Fraunces, Georgia, serif'
FONT_SANS = 'GT Alpina, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Riso Duotone · Pink Overprint',
    bg='#F4CFD1',
    bg_alt='#EBBABE',
    bg_dim='#EBBABE',
    ink='#2E1F2E',
    gray='rgba(46,31,46,0.55)',
    hair='#2E1F2E',
    primary=HUE["rust"],
    ...
    signature_note="RISO DUOTONE",
)

# alias for registry contract
RISO_DUOTONE = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class RisoDuotoneSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # Riso: two overprinted circles + pink+cyan duotone
        parts.append(f'<circle cx="{cx-6}" cy="{cy-3}" r="{r-4}" fill="{accent}" opacity="0.72"/>')
        parts.append(f'<circle cx="{cx+6}" cy="{cy+3}" r="{r-4}" fill="{primary}" opacity="0.72"/>')
        # center white oval
        parts.append(f'<ellipse cx="{cx}" cy="{cy}" rx="{r-14}" ry="{r-20}" fill="{pal.bg}" opacity="0.85"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-30}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{primary}" letter-spacing="0.22em">{esc(kicker.upper())}</text>')
        if name:
...
SKIN_INSTANCE = RisoDuotoneSkin()


__all__ = [
    "RISO_DUOTONE", "PALETTE",
    "RisoDuotoneSkin", "SKIN_INSTANCE",

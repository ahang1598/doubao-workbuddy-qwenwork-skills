"""
Skin · bcg_forest · BCG Forest · Corporate Green

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: MBB-style thin ring, forest green.
"""
...
FONT_SANS = 'Neue Haas Grotesk, Helvetica Neue, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='BCG Forest · Sage White',
    bg='#F4F7F1',
    bg_alt='#E9EEE4',
    bg_dim='#E9EEE4',
    ink='#0D3320',
    gray='rgba(13,51,32,0.55)',
    hair='#0D3320',
    primary=HUE["rust"],
    ...
    signature_note="BCG FOREST",
)

# alias for registry contract
BCG_FOREST = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class BCGForestSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-8}" fill="none" stroke="{pal.bg}" stroke-width="0.7" opacity="0.55"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-30}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8.5" font-weight="700" fill="{accent}" letter-spacing="0.28em">{esc(kicker.upper())}</text>')
            parts.append(f'<line x1="{cx-24}" y1="{cy-22}" x2="{cx+24}" y2="{cy-22}" stroke="{accent}" stroke-width="0.5" opacity="0.6"/>')
        if name:
        ...
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.bg}" opacity="0.7" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = BCGForestSkin()


__all__ = [
    "BCG_FOREST", "PALETTE",
    "BCGForestSkin", "SKIN_INSTANCE",

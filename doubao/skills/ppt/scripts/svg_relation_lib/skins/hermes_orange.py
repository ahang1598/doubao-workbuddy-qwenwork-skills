"""
Skin · hermes_orange · Hermès Orange · Saddle

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: saddle stitch double-line + Hermès orange.
"""
...
FONT_SERIF = 'Serifa, Rockwell, Georgia, serif'
FONT_SANS = 'Univers, Helvetica Neue, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Hermès Orange · Saddle Peach',
    bg='#F8DEB2',
    bg_alt='#EEC98A',
    bg_dim='#EEC98A',
    ink='#3D1F0F',
    gray='rgba(61,31,15,0.55)',
    hair='#3D1F0F',
    primary=HUE["rust"],
    ...
    signature_note="HERMES ORANGE",
)

# alias for registry contract
HERMES_ORANGE = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class HermesOrangeSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # Hermès saddle stitch double-line + orange
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{primary}"/>')
        # inner saddle-stitch dashed border (white)
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-7}" fill="none" stroke="{pal.bg}" stroke-width="1.2" stroke-dasharray="4 2" opacity="0.85"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-26}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="9" font-weight="700" fill="{pal.bg}" letter-spacing="0.32em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.bg}" letter-spacing="0.02em">{esc(name)}</text>')
        if stat:
        ...
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.bg}" opacity="0.75" letter-spacing="0.18em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = HermesOrangeSkin()


__all__ = [
    "HERMES_ORANGE", "PALETTE",
    "HermesOrangeSkin", "SKIN_INSTANCE",

"""
Skin · arxiv_clean · Arxiv Clean · Latin Modern

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: thin double ruler + Computer Modern.
"""
...
FONT_SERIF = 'Latin Modern Roman, CMU Serif, EB Garamond, Georgia, serif'
FONT_SANS = 'Latin Modern Sans, Computer Modern, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Arxiv Clean · Latin Modern',
    bg='#FFFFFF',
    bg_alt='#F5F5F5',
    bg_dim='#F5F5F5',
    ink='#0A0A0A',
    gray='rgba(80,80,80,0.7)',
    hair='#0A0A0A',
    primary=HUE["rust"],
    ...
    signature_note="ARXIV CLEAN",
)

# alias for registry contract
ARXIV_CLEAN = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class ArxivCleanSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # arxiv paper: thin double ruler + Computer Modern
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg}" stroke="{pal.ink}" stroke-width="0.5"/>')
        parts.append(f'<line x1="{cx-r+8}" y1="{cy-r+18}" x2="{cx+r-8}" y2="{cy-r+18}" stroke="{pal.ink}" stroke-width="0.4"/>')
        parts.append(f'<line x1="{cx-r+8}" y1="{cy-r+22}" x2="{cx+r-8}" y2="{cy-r+22}" stroke="{pal.ink}" stroke-width="0.4"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-r+14}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="9" font-style="italic" fill="{pal.ink}">§{esc(kicker.title())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+2}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.ink}">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+26}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
...
SKIN_INSTANCE = ArxivCleanSkin()


__all__ = [
    "ARXIV_CLEAN", "PALETTE",
    "ArxivCleanSkin", "SKIN_INSTANCE",

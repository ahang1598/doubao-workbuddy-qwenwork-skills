"""
Skin · goldman_slate · Goldman Slate · IB Analyst Note

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: halo ring with GS deep blue + gold fig#.
"""
...
FONT_SERIF = 'Chronicle Display, Söhne, Georgia, serif'
FONT_SANS = 'Söhne, Helvetica Neue, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Goldman Slate · IB Analyst Note',
    bg='#FAFAFB',
    bg_alt='#F0F0F5',
    bg_dim='#F0F0F5',
    ink='#232328',
    gray='rgba(120,120,130,1)',
    hair='#232328',
    primary=HUE["rust"],
    ...
    signature_note="GOLDMAN SLATE",
)

# alias for registry contract
GOLDMAN_SLATE = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class GoldmanSlateSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # halo glow + GS blue center + gold Fig#
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r+8}" fill="none" stroke="{primary}" stroke-width="0.5" opacity="0.35"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{primary}"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-10}" fill="none" stroke="{accent}" stroke-width="0.6" opacity="0.65" stroke-dasharray="1 3"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8" font-weight="700" fill="{accent}" letter-spacing="0.30em">FIG · {esc(kicker.upper())}</text>')
        if name:
...
SKIN_INSTANCE = GoldmanSlateSkin()


__all__ = [
    "GOLDMAN_SLATE", "PALETTE",
    "GoldmanSlateSkin", "SKIN_INSTANCE",

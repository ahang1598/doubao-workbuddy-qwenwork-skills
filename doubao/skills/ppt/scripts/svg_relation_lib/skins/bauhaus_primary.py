"""
Skin · bauhaus_primary · Bauhaus Primary · Herbert Bayer

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: red circle + blue square + yellow triangle triad.
"""
...
FONT_SANS = 'Futura Bold, Futura, Century Gothic, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Bauhaus Primary · Yellow',
    bg='#FDE454',
    bg_alt='#F5D33A',
    bg_dim='#F5D33A',
    ink='#1A1A1A',
    gray='rgba(26,26,26,0.55)',
    hair='#1A1A1A',
    primary=HUE["rust"],
    ...
    signature_note="BAUHAUS PRIMARY",
)

# alias for registry contract
BAUHAUS_PRIMARY = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class BauhausPrimarySkin:
    name = 'bauhaus_primary'
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # Bauhaus red circle + blue square + yellow triangle triad
        # background triad tiny shapes
        parts.append(f'<circle cx="{cx-r-8}" cy="{cy-r-6}" r="8" fill="{primary}"/>')
        parts.append(f'<rect x="{cx+r-2}" y="{cy-r-12}" width="14" height="14" fill="{accent}"/>')
        parts.append(f'<path d="M {cx-6} {cy+r+16} L {cx+6} {cy+r+16} L {cx} {cy+r+4} Z" fill="#F0C808"/>')
        # main circle
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg}" stroke="{pal.ink}" stroke-width="1.8"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-24}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{pal.ink}" letter-spacing="0.24em">{esc(kicker.upper())}</text>')
        if name:
        ...
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.ink}" opacity="0.7" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = BauhausPrimarySkin()


__all__ = [
    "BAUHAUS_PRIMARY", "PALETTE",
    "BauhausPrimarySkin", "SKIN_INSTANCE",

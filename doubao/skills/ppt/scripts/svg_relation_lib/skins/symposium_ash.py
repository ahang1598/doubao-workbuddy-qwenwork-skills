"""
Skin · symposium_ash · Symposium Ash · Titanium + 朱印

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: square stamp seal + minimalist type.
"""
...
FONT_SERIF = 'Source Han Serif, Noto Serif CJK SC, EB Garamond, serif'
FONT_SANS = 'Source Han Sans, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Symposium Ash · Titanium + 朱印',
    bg='#E7E8E5',
    bg_alt='#DDDEDA',
    bg_dim='#DDDEDA',
    ink='#1F1F1F',
    gray='rgba(31,31,31,0.55)',
    hair='#1F1F1F',
    primary=HUE["rust"],
    ...
    signature_note="SYMPOSIUM ASH",
)

# alias for registry contract
SYMPOSIUM_ASH = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class SymposiumAshSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # circle + red square stamp seal (Chinese seal style)
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg}" stroke="{pal.ink}" stroke-width="1.4"/>')
        stamp_s = 22
        parts.append(f'<rect x="{cx+r-stamp_s-4}" y="{cy-r-4}" width="{stamp_s}" height="{stamp_s}" fill="{primary}" opacity="0.88"/>')
        parts.append(f'<text x="{cx+r-stamp_s/2-4}" y="{cy-r+stamp_s-8}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="12" font-weight="700" fill="{pal.bg}">印</text>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8.5" font-weight="700" fill="{pal.gray}" letter-spacing="0.32em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.ink}" letter-spacing="0.08em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+30}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="15" font-weight="700" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7.5" fill="{pal.gray}" letter-spacing="0.18em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = SymposiumAshSkin()


__all__ = [
    "SYMPOSIUM_ASH", "PALETTE",
    "SymposiumAshSkin", "SKIN_INSTANCE",

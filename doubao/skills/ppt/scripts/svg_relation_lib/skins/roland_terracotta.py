"""
Skin · roland_terracotta · Roland Berger Terracotta · Craft

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: hand-crafted thick frame + serif italic.
"""
...
FONT_SERIF = 'Times New Roman, GT Sectra, Georgia, serif'
...
PALETTE_OBJ = Palette(
    name='Roland Berger Terracotta · Craft',
    bg='#F5EFE6',
    bg_alt='#EDE5D6',
    bg_dim='#EDE5D6',
    ink='#2E1F17',
    gray='rgba(46,31,23,0.55)',
    hair='#2E1F17',
    primary=HUE["rust"],
    ...
    signature_note="ROLAND TERRACOTTA",
)

# alias for registry contract
ROLAND_TERRACOTTA = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class RolandTerracottaSkin:
    name = 'roland_terracotta'
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # thick hand-crafted frame + serif italic
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg_alt}" stroke="{primary}" stroke-width="3.5"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-8}" fill="none" stroke="{primary}" stroke-width="0.5" opacity="0.5"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="9.5" font-style="italic" fill="{primary}" letter-spacing="0.22em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" font-style="italic" fill="{pal.ink}">{esc(name)}</text>')
        if stat:
        ...
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.gray}" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = RolandTerracottaSkin()


__all__ = [
    "ROLAND_TERRACOTTA", "PALETTE",
    "RolandTerracottaSkin", "SKIN_INSTANCE",

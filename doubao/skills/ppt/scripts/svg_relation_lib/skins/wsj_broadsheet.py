"""
Skin · wsj_broadsheet · WSJ Broadsheet · Newspaper Cream

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: broadsheet headline serif + kicker rule.
"""
...
FONT_SERIF = 'GT Sectra, Playfair Display, Georgia, serif'
FONT_SANS = 'Söhne, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='WSJ Broadsheet · Newspaper Cream',
    bg='#F1EBD8',
    bg_alt='#E8E1CB',
    bg_dim='#E8E1CB',
    ink='#141410',
    gray='rgba(122,117,106,1)',
    hair='#141410',
    primary=HUE["rust"],
    ...
    signature_note="WSJ BROADSHEET",
)

# alias for registry contract
WSJ_BROADSHEET = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class WSJBroadsheetSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # broadsheet headline serif + kicker rule (WSJ style)
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg_alt}"/>')
        parts.append(f'<line x1="{cx-r}" y1="{cy-r+3}" x2="{cx+r}" y2="{cy-r+3}" stroke="{primary}" stroke-width="2.4"/>')
        parts.append(f'<line x1="{cx-r}" y1="{cy+r-3}" x2="{cx+r}" y2="{cy+r-3}" stroke="{primary}" stroke-width="0.6"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-26}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8.5" font-weight="700" fill="{primary}" letter-spacing="0.32em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="24" font-weight="700" fill="{pal.ink}" letter-spacing="-0.01em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+28}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="15" font-weight="700" font-style="italic" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="8" font-style="italic" fill="{pal.gray}">{esc(stat_note)}</text>')

        return "".join(parts)


SKIN_INSTANCE = WSJBroadsheetSkin()


__all__ = [
    "WSJ_BROADSHEET", "PALETTE",
    "WSJBroadsheetSkin", "SKIN_INSTANCE",

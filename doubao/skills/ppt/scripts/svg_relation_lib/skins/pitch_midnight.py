"""
Skin · pitch_midnight · Pitch Midnight · TED Talk

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: dark midnight spotlight + champagne gold.
"""
...
FONT_SERIF = 'Playfair Display, GT Sectra, Georgia, serif'
FONT_SANS = 'Inter Display, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Pitch Midnight · TED Talk',
    bg='#0C1729',
    bg_alt='#1A2438',
    bg_dim='#1A2438',
    ink='#F5F0DC',
    gray='rgba(245,240,220,0.55)',
    hair='#F5F0DC',
    primary=HUE["rust"],
    ...
    signature_note="PITCH MIDNIGHT",
)

# alias for registry contract
PITCH_MIDNIGHT = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class PitchMidnightSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # TED spotlight: radial dark + champagne
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r+6}" fill="none" stroke="{primary}" stroke-width="4" opacity="0.14"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r+2}" fill="none" stroke="{primary}" stroke-width="1.4" opacity="0.5"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg}"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{primary}" letter-spacing="0.30em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="24" font-weight="700" font-style="italic" fill="{pal.ink}" letter-spacing="0.02em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+30}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="16" font-weight="700" font-style="italic" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.ink}" opacity="0.6" letter-spacing="0.16em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = PitchMidnightSkin()


__all__ = [
    "PITCH_MIDNIGHT", "PALETTE",
    "PitchMidnightSkin", "SKIN_INSTANCE",

"""
Skin · terminal_matrix · Terminal Matrix · Green Phosphor

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: [bracket] terminal frame + phosphor green.
"""
...
FONT_SERIF = 'JetBrains Mono, Menlo, monospace'
...
PALETTE_OBJ = Palette(
    name='Terminal Matrix · Green Phosphor',
    bg='#0A0F0A',
    bg_alt='#141A14',
    bg_dim='#141A14',
    ink='#39FF14',
    gray='rgba(57,255,20,0.5)',
    hair='#39FF14',
    primary=HUE["rust"],
    ...
    signature_note="TERMINAL MATRIX",
)

# alias for registry contract
TERMINAL_MATRIX = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class TerminalMatrixSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # [bracket] terminal frame + phosphor green
        parts.append(f'<rect x="{cx-r}" y="{cy-r}" width="{r*2}" height="{r*2}" fill="{pal.bg}" stroke="{primary}" stroke-width="1"/>')
        # corner brackets
        bl = 12
        for sx, sy in [(-1,-1),(1,-1),(-1,1),(1,1)]:
            px = cx + sx * (r - 4); py = cy + sy * (r - 4)
            parts.append(f'<path d="M {px-sx*bl} {py} L {px} {py} L {px} {py-sy*bl}" stroke="{primary}" stroke-width="2" fill="none"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-24}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{primary}" letter-spacing="0.20em">[ {esc(kicker.upper())} ]</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SANS}" font-size="20" font-weight="700" fill="{primary}" letter-spacing="0em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="16" font-weight="700" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7.5" fill="{primary}" opacity="0.7" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = TerminalMatrixSkin()


__all__ = [
    "TERMINAL_MATRIX", "PALETTE",
    "TerminalMatrixSkin", "SKIN_INSTANCE",

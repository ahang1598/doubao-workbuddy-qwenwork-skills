"""
Skin · chalkboard_slate · Chalkboard Slate · Classroom

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: chalk scribble double-circle + dashed.
"""
...
FONT_SERIF = 'Chalkduster, Comic Sans MS, Kalam, cursive'
FONT_SANS = 'Chalkduster, Kalam, Comic Sans MS, cursive'
...
PALETTE_OBJ = Palette(
    name='Chalkboard Slate · Classroom',
    bg='#2B4A3A',
    bg_alt='#3A5A48',
    bg_dim='#3A5A48',
    ink='#FEFEF0',
    gray='rgba(254,254,240,0.5)',
    hair='#FEFEF0',
    primary=HUE["rust"],
    ...
    signature_note="CHALKBOARD SLATE",
)

# alias for registry contract
CHALKBOARD_SLATE = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class ChalkboardSlateSkin:
    name = 'chalkboard_slate'
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # chalkboard scribble double-circle
        parts.append(f'<circle cx="{cx-2}" cy="{cy+1}" r="{r}" fill="none" stroke="{primary}" stroke-width="1.8" stroke-dasharray="4 2" opacity="0.9"/>')
        parts.append(f'<circle cx="{cx+2}" cy="{cy-1}" r="{r-3}" fill="none" stroke="{primary}" stroke-width="0.8" opacity="0.5"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="10" fill="{accent}">✎ {esc(kicker)}</text>')
        if name:
        ...
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8" fill="{primary}" opacity="0.7">— {esc(stat_note)} —</text>')

        return "".join(parts)


SKIN_INSTANCE = ChalkboardSlateSkin()


__all__ = [
    "CHALKBOARD_SLATE", "PALETTE",
    "ChalkboardSlateSkin", "SKIN_INSTANCE",

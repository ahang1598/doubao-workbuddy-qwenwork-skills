"""
Skin · manuscript_sepia · Manuscript Sepia · Vellum + Ink

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: illuminated capital + border flourish.
"""
...
FONT_SERIF = 'IM Fell English, EB Garamond, Georgia, serif'
FONT_SANS = 'EB Garamond, Georgia, serif'
...
PALETTE_OBJ = Palette(
    name='Manuscript Sepia · Vellum + Ink',
    bg='#F0E4C8',
    bg_alt='#E8DBB8',
    bg_dim='#E8DBB8',
    ink='#3D2817',
    gray='rgba(61,40,23,0.5)',
    hair='#3D2817',
    primary=HUE["rust"],
    ...
    signature_note="MANUSCRIPT SEPIA",
)

# alias for registry contract
MANUSCRIPT_SEPIA = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class ManuscriptSepiaSkin:
    name = 'manuscript_sepia'
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg_alt}" stroke="{primary}" stroke-width="2"/>')
        # illuminated corner flourishes (four L-shaped)
        for sx, sy in [(-1,-1),(1,-1),(-1,1),(1,1)]:
            px = cx + sx * (r - 6)
            py = cy + sy * (r - 6)
            parts.append(f'<path d="M {px} {py} l {sx*10} 0 M {px} {py} l 0 {sy*10}" stroke="{accent}" stroke-width="1.2" fill="none"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-30}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="9.5" font-style="italic" fill="{accent}" letter-spacing="0.28em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.ink}">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+30}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="15" font-weight="700" font-style="italic" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="7.5" font-style="italic" fill="{pal.gray}" letter-spacing="0.14em">{esc(stat_note)}</text>')

        return "".join(parts)


SKIN_INSTANCE = ManuscriptSepiaSkin()


__all__ = [
    "MANUSCRIPT_SEPIA", "PALETTE",
    "ManuscriptSepiaSkin", "SKIN_INSTANCE",

"""
Skin · annual_report_ivory · Annual Report · Ivory + Deep Forest

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: chapter number rondel + deep forest.
"""
...
FONT_SERIF = 'Baskerville, EB Garamond, Georgia, serif'
...
PALETTE_OBJ = Palette(
    name='Annual Report · Cool Ash + Deep Forest',
    bg='#ECEEE9',
    bg_alt='#DFE3DC',
    bg_dim='#DFE3DC',
    ink='#0A1A0F',
    gray='rgba(10,26,15,0.55)',
    hair='#0A1A0F',
    primary=HUE["rust"],
    ...
    signature_note="ANNUAL REPORT IVORY",
)

# alias for registry contract
ANNUAL_REPORT_IVORY = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class AnnualReportIvorySkin:
    name = 'annual_report_ivory'
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # chapter number rondel + deep forest (annual report)
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{primary}"/>')
        parts.append(f'<circle cx="{cx-r+16}" cy="{cy-r+16}" r="14" fill="{accent}"/>')
        parts.append(f'<text x="{cx-r+16}" y="{cy-r+20}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" font-style="italic" fill="{primary}">V</text>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-24}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8.5" font-weight="700" fill="{accent}" letter-spacing="0.32em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.bg}">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+30}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" font-style="italic" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.bg}" opacity="0.7" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = AnnualReportIvorySkin()


__all__ = [
    "ANNUAL_REPORT_IVORY", "PALETTE",
    "AnnualReportIvorySkin", "SKIN_INSTANCE",

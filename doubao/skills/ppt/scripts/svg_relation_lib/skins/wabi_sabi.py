"""
Skin · wabi_sabi · Wabi Sabi · Kyoto Ink

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: ink brush circle + red hanko square seal.
"""
...
FONT_SERIF = 'Noto Serif JP, Source Han Serif, EB Garamond, serif'
FONT_SANS = 'Noto Sans JP, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Wabi Sabi · Kyoto Ink',
    bg='#F0E8D4',
    bg_alt='#E5DBC0',
    bg_dim='#E5DBC0',
    ink='#1A1611',
    gray='rgba(26,22,17,0.55)',
    hair='#1A1611',
    primary=HUE["rust"],
    ...
    signature_note="WABI SABI",
)

# alias for registry contract
WABI_SABI = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class WabiSabiSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # wabi-sabi: ink brush circle + red hanko square seal
        # main brush circle (rough stroke)
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg_alt}" stroke="{pal.ink}" stroke-width="2.4" opacity="0.92"/>')
        parts.append(f'<circle cx="{cx-1}" cy="{cy+1}" r="{r-2}" fill="none" stroke="{pal.ink}" stroke-width="0.5" opacity="0.4"/>')
        # hanko red square seal bottom-right
        parts.append(f'<rect x="{cx+r-20}" y="{cy+r-4}" width="24" height="24" fill="{accent}"/>')
        parts.append(f'<text x="{cx+r-8}" y="{cy+r+13}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" fill="{pal.bg}">和</text>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="9" fill="{pal.ink}" letter-spacing="0.32em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.ink}" letter-spacing="0.06em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+30}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="7.5" fill="{pal.gray}" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = WabiSabiSkin()


__all__ = [
    "WABI_SABI", "PALETTE",
    "WabiSabiSkin", "SKIN_INSTANCE",

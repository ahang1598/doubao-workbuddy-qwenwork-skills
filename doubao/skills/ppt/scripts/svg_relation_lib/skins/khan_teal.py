"""
Skin · khan_teal · Khan Teal · Rounded

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: rounded corners + progress dot + Lato.
"""
...
FONT_SANS = 'Lato, Open Sans, Inter, sans-serif'
...
PALETTE_OBJ = Palette(
    name='Khan Teal · Rounded',
    bg='#FFFFFF',
    bg_alt='#F0F9F8',
    bg_dim='#F0F9F8',
    ink='#1F2E3A',
    gray='rgba(31,46,58,0.55)',
    hair='#1F2E3A',
    primary=HUE["rust"],
    ...
    signature_note="KHAN TEAL",
)

# alias for registry contract
KHAN_TEAL = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class KhanTealSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # Khan: rounded pill + progress dot + Lato
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{primary}"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-5}" fill="none" stroke="{pal.bg}" stroke-width="0.5" opacity="0.6"/>')
        parts.append(f'<circle cx="{cx+r-8}" cy="{cy-r+8}" r="4" fill="{accent}" stroke="{pal.bg}" stroke-width="1.4"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-30}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{accent}" letter-spacing="0.22em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SANS}" font-size="22" font-weight="700" fill="{pal.bg}" letter-spacing="-0.005em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="15" font-weight="700" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
...
SKIN_INSTANCE = KhanTealSkin()


__all__ = [
    "KHAN_TEAL", "PALETTE",
    "KhanTealSkin", "SKIN_INSTANCE",

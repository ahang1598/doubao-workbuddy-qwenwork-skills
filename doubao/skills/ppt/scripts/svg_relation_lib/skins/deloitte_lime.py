"""
Skin · deloitte_lime · Deloitte Lime · Big Four

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: left vertical rule + minimalist.
"""
...
PALETTE_OBJ = Palette(
    name='Deloitte Lime · Slate',
    bg='#EEEFEA',
    bg_alt='#E2E4DD',
    bg_dim='#E2E4DD',
    ink='#000000',
    gray='rgba(30,30,30,0.55)',
    hair='#000000',
    primary=HUE["rust"],
    ...
    signature_note="DELOITTE LIME",
)

# alias for registry contract
DELOITTE_LIME = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class DeloitteLimeSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # left vertical rule + minimalist (Deloitte lime style)
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg}" stroke="{pal.ink}" stroke-width="0.7"/>')
        parts.append(f'<rect x="{cx-r}" y="{cy-r+8}" width="4" height="{r*2-16}" fill="{primary}"/>')
        if kicker:
            parts.append(f'<text x="{cx+4}" y="{cy-28}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8.5" font-weight="700" fill="{primary}" letter-spacing="0.22em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx+4}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SANS}" font-size="22" font-weight="700" fill="{pal.ink}">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx+4}" y="{cy+28}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx+4}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.gray}" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = DeloitteLimeSkin()


__all__ = [
    "DELOITTE_LIME", "PALETTE",
    "DeloitteLimeSkin", "SKIN_INSTANCE",

"""
Skin · oxford_indigo · Oxford Indigo · University Crest

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: crest badge + gothic frame.
"""
...
FONT_SERIF = 'Bodoni MT, Bodoni 72, Playfair Display, Georgia, serif'
FONT_SANS = 'Inter, Optima, sans-serif'
...
PALETTE_OBJ = Palette(
    ...
    signature_note="OXFORD INDIGO",
)

# alias for registry contract
OXFORD_INDIGO = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class OxfordIndigoSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-6}" fill="none" stroke="{accent}" stroke-width="0.7" opacity="0.7"/>')
        parts.append(f'<path d="M {cx-8} {cy-r+8} L {cx} {cy-r+2} L {cx+8} {cy-r+8} L {cx+8} {cy-r+20} L {cx} {cy-r+26} L {cx-8} {cy-r+20} Z" fill="{accent}"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-24}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8" font-weight="700" fill="{accent}" letter-spacing="0.30em">{esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.bg}" letter-spacing="0.04em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+28}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="14" font-weight="700" fill="{accent}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+46}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.bg}" opacity="0.72" letter-spacing="0.18em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = OxfordIndigoSkin()


__all__ = [
    "OXFORD_INDIGO", "PALETTE",
    "OxfordIndigoSkin", "SKIN_INSTANCE",

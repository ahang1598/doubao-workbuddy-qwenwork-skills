"""
Skin · bloomberg_amber · Bloomberg Amber · Terminal

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: terminal panel + tabular-num orange.
"""
...
FONT_SANS = 'IBM Plex Mono, JetBrains Mono, Menlo, monospace'
...
PALETTE_OBJ = Palette(
    ...
    signature_note="BLOOMBERG AMBER",
)

# alias for registry contract
BLOOMBERG_AMBER = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class BloombergAmberSkin:
    name = 'bloomberg_amber'
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # terminal panel + tabular-num orange (Bloomberg)
        parts.append(f'<rect x="{cx-r}" y="{cy-r}" width="{r*2}" height="{r*2}" fill="{pal.bg_alt}" stroke="{primary}" stroke-width="0.8"/>')
        parts.append(f'<rect x="{cx-r}" y="{cy-r}" width="{r*2}" height="14" fill="{primary}"/>')
        parts.append(f'<text x="{cx-r+5}" y="{cy-r+10}" font-family="{FONT_SANS}" font-size="8" font-weight="700" fill="{pal.bg}" letter-spacing="0.16em">TERMINAL</text>')
        parts.append(f'<circle cx="{cx+r-6}" cy="{cy-r+7}" r="2" fill="{accent}"/>')
        if kicker:
            parts.append(f'<text x="{cx}" y="{cy-20}" text-anchor="middle" font-family="{FONT_SANS}" font-size="8.5" font-weight="700" fill="{primary}" letter-spacing="0.18em">&gt; {esc(kicker.upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+6}" text-anchor="middle" font-family="{FONT_SANS}" font-size="22" font-weight="700" fill="{pal.ink}" letter-spacing="-0.01em">{esc(name)}</text>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+30}" text-anchor="middle" font-family="{FONT_SANS}" font-size="16" font-weight="700" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+50}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7.5" fill="{pal.gray}" letter-spacing="0.14em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = BloombergAmberSkin()


__all__ = [
    "BLOOMBERG_AMBER", "PALETTE",
    "BloombergAmberSkin", "SKIN_INSTANCE",

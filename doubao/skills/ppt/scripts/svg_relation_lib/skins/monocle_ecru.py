"""
Skin · monocle_ecru · Monocle Ecru · Editorial Travel

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: editorial medallion + olive+tangerine.
"""
...
FONT_SERIF = 'Plantin, Georgia, GT Sectra, serif'
...
PALETTE_OBJ = Palette(
    name='Monocle Ecru · Mustard',
    bg='#EBDBAF',
    bg_alt='#DDCB94',
    bg_dim='#DDCB94',
    ink='#1E2E1A',
    gray='rgba(30,46,26,0.55)',
    hair='#1E2E1A',
    primary=HUE["rust"],
    ...
    signature_note="MONOCLE ECRU",
)

# alias for registry contract
MONOCLE_ECRU = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class MonocleEcruSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        # Monocle medallion: circle + kerned wordmark
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{primary}"/>')
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r-6}" fill="none" stroke="{accent}" stroke-width="0.7" opacity="0.6"/>')
        # top accent dot
        parts.append(f'<circle cx="{cx}" cy="{cy-r+6}" r="3" fill="{accent}"/>')
        if kicker:
        ...
        if name:
            parts.append(f'<text x="{cx}" y="{cy+4}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="22" font-weight="700" fill="{pal.bg}" letter-spacing="0.08em">{esc(name.upper())}</text>')
        if stat:
        ...
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+48}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7" fill="{pal.bg}" opacity="0.7" letter-spacing="0.18em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = MonocleEcruSkin()


__all__ = [
    "MONOCLE_ECRU", "PALETTE",
    "MonocleEcruSkin", "SKIN_INSTANCE",

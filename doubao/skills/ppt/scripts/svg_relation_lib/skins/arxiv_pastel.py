"""
Skin · arxiv_pastel · Arxiv Pastel · Academic Cool White

Auto-generated as part of the 40-skin mindmap sprint.
Hub decoration: arxiv tab-header + underline.
"""
...
FONT_SERIF = 'Charter, IBM Plex Serif, Georgia, serif'
...
PALETTE_OBJ = Palette(
    name='Arxiv Pastel · Academic Cool White',
    bg='#FAFAFA',
    bg_alt='#EEF0F3',
    bg_dim='#EEF0F3',
    ink='#1A2438',
    gray='rgba(70,80,100,0.6)',
    hair='#1A2438',
    primary=HUE["rust"],
    ...
    signature_note="ARXIV PASTEL",
)

# alias for registry contract
ARXIV_PASTEL = PALETTE_OBJ
PALETTE = PALETTE_OBJ
...
class ArxivPastelSkin:
    ...
    def _mindmap_hub(self, x, y, w, h, name, pal, **kw):
        ...
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{r}" fill="{pal.bg_alt}" stroke="{primary}" stroke-width="1.4"/>')
        parts.append(f'<rect x="{cx-40}" y="{cy-r-2}" width="80" height="18" fill="{primary}"/>')
        parts.append(f'<text x="{cx}" y="{cy-r+11}" text-anchor="middle" font-family="{FONT_SANS}" font-size="9" font-weight="700" fill="{pal.bg}" letter-spacing="0.18em">{esc((kicker or "TAB").upper())}</text>')
        if name:
            parts.append(f'<text x="{cx}" y="{cy+2}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="20" font-weight="700" fill="{pal.ink}">{esc(name)}</text>')
        parts.append(f'<line x1="{cx-30}" y1="{cy+14}" x2="{cx+30}" y2="{cy+14}" stroke="{primary}" stroke-width="0.6" opacity="0.5"/>')
        if stat:
            parts.append(f'<text x="{cx}" y="{cy+34}" text-anchor="middle" font-family="{FONT_SERIF}" font-size="16" font-weight="700" fill="{primary}">{esc(stat)}</text>')
        if stat_note:
            parts.append(f'<text x="{cx}" y="{cy+50}" text-anchor="middle" font-family="{FONT_SANS}" font-size="7.5" fill="{pal.gray}" letter-spacing="0.16em">{esc(stat_note.upper())}</text>')

        return "".join(parts)


SKIN_INSTANCE = ArxivPastelSkin()


__all__ = [
    "ARXIV_PASTEL", "PALETTE",
    "ArxivPastelSkin", "SKIN_INSTANCE",

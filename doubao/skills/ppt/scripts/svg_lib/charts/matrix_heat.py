...
def draw_matrix_heat(
    ...
    _max_row_label_px = max(
        (
            _estimate_label_width_px(lbl, label_fs, 0.0, bold=True, font_family="Inter, sans-serif")
            for lbl in _row_label_strs
        ),
        default=0.0,
    )
    ...
    def cx(i):
        return ml_grid + i * cell + cell / 2

    def cy(j):
        return mt_grid + j * cell + cell / 2
        ...
        for i in range(n_cols):
            ...
            parts.append(
                _render_cell(
                    ...
                    ink,
                    ink2,
                    vspan_min if diverging else vmin,
                    vspan_max if diverging else vmax,
                    diverging,
                    label_fs,
                )
            )
    ...
    if diverging:
        ticks = [(vspan_min, f"{vspan_min:.1f}"), (0, "0"), (vspan_max, f"+{vspan_max:.1f}")]
        for tv, tl in ticks:
...
def _render_cell(variant, x0, y0, cell, v, cell_col, txt_col, ink, ink2, vlow, vhigh, diverging, label_fs):
    ...
    if variant == "circle_full":
        ...
        s += f'<circle cx="{cxp:.1f}" cy="{cyp:.1f}" r="{radius:.1f}" fill="{cell_col}"/>'
        return s
...
def make_matrix_heat(
    ...
    is_legacy = (
        ...
        and n_rows_raw == n_cols_raw
        and any(v == -1 for row in matrix for v in row)
    )
    ...
    def _cjk_extra_px(_s, _fs, _bold):
        ...
        for _ch in _s:
            ...
            if (
                0x2E80 <= _code <= 0x9FFF
                or 0x3000 <= _code <= 0xD7AF
                or 0xF900 <= _code <= 0xFAFF
                or 0xFE30 <= _code <= 0xFE4F
                or 0xFF01 <= _code <= 0xFF60
                or 0xFFE0 <= _code <= 0xFFE6
            ):
    ...
    _max_row_label_px = max(
        (
            _estimate_label_width_px(lbl, row_label_font, 0.0, bold=True, font_family=_body_font)
            + _cjk_extra_px(lbl, row_label_font, True)
            for lbl in _row_label_strs
        ),
        default=0.0,
    )
    ...
    if show_colorbar or show_row_mean:
        ...
        _rm_max_txt_w = max(
            (
                _estimate_label_width_px(_fmt.format(rm), _rm_fs_upper, 0.0, bold=False, font_family=_body_font)
                for rm in _row_means_est
            ),
            default=0.0,
        )
        ...
        _tick_max_w = max(
            (
                _estimate_label_width_px(
                    t, min(14.0, 12 * max(0.7, min(1.8, 1.0))), 0.0, bold=False, font_family=_body_font
                )
        ...
        _sub_default = (
            f"Diverging · midpoint {vmin:g}"
            if (vmin < 0 and vmax > 0) or (midpoint is not None)
            else f"Sequential · {vmin:g}-{vmax:g}"
        )
        _sub_est = colorbar_sub or _sub_default
        _sub_w = _estimate_label_width_px(_sub_est, _fs_bar_est, 0.05, bold=False, font_family=_body_font)
        _label_w = _estimate_label_width_px(
            colorbar_label or "", _fs_bar_est * 1.15, 0.14, bold=True, font_family=_body_font
        )
    ...
    def cell_x(mi):
        return MARGIN_L + mi * cell_w

    def cell_y(bi):
        return MARGIN_T + bi * cell_h
    ...
    if subtitle:
        ...
        parts.append(
            f'<line x1="{MARGIN_L}" y1="{_y_line:.1f}" x2="{W - 40}" y2="{_y_line:.1f}" '
    ...
    if figure_label:
        ...
        parts.append(
            f'<text x="{MARGIN_L + label_w}" y="{_y_figure:.1f}" font-family="{_body_font}" font-size="{fs_figure}" '
    ...
    for bi, bname in enumerate(row_labels):
        ...
        parts.append(
            f'<text x="{MARGIN_L - 12}" y="{y - (3 if sub else -3):.1f}" text-anchor="end" '
            f'font-family="{_body_font}" font-size="{row_label_font:.1f}" font-weight="600" '
            f'fill="{_INK}">{_xesc(str(bname))}</text>'
        )
        if sub:
            parts.append(
                f'<text x="{MARGIN_L - 12}" y="{y + 10:.1f}" text-anchor="end" '
    ...
    for bi in range(n_rows):
        for mi in range(n_cols):
            ...
            fmt_str = "{:" + value_fmt + "}"
            parts.append(
                f'<text x="{x + cell_w / 2:.1f}" y="{y + cell_h / 2 + cell_font_size * 0.32:.1f}" '
                f'text-anchor="middle" font-family="{_body_font}" '
    ...
    if domain_groups:
        for dg in domain_groups:
            ...
            parts.append(
                f'<text x="{domain_x}" y="{y_mid + 3:.1f}" '
    ...
    if show_row_mean:
        ...
        parts.append(
            f'<text x="{bar_x}" y="{MARGIN_T - 8}" font-family="{_body_font}" '
        ...
        for bi, avg in enumerate(row_means):
            ...
            parts.append(
                f'<rect x="{bar_x}" y="{y - 4:.1f}" width="{max(0.5, bar_len):.1f}" height="8" '
            ...
            parts.append(
                f'<text x="{bar_x + bar_maxw + 6}" y="{y + 3:.1f}" text-anchor="start" '
    ...
    if show_col_mean:
        ...
        _col_max_txt_upper = max(
            (
                _estimate_label_width_px(fmt_str.format(a), _fs_col_mean_upper, 0.0, bold=True, font_family=_body_font)
                for a in col_means
            ),
            default=0.0,
        )
        ...
        parts.append(
            f'<text x="{MARGIN_L - 12}" y="{y_bot:.1f}" text-anchor="end" '
        ...
        for mi, avg in enumerate(col_means):
            x = cell_x(mi) + cell_w / 2
            parts.append(
                f'<text x="{x:.1f}" y="{y_bot:.1f}" text-anchor="middle" '
    ...
    if show_colorbar:
        ...
        parts.append(
            f'<defs><linearGradient id="cbGrad_{id(matrix) & 0xFFFF}" x1="0" y1="1" x2="0" y2="0">'
            f"{''.join(grad_stops)}"
            f"</linearGradient></defs>"
        )
        parts.append(
        ...
        for tv in cb_ticks:
            ...
            parts.append(
                f'<line x1="{cb_x + cb_w}" y1="{ty:.1f}" x2="{cb_x + cb_w + 4}" y2="{ty:.1f}" '
            ...
            parts.append(
                f'<text x="{cb_x + cb_w + 10}" y="{ty + 3.5:.1f}" text-anchor="start" '
        ...
        parts.append(
            f'<text x="{cb_x - 4}" y="{_y_value:.1f}" font-family="{_body_font}" '
        ...
        parts.append(
            f'<text x="{cb_x - 4}" y="{_y_sub:.1f}" font-family="{_body_font}" '
            f'font-size="{fs_bar}" fill="{c_muted}" letter-spacing=".05em">'
            f"{_xesc(sub_txt)}</text>"
        )
        # ↑↓ 副注
        parts.append(
            f'<text x="{cb_x + cb_w / 2:.1f}" y="{_y_arrow:.1f}" text-anchor="middle" '
            f'font-family="{_body_font}" font-size="{_fs_cb_arrow}" fill="{c_muted}" '
            f'letter-spacing=".03em">↑ Above midpoint</text>'
        )
        parts.append(
            f'<text x="{cb_x + cb_w / 2:.1f}" y="{cb_y + cb_h + _gap * 1.5:.1f}" text-anchor="middle" '
            f'font-family="{_body_font}" font-size="{_fs_cb_arrow}" fill="{c_muted}" '

...
def draw_funnel(
    ...
    header = _header(ctx)
    return svg_open(0, 0, ctx.W, ctx.H, bg=pal["bg"]) + ctx.defs_svg() + header + body + svg_close()
...
def _draw_rectangle_flat(ctx: _Ctx) -> str:
    ...
    for i in range(n):
        ...
        col = ctx.series_color(i)
        parts.append(
            f'<rect x="{x:.1f}" y="{y:.1f}" width="{w:.1f}" height="{row_h:.1f}" fill="{_rgba_with_alpha(col, 0.88)}"/>'
        )
...
def _draw_bar_lollipop(ctx: _Ctx) -> str:
    ...
    for i in range(n):
        ...
        parts.append(f'<circle cx="{disc_cx:.1f}" cy="{y:.1f}" r="{r:.1f}" fill="{_rgba_with_alpha(col, 0.18)}"/>')
        parts.append(f'<circle cx="{disc_cx:.1f}" cy="{y:.1f}" r="{max(r - 3, r * 0.55):.1f}" fill="{col}"/>')
...
def _draw_nested_arrow(ctx: _Ctx) -> str:
    ...
    for i in range(n):
        ...
        parts.append(f'<path d="{path}" fill="{_rgba_with_alpha(col, 0.92)}"/>')
...
def make_funnel_classic(
    ...
    if not _variant_is_classic("funnel_classic", variant):
        _data = {
            "stages": list(zip(stage_labels or [f"Stage {i + 1}" for i in range(len(stages))], stages)),
    ...
    def row_top(i):
        return funnel_top + i * (row_h + row_gap)

    def row_bot(i):
        return row_top(i) + row_h
    ...
    def width_of(i):
        return plot_w_max * stages[i] / top_count
    ...
    if figure_label:
        parts.append(
            f'<text x="{MARGIN_L}" y="112" font-family="{_body_font}" font-size="{_fs_col_header}" '
            f'fill="{c_muted}" font-weight="600" letter-spacing=".15em">{_xesc(figure_label)}</text>'
        )
        lbl_w = max(72, len(figure_label) * 8 + 20)
        parts.append(
            f'<text x="{MARGIN_L + lbl_w}" y="112" font-family="{_body_font}" font-size="{_fs_col_header}" '
    ...
    for i in range(n):
        ...
        if top_w >= 200:
            ...
            parts.append(
                f'<text x="{funnel_cx:.1f}" y="{_label_baseline:.1f}" text-anchor="middle" '
                f'font-family="{_body_font}" font-size="{_fs_body_stage}" font-weight="600" '
                f'fill="{text_col}">{_xesc(str(name))}</text>'
            )
            parts.append(
                f'<text x="{funnel_cx:.1f}" y="{_value_baseline:.1f}" text-anchor="middle" '
            ...
            if desc:
                ...
                if _desc_bbox_bot <= yb - 1.0:
                    parts.append(
                        f'<text x="{funnel_cx:.1f}" y="{_desc_baseline:.1f}" text-anchor="middle" '
    ...
    if show_drop_off:
        for i in range(n - 1):
            ...
            parts.append(
                f'<text x="{label_x}" y="{y + _big_baseline_offset:.1f}" text-anchor="end" '
                f'font-family="{_head_font}" font-size="{_fs_drop_big}" font-weight="700" '
                f'fill="{RED_COL}">−{lost_rate:.1f}%</text>'
            )
            parts.append(
                f'<text x="{label_x}" y="{y + _sub_baseline_offset:.1f}" text-anchor="end" '
            ...
            if x_arrow > x_from:
                ...
                parts.append(
                    f'<line x1="{x_from}" y1="{y:.1f}" x2="{x_arrow:.1f}" y2="{y:.1f}" '
                    f'stroke="{red_dim}" stroke-width="0.8" stroke-dasharray="2 2"/>'
                )
                parts.append(
                    f'<polygon points="{x_arrow + 6:.1f},{y:.1f} {x_arrow - 2:.1f},{y - 3:.1f} '
    ...
    if show_cumulative:
        ...
        for i in range(n):
            ...
            by = y_mid - bar_h / 2
            parts.append(
                f'<rect x="{right_x_bar_start:.1f}" y="{by:.1f}" width="{bar_area_w:.1f}" '
            ...
            parts.append(
                f'<rect x="{right_x_bar_start:.1f}" y="{by + 1:.1f}" width="{fill_w:.1f}" '
                f'height="{bar_h - 2}" fill="{_rgba_with_alpha(color, 0.9)}" rx="2"/>'
            )
            parts.append(
                f'<text x="{right_x_bar_end + 8:.1f}" y="{y_mid + 4:.1f}" text-anchor="start" '
    ...
    if show_insights:
        ...
        insights = [
            ("OVERALL CONVERSION", f"{overall_conv:.2f}%", f"{stages[0]:,} → {stages[-1]:,}"),
            (
                "BIGGEST DROP-OFF",
                f"−{biggest_drop_pct:.0f}%",
        ...
        for i, (label, big, sub) in enumerate(insights):
            ...
            parts.append(
                f'<rect x="{ix:.1f}" y="{insight_y}" width="{insight_w:.1f}" height="{_insight_h:.1f}" '
            ...
            parts.append(
                f'<text x="{ix + 16:.1f}" y="{insight_y + _hdr_baseline:.1f}" font-family="{_body_font}" '
                f'font-size="{_fs_insight_hdr}" fill="{c_muted}" font-weight="600" letter-spacing=".15em">'
                f"{_xesc(label)}</text>"
            )
            big_col = RED_COL if i == 1 else _INK
            parts.append(
                f'<text x="{ix + 16:.1f}" y="{insight_y + _big_baseline:.1f}" font-family="{_head_font}" '
                f'font-size="{_fs_insight_big}" fill="{big_col}" font-weight="700">{_xesc(big)}</text>'
            )
            parts.append(
                f'<text x="{ix + 16:.1f}" y="{insight_y + _sub_baseline:.1f}" font-family="{_body_font}" '
    ...
    if note:
        ...
        if figure_label:
            parts.append(
                f'<text x="{width - MARGIN_R}" y="{foot_y}" text-anchor="end" '

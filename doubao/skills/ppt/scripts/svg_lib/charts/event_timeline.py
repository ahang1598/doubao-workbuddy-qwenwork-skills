...
def draw_event_timeline(
    ...
    header = _header(ctx)
    return svg_open(0, 0, ctx.W, ctx.H, bg=pal["bg"]) + header + body + svg_close()
...
def _draw_horizontal_alt(ctx: _Ctx, use_pin: bool = False) -> str:
    ...
    # main axis
    parts.append(f'<line x1="{x0}" y1="{axis_y:.1f}" x2="{x1}" y2="{axis_y:.1f}" stroke="{ink}" stroke-width="1.2"/>')
    ...
    for i, (date, title, sub, cat) in enumerate(ctx.events):
        ...
        if use_pin:
            ...
            if above:
                ...
                parts.append(f'<path d="{d}" fill="{col}" stroke="{col}" stroke-width="0.8"/>')
            ...
            parts.append(
                f'<circle cx="{cx:.1f}" cy="{axis_y:.1f}" r="4.5" fill="{bg}" stroke="{col}" stroke-width="2"/>'
            )
        ...
        parts.append(f'<rect x="{card_x:.1f}" y="{card_y:.1f}" width="3" height="{card_h}" fill="{col}"/>')
        # texts
        parts.append(
        ...
        if two_line_title:
            line1, line2, title_fs = _wrap_two_lines_fit(title, avail_w, ctx.fs_title, min_fs=9.0)
            parts.append(
            ...
            if title_fs < 10.5 and " " in title:
                ...
                line1, line2, title_fs = _wrap_two_lines_fit(title, avail_w, ctx.fs_title, min_fs=10.0)
                if medium_pin_roomy:
...
def _draw_stepped(ctx: _Ctx) -> str:
    ...
    parts.append(f'<path d="{" ".join(step_area)}" fill="{_rgba_with_alpha(accent, 0.10)}"/>')
    parts.append(f'<path d="{" ".join(step_line)}" fill="none" stroke="{accent}" stroke-width="1.6"/>')
    parts.append(f'<line x1="{x0}" y1="{plot_bot}" x2="{x1}" y2="{plot_bot}" stroke="{ink}" stroke-width="0.8"/>')
    ...
    for i, (date, title, sub, cat) in enumerate(ctx.events):
        ...
        cx = xs[i]
        cy = ys[i]
        parts.append(f'<circle cx="{cx:.1f}" cy="{cy:.1f}" r="5" fill="{bg}" stroke="{col}" stroke-width="2"/>')
...
def _draw_vertical_alt(ctx: _Ctx) -> str:
    ...
    parts.append(f'<line x1="{axis_x:.1f}" y1="{y0}" x2="{axis_x:.1f}" y2="{y1}" stroke="{ink}" stroke-width="1.2"/>')
    ...
    for i, (date, title, sub, cat) in enumerate(ctx.events):
        ...
        # anchor
        parts.append(f'<circle cx="{axis_x:.1f}" cy="{cy:.1f}" r="5" fill="{bg}" stroke="{col}" stroke-width="2"/>')
        ...
        parts.append(f'<rect x="{accent_x:.1f}" y="{card_y:.1f}" width="3" height="{card_h}" fill="{col}"/>')
        parts.append(
...
def _draw_circular(ctx: _Ctx) -> str:
    ...
    if dense:
        ...
        for i, (date, title, sub, cat) in enumerate(ctx.events):
            ...
            parts.append(f'<circle cx="{ax:.1f}" cy="{ay:.1f}" r="4" fill="{bg}" stroke="{col}" stroke-width="1.8"/>')
            ...
            if sub:
                ...
                sub_tspan = f'<tspan dx="6" fill="{mut}" font-size="{sub_fs:.1f}">{xesc(sub)}</tspan>'
            parts.append(
    ...
    for i, (date, title, sub, cat) in enumerate(ctx.events):
        ...
        parts.append(
            f'<circle cx="{ax:.1f}" cy="{ay:.1f}" r="{ANCHOR_R}" fill="{bg}" stroke="{col}" stroke-width="2"/>'
        )
        ...
        if anchor == "end":
            ...
            ax_x = card_x
        parts.append(f'<rect x="{ax_x:.1f}" y="{card_y:.1f}" width="3" height="{card_h}" fill="{col}"/>')
...
def make_event_timeline(
    ...
    # 分隔线
    parts.append(f'<line x1="{MARGIN_L}" y1="92" x2="{W - MARGIN_R}" y2="92" stroke="{_INK}" stroke-width="0.8"/>')

    # FIGURE caption
    if figure_label:
        parts.append(
            f'<text x="{MARGIN_L}" y="112" font-family="{_body_font}" font-size="12" '
        ...
        parts.append(
            f'<text x="{MARGIN_L + label_w}" y="112" font-family="{_body_font}" font-size="12" '
    ...
    if era_bands:
        ...
        for i, band in enumerate(era_bands[1:], start=1):
            xs_b = x_of(band[0], 1)
            parts.append(
                f'<line x1="{xs_b:.1f}" y1="{band_y_top}" x2="{xs_b:.1f}" y2="{band_y_bot}" '
                f'stroke="{_INK4}" stroke-width="0.4" stroke-dasharray="2 4"/>'
            )

    # 主轴
    parts.append(
        f'<line x1="{axis_x_left}" y1="{axis_y}" x2="{axis_x_right}" y2="{axis_y}" stroke="{_INK}" stroke-width="1.2"/>'
    )
    ...
    for yr in range(yr_start, yr_end + 1):
        xv = x_of(yr, 1)
        parts.append(
            f'<line x1="{xv:.1f}" y1="{axis_y - 5}" x2="{xv:.1f}" y2="{axis_y + 5}" stroke="{_INK}" stroke-width="1"/>'
        )
        parts.append(
            f'<text x="{xv:.1f}" y="{axis_y + 22}" text-anchor="middle" '
    ...
    for yr in range(yr_start, yr_end + 1):
        for mo in [4, 7, 10]:
            ...
            xv = x_of(yr, mo)
            parts.append(
                f'<line x1="{xv:.1f}" y1="{axis_y - 2}" x2="{xv:.1f}" y2="{axis_y + 2}" '
    ...
    for i, (y, mo, cat, ev_title, ev_desc) in enumerate(norm_events):
        ...
        parts.append(f'<circle cx="{x:.1f}" cy="{axis_y}" r="4.5" fill="{halo}" stroke="{col}" stroke-width="2"/>')
        ...
        parts.append(
            f'<line x1="{x:.1f}" y1="{axis_y}" x2="{_lead_x:.1f}" y2="{lead_end_y:.1f}" '
            f'stroke="{_rgba_with_alpha(col, 0.55)}" stroke-width="0.8"/>'
        )
        ...
        date_str = f"{y}·{mo:02d}"
        parts.append(
            f'<text x="{card_x + 10:.1f}" y="{card_y + 13:.1f}" '
        ...
        parts.append(
            f'<text x="{card_x + 10:.1f}" y="{card_y + 28:.1f}" '
        ...
        if ev_desc:
            ...
            parts.append(
                f'<text x="{card_x + 10:.1f}" y="{card_y + 48:.1f}" '
                f'font-family="{_body_font}" font-size="11" fill="{c_muted}">{_xesc(line1)}</text>'
            )
            if line2:
                parts.append(
                    f'<text x="{card_x + 10:.1f}" y="{card_y + 63:.1f}" '
    ...
    if categories and used_cats:
        ...
        lg_x = MARGIN_L
        parts.append(
            f'<text x="{lg_x}" y="{lg_y - 8}" font-family="{_body_font}" '
        ...
        for i, key in enumerate(used_cats):
            ...
            x = lg_x + i * gap
            parts.append(
                f'<circle cx="{x + 6}" cy="{lg_y + 3}" r="5" fill="{halo}" stroke="{col}" stroke-width="1.8"/>'
            )
            parts.append(f'<circle cx="{x + 6}" cy="{lg_y + 3}" r="2" fill="{col}"/>')
            parts.append(
                f'<text x="{x + 18}" y="{lg_y + 6}" font-family="{_body_font}" '
    ...
    foot_y = H - 32
    parts.append(
        f'<line x1="{MARGIN_L}" y1="{foot_y - 14}" x2="{W - MARGIN_R}" y2="{foot_y - 14}" '
        f'stroke="{_INK4}" stroke-width="0.5"/>'
    )
    default_note = (
        "Events are placed alternately above and below the axis to reduce overlap. "
    ...
    if figure_label:
        parts.append(
            f'<text x="{W - MARGIN_R}" y="{foot_y}" text-anchor="end" '

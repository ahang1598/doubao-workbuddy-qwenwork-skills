...
def draw_quadrant(
    ...
    for i in order:
        ...
        if variant == "dot_cross":
            r = dot_r
            parts.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="{r + 4:.1f}" fill="rgba({rr},{gg},{bb},0.15)"/>')
            parts.append(
                f'<circle cx="{px:.1f}" cy="{py:.1f}" r="{r:.1f}" '
                f'fill="rgba({rr},{gg},{bb},0.85)" stroke="rgba({rr},{gg},{bb},1)" stroke-width="1.2"/>'
            )
            parts.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="1.6" fill="{bg}"/>')
            ...
            if n <= 8:
                parts.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="{r + 3:.1f}" fill="rgba({rr},{gg},{bb},0.15)"/>')
            ...
            parts.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="1.8" fill="rgba({rr},{gg},{bb},1)"/>')
            _emit_label_from_plan(parts, label_plans[i], name, ink, bg, label_fs)
...
def _append_name_label(parts, px, py, r, name, ink, bg, fs, plot_x=None, plot_y=None, plot_w=None, plot_h=None):
...
def _bbox_overlap(a, b, pad=1.0):
    ...
    return not (
        a[0] + a[2] + pad <= b[0] or b[0] + b[2] + pad <= a[0] or a[1] + a[3] + pad <= b[1] or b[1] + b[3] + pad <= a[1]
    )
...
def _bbox_overlaps_circle(bbox, ccx, ccy, cr):
    ...
    cbx = ccx - cr
    cby = ccy - cr
    return not (bx + bw <= cbx or cbx + 2 * cr <= bx or by + bh <= cby or cby + 2 * cr <= by)
...
def _place_scatter_labels(
    ...
    def in_bounds(bbox):
        bx, by, bw, bh = bbox
        return (
            bx >= plot_x - 1 and bx + bw <= plot_x + plot_w + 1 and by >= plot_y - 1 and by + bh <= plot_y + plot_h + 1
        )
    ...
    for i in order:
        ...
        for dx, dy, anchor in candidates(px, py, r):
            ...
            score = (
                int(median_cross) * 1000 + int(line_hit) * 500 + center_inside * 100 + dot_overlap * 10 + text_overlap
            )
        ...
        if best is None:
            ...
            anchor = "start"
            bbox = _text_bbox(lx, ly, anchor, name, fs)
            results[i] = {"show": (i in keep_all_idx or N <= 12), "lx": lx, "ly": ly, "anchor": anchor, "bbox": bbox}
            if results[i]["show"]:
...
def make_quadrant_2x2(
    ...
    if not _variant_is_classic("quadrant_2x2", variant):
        _data = {
            "items": list(items),
            "x_axis": tuple(x_axis) if x_axis else (0.0, 1.0),
            "y_axis": tuple(y_axis) if y_axis else (0.0, 1.0),
            "x_title": x_title,
            "y_title": y_title,
            "highlight_name": (
                items[highlight_index][0] if highlight_index is not None and 0 <= highlight_index < len(items) else None
            ),
        }
    ...
    for it in items:
        ...
        if size is None:
            size = 100
        norm.append({"name": str(name), "x": float(x), "y": float(y), "size": float(size), "cat": cat, "note": note_})

    # 检测简单模式（无 x_range 且 items 都在 [0,1]）
    simple_mode = x_range is None and y_range is None and all(0 <= it["x"] <= 1 and 0 <= it["y"] <= 1 for it in norm)
    if simple_mode:
        x_range = (0, 1)
        y_range = (0, 1)
    ...
    if subtitle:
        ...
        # 分隔线（跨越到右侧面板）
        parts.append(f'<line x1="{MARGIN_L}" y1="92" x2="{W - 40}" y2="92" stroke="{_INK}" stroke-width="0.8"/>')
    if figure_label:
        parts.append(
            f'<text x="{MARGIN_L}" y="112" font-family="{_body_font}" font-size="{_fs_figure}" '
        ...
        parts.append(
            f'<text x="{MARGIN_L + label_w}" y="112" font-family="{_body_font}" font-size="{_fs_figure}" '
    ...
    if _show_corner_labels:
        ...
        for cx, cy, anch, lab, sub, col, side in corner_positions:
            parts.append(
                f'<text x="{cx:.1f}" y="{cy:.1f}" text-anchor="{anch}" '
            ...
            if sub and _show_corner_subs:
                ...
                parts.append(
                    f'<text x="{cx:.1f}" y="{cy + sub_dy:.1f}" text-anchor="{anch}" '
    ...
    # 中位线（双虚线，粗一些）
    parts.append(
        f'<line x1="{mx:.1f}" y1="{plot_y}" x2="{mx:.1f}" y2="{plot_y + plot_h}" '
        f'stroke="{c_muted}" stroke-width="1.2" stroke-dasharray="4 4"/>'
    )
    parts.append(
        f'<line x1="{plot_x}" y1="{my:.1f}" x2="{plot_x + plot_w}" y2="{my:.1f}" '
        f'stroke="{c_muted}" stroke-width="1.2" stroke-dasharray="4 4"/>'
    )

    # 中位线标注 "MEDIAN"
    parts.append(
        f'<text x="{mx:.1f}" y="{plot_y - 6}" text-anchor="middle" '
        f'font-family="{_body_font}" font-size="{_fs_median}" fill="{c_muted}" '
        f'letter-spacing=".14em">MEDIAN</text>'
    )
    parts.append(
        f'<text x="{plot_x + plot_w + 6}" y="{my + 3:.1f}" text-anchor="start" '
        f'font-family="{_body_font}" font-size="{_fs_median}" fill="{c_muted}" '
        f'letter-spacing=".14em">MEDIAN</text>'
    )
    ...
    parts.append(
        f'<line x1="{plot_x}" y1="{plot_y}" x2="{plot_x}" y2="{plot_y + plot_h}" stroke="{_INK}" stroke-width="1"/>'
    )
    ...
    if not simple_mode:
        for xv in xticks:
            px = x_of(xv)
            parts.append(
                f'<text x="{px:.1f}" y="{plot_y + plot_h + 18}" text-anchor="middle" '
    ...
    if x_title:
        tspan = (
            f' <tspan fill="{c_muted}" font-weight="400" font-style="italic">{_xesc(x_title_sub)}</tspan>'
            if x_title_sub
            else ""
        )
        parts.append(
            f'<text x="{plot_x + plot_w / 2}" y="{plot_y + plot_h + 42}" text-anchor="middle" '
            f'font-family="{_body_font}" font-size="{_fs_axis_title}" font-weight="600" '
            f'fill="{_INK}">{_xesc(x_title)}  →  {tspan}</text>'
        )
    if y_title:
        tspan = (
            f' <tspan fill="{c_muted}" font-weight="400" font-style="italic">{_xesc(y_title_sub)}</tspan>'
            if y_title_sub
            else ""
        )
        ytx = plot_x - 60
    ...
    if simple_mode and (x_axis or y_axis):
        if x_axis and len(x_axis) >= 2:
            parts.append(
                f'<text x="{plot_x + 8}" y="{plot_y + plot_h + 18}" '
            ...
            parts.append(
                f'<text x="{plot_x + plot_w - 8}" y="{plot_y + plot_h + 18}" '
    ...
    for idx, (i, px, py, r, col, init_tx, init_ty, anch, name, note, show) in enumerate(_label_data):
        ...
        parts.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="{r + 2:.1f}" fill="rgba({r_r},{r_g},{r_b},0.18)"/>')
        # 主体
        parts.append(
            f'<circle cx="{px:.1f}" cy="{py:.1f}" r="{r:.1f}" '
        ...
        if abs(ty - init_ty) > 6:
            ...
            lead_y2 = ty - 4
            parts.append(
                f'<line x1="{lead_x1:.1f}" y1="{lead_y1:.1f}" '
        ...
        parts.append(
            f'<text x="{tx:.1f}" y="{ty:.1f}" text-anchor="{anch}" '
        ...
        if note:
            parts.append(
                f'<text x="{tx:.1f}" y="{ty + _fs_label + 1:.1f}" text-anchor="{anch}" '
    ...
    if has_side:
        ...
        if used_cats:
            parts.append(
                f'<text x="{side_x}" y="{side_y}" font-family="{_body_font}" '
                f'font-size="{_fs_side_hdr}" fill="{c_muted}" font-weight="600" letter-spacing=".15em">'
                f"CATEGORY</text>"
            )
            parts.append(
                f'<line x1="{side_x}" y1="{side_y + 8}" x2="{side_x + side_w}" y2="{side_y + 8}" '
        ...
        if bubble_mode and all_sizes:
            parts.append(
                f'<text x="{side_x}" y="{side_y_cursor}" font-family="{_body_font}" '
                f'font-size="{_fs_side_hdr}" fill="{c_muted}" font-weight="600" letter-spacing=".15em">'
                f"POINT SIZE — parameters</text>"
            )
            parts.append(
                f'<line x1="{side_x}" y1="{side_y_cursor + 8}" x2="{side_x + side_w}" y2="{side_y_cursor + 8}" '
            ...
            for i, (b, lbl) in enumerate(samples):
                ...
                r = r_of(b)
                parts.append(
                    f'<circle cx="{sx}" cy="{sample_y}" r="{r:.1f}" '
                    f'fill="{_rgba_with_alpha(c_muted, 0.5)}" stroke="{stroke_c}" stroke-width="1.2"/>'
                )
                parts.append(
                    f'<text x="{sx}" y="{sample_y + r + 12:.1f}" text-anchor="middle" '
                    f'font-family="{_body_font}" font-size="{_fs_side_hdr}" fill="{c_muted}">'
                    f"{_xesc(lbl)}</text>"
                )
            side_y_cursor += 90
        ...
        if insights:
            ...
            for i, ins in enumerate(insights):
                ...
                for k, ln in enumerate(lines[:4]):
                    parts.append(
                        f'<text x="{side_x + 14}" y="{iy + 33 + k * 12}" '
                        f'font-family="{_body_font}" font-size="{_fs_side_hdr}" fill="{c_muted}">'
                        f"{_xesc(ln)}</text>"
                    )
    ...
    if note or source:
        foot_y = H - 45
        parts.append(
            f'<line x1="{MARGIN_L}" y1="{foot_y - 14}" x2="{W - 40}" y2="{foot_y - 14}" '
            f'stroke="{_INK4}" stroke-width="0.5"/>'
        )
        if note:
            parts.append(
        ...
        if figure_label:
            parts.append(
                f'<text x="{W - 40}" y="{foot_y + 14}" text-anchor="end" '

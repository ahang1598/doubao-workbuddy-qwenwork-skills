...
适用于婚礼主持手卡、司仪提词卡等场景的背面封面图设计。生成后可配合 `resize-image`（缩放至目标尺寸）、`compress-image` / `slim-image`（体积优化）、`adjust-image-color`（色调微调）等工具进行后处理。

### 提示词模板（直接复制，替换占位符）

```
你是一个有丰富经验的封面设计师，现在要设计一张用于结婚主持卡封面图，生成一张 A5 大小的图片，背景为简约高级红色，可以有暗花，图片内侧可以有简约的金边图案，然后这个封面图可以纵向拿着上下对折，对折的上一半里面放囍字和新郎新娘的名字：{{新郎姓名}} 和 {{新娘姓名}}，结婚日期为 {{结婚日期}}，下一半不需要添加文字和囍字
```
...
| 项目 | 规格 |
|------|------|
| **尺寸** | 1448×2048px，竖版比例接近 A5（148×210mm），可直接印刷 |
| **背景** | 深邃酒红色，配有低调缠枝花卉暗纹 |
| **金边** | 四周纤细金色描边，四角中式回纹角花 |
| **上半部分** | 金色大 "囍" 字 + 新人姓名 "{{新郎姓名}} {{新娘姓名}}" + 婚期 "{{结婚日期}}" |
| **下半部分** | 纯净留白，仅保留红底暗纹金边，对折后为背面 |
| **对折线** | 画面中部有自然分隔线，沿此线上下对折即可 |
...
| `{{新郎姓名}}` | 黄xx | 中文名，直接写 |
| `{{新娘姓名}}` | 任x | 中文名，直接写 |
| `{{结婚日期}}` | 2026-09-07 | 建议用横杠分隔，与囍字风格统一 |

### 生成后处理建议

1. **尺寸校准**：若需精确 A5 印刷尺寸（2480×1748px @300dpi 横版或 1748×2480px 竖版），使用 `resize-image` 按长边等比缩放
2. **体积优化**：用于网页/电子预览时用 `slim-image` 质量优先瘦身；用于打印传输时用 `compress-image` 指定 quality=90 以上
3. **色调统一**：若与手卡正面酒红色（#8f2c3a）有偏差，用 `adjust-image-color` 微调饱和度和对比度
4. **画质增强**：如生成图偏模糊，用 `enhance-image` 提升清晰度与色彩表现

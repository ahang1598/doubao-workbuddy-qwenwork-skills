...
- 显式传入 `--as user`。
- 需要 `vc:meeting.bot.manage:write` scope。命令风险级别为 `write`，不使用 `--yes`；真实执行前仍需用户明确会议、目标和动作。
...
两个请求的 query 都只包含 `user_id_type`，body 只包含 `meeting_id` 与 `target_user_id`。先用 `--dry-run` 核对路径、query 和 body；预览结果与用户意图不一致时停止，不执行真实请求。
...
- API 失败保持为 CLI 结构化错误；保留错误码和 `log_id` 排查，不重复发送写请求。
...
| `121103`: `no permission` | 当前身份没有执行该会控操作的权限 | 核对 `vc:meeting.bot.manage:write` scope 和会中主持权限 |
...
| `122003`: `operator is not in the meeting` | 当前用户不在该会议中 | 确认当前用户已加入目标会议后再执行 |
...
- 结束整场会议使用 [vc +meeting-end](lark-vc-meeting-end.md)。

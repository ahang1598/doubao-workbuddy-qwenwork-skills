...
Load `brandkit-design-brain.md` and run `PROPOSE_TYPOGRAPHY`, then render one board per font pair with `<skill dir>/scripts/brandkit.py preview` using:
...
Use the Brandkit renderer for concept/type previews. Model-rendered typography is never evidence of font choice or exact letterforms, and WorkRally's in-image text quality is unverified.

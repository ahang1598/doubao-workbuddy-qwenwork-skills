...
**Canonical style-donor ref — ALWAYS attach it to the style-key call:** run the
URL below through `upload_file` first (it is not in WorkRally's `input_images`
allowlist) and pass the returned WorkRally URL as the canonical reference. There is
no `media_id` to resolve, and a failed download means formula-only — say so. This
applies to hands-off long-form runs too. Never build this key from prose alone.
...
2. For every recurring character, list the eras they appear in and WHAT
   ...
   **IDENTITY CHAIN (mandatory):** the FIRST incarnation is generated from
   ...
   the first incarnation's image as a reference** (`input_images` =
   [style anchor URL, base variant result URL]) with the
...
- **Generate chapter by chapter** (`canvas_generate_video` groups of at most
  six + `canvas_get_task` polling inside a chapter): collect a chapter's blocks, then move

...
> ## ⚠ UNVERIFIED ON WORKRALLY — CHECK BEFORE PROMISING
>
> This direction needs a model that generates a **sung** track with the given lyrics.
> On Higgsfield that was a dedicated audio model; WorkRally exposes
> `canvas_generate_audio` with `mode:"music"`, and **whether any available model sings
> real lyrics has not been tested**.
>
> Before quoting this direction to a user: call `canvas_audio_model_list` and look for a
> model that declares lyric/vocal support. If none does, say so plainly and offer the
> alternatives — a narrated Kids run with an instrumental bed, or a wordless music video.
> Never generate a `mode:"music"` track and present an instrumental as the requested song,
> and never claim the lyrics were sung when you have not listened to the result.
>
> Everything below is the original recipe, kept because the lyric craft, the block-to-song
> map and the assembly flags are all still correct once a singing model exists.
...
- **Kids-catalog styles ONLY** (Studio 3D / Pastel Flat 2D / Colorful 3D /
  Hand-drawn Ink / Poster Vector). A non-Kids style ask in SONG
  MODE → map to the closest Kids style and confirm in one line.

## What changes vs the normal Kids run

- **No narrator**: the song IS the voice. SKIP the voice round,
  skip `voice.lock` — the per-block-line rules do not apply; the audio
  is ONE continuous track.
- **No separate music bed**: the song IS the music. Never add a second music layer.
...
3. **Generate the SONG** — `canvas_generate_audio` `mode:"music"`, **prompt-only: no
   `voice_id`** (the singing voice is described inside the prompt). Pick the model from
   `canvas_audio_model_list` — see the warning at the top of this file — and poll it with
   `canvas_get_task`.
   ...
   passing song, interactive mode posts its exact
   final `{index, task_id}`, asks whether to continue to images, and ends the turn.
...
6. **Generate the 12 (or 6) blocks** (Phase 4 as usual, one
   `canvas_generate_video` call per block + `canvas_get_task` polling): prompts
   ...
   complete, interactive mode posts the exact
   final block ledger, asks whether to assemble, and ends the turn.
7. **Assemble with `--song`** (Phase 6):
   ```bash
   bash ${FACELESS_SKILL_DIR}/scripts/assemble_final.sh --out work/output/final.mp4 \
     --blocks N --manifest clips.txt --song song.wav
   ```
   ...
   (`"song":true`).
8. Deliver ONE `final.mp4`. Put it in the media library with
   `upload_file` + `asset_create` only when the user asked for that.

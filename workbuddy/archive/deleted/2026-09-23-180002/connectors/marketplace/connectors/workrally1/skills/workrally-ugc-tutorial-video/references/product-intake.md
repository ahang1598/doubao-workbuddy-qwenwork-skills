...
- `product_reference` — an HTTPS product image URL, ready for `canvas_generate_image.input_images`.
  A local file becomes one by calling `upload_file`; a page image is used as-is when the CDN
  whitelist accepts it, otherwise download it and `upload_file` it
- `product_video_reference` — the SAME URL. WorkRally's `canvas_generate_video` takes image URLs in
  `reference_assets[].url`, so there is no separate video-side media UUID to create
...
Bring a local file in with one `upload_file(file_path=<绝对路径>)` call and keep the returned CDN
URL as `product_reference`. Add `asset_create(asset_url=..., project_id=...)` only when the user
wants it in the media library — generation itself needs the URL, not an `asset_id`.

## Path B — only a URL

There is no page-extraction tool here; the **local shell** does it. Run this in the local shell
(no `sandbox_exec`) to fetch the page and print the fields you need:
...
- Keep the first usable HTTPS hero image as `product_reference`. Keep at most four additional
  same-SKU URLs only when later prompts genuinely need another angle.
- `input_images` / `reference_assets[].url` only accept URLs inside the CDN whitelist. When a
  product-page URL is rejected, `curl` it down in the local shell and `upload_file` the file to get
  a whitelisted URL. `product_video_reference` is then that same URL — nothing else to create.

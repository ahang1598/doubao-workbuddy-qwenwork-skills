...
# CJK has no word delimiters, so `\w+` would swallow a whole Chinese line into ONE
# token while Whisper returns it as many small tokens — the alignment similarity then
# collapses and every Chinese caption is refused. One CJK codepoint = one token.
CJK_RANGE = "\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\u3040-\u30ff\uac00-\ud7af"
CJK_PATTERN = re.compile(f"[{CJK_RANGE}]")
...
def _is_cjk_side(char: str) -> bool:
    """CJK letters plus the fullwidth punctuation that sits between them."""
    return bool(CJK_PATTERN.match(char)) or "\u3000" <= char <= "\u303f" or "\uff00" <= char <= "\uffef"


def join_tokens(tokens: list[str]) -> str:
    """Join caption tokens the way the language writes them.

    Latin words keep their space; two CJK sides are butted together, otherwise a
    Chinese caption renders as "这 是 一 条 字 幕".
    """
    text = ""
    for raw in tokens:
        token = str(raw).strip()
        if not token:
            continue
        if text and not (_is_cjk_side(text[-1]) and _is_cjk_side(token[0])):
            text += " "
        text += token
    return text
...
def _expand_cjk_words(words: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Split multi-character CJK tokens so both sides of the alignment agree.

    Whisper returns Chinese as chunks of varying length ("今天", "我们来"), while the
    authored script is tokenized one character at a time. Comparing those two
    sequences directly collapses the similarity ratio and the run aborts. Splitting
    the STT chunk into characters with proportional timings makes the two comparable
    without inventing any timing information the STT did not already give.
    """
    expanded: list[dict[str, Any]] = []
    for word in words:
        token = str(word["word"]).strip()
        characters = TOKEN_PATTERN.findall(token)
        if len(characters) < 2 or not all(CJK_PATTERN.match(c) for c in characters):
...
def group_captions(
    ...
    *,
    ...
    def _flush() -> None:
        if not current:
            return
        text = join_tokens([str(word["word"]) for word in current]).strip()
        ...
        captions.append(
            {
                ...
                # Exact token count. Re-deriving it from `text` breaks on CJK, where
                # a caption has no spaces to split on.
                "words": len(current),
            }
        )
        current.clear()
    ...
    for word in words:
        if current:
            ...
            if (
                ...
                _flush()
        current.append(word)
        if SENTENCE_END_PATTERN.search(str(word["word"]).strip()):
            _flush()
    _flush()
    return captions
...
def main() -> int:
    ...
    caption_words = sum(int(caption["words"]) for caption in captions)
    span = (

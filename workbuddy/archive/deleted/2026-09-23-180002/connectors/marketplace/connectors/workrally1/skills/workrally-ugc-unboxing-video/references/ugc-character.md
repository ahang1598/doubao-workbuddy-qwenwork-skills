...
prompt string yourself, and submit it. **The model comes from `canvas_image_model_list`** —
Higgsfield's `soul_2` does not exist on WorkRally, so pick a portrait-capable model whose
`resolution_options` reach 2K (enum `5`) and never hardcode an ID.
...
Poll each returned `task_id` with `canvas_get_task` every 3s until `state: 4`, then keep
`character_url` = `output_assets[0].url`. That identity is reused by every board and every clip
and is never regenerated mid-run. Record the traits you wrote (age band, hair, build, wardrobe
anchors) in your own notes — you cannot re-inspect the generated image later, so the written
description IS the continuity contract.

Wherever this file says `soul_2`, `gpt_image_2`, `seedream_*` or Seedance, read it as "the model
chosen from `canvas_image_model_list` / `canvas_video_provider_config` for that step". Write the
prompt itself in Simplified Chinese — WorkRally's models are Chinese-first — keeping the block
order and the camera / light / material vocabulary of the English templates below.

You are a UGC creator/persona prompt enhancer. You work from structured inputs describing a product context and any user-specified overrides, and you output ONE production-ready prompt string for the chosen text-to-image model that generates the creator/persona who will appear in the downstream UGC video.
...
the `prompt` of the `canvas_generate_image` call at the top of this file
(model from `canvas_image_model_list`, `3:4`, `resolution: 5`).
...
Every wardrobe entry MUST be wrapped with explicit modesty language in the final prompt — even when the matrix says only "fitted blouse" or "casual chic top". Portrait models otherwise default to plunging V-cuts and exposed cleavage, which then trips the downstream content filters at the board step and the video step. This rule is non-negotiable and applies regardless of tier or category.

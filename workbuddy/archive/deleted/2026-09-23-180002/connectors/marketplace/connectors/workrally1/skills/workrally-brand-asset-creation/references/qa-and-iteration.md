...
- The Brandkit state script's `state --action get_status` action was called in this turn
...
A successful script exit or a task reaching `state: 4` is not visual evidence. Inspect the rendered image itself. If the host cannot show you the actual pixels, disclose that visual QA remains unverified; do not claim it passed.
...
- All three use the identical model, palette statement, background treatment, aspect ratio, and resolution
- No generated mark was described as SVG, vector, or deterministically recolored
...
- Compare the generated result with every scene/product/logo reference
- Confirm every output uses the user-approved aspect ratio
- For text-bearing mockups, compare the second-stage output with the exact blank base and verify only the controlled branding/text application changed; read every rendered character against the source copy

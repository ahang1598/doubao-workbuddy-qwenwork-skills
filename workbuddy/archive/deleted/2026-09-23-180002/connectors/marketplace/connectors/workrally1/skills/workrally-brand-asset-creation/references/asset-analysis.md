...
Inventory the attachments, local file paths, and URLs the user gave in this conversation first. Most
analysis reads the local file directly — only call `upload_file` when a file needs an HTTPS URL for
a later generation step, and `asset_create` only when the user wants it in the WorkRally asset
library. Assets that already exist in that library are not automatically this job's official assets.
...
For deterministic measurement, work on the local file (or `curl` the hosted URL into the working
directory) and use whatever is on the machine — ImageMagick (`magick identify`, `magick convert`),
`rsvg-convert`, or Python. Check before you rely on a tool; if none is available, say the
measurement was not taken rather than estimating from a thumbnail. Measure:
...
Read only the user's own local files, their WorkRally asset URLs, or URLs they supplied; never fetch unrelated third-party content. Document URLs use the dedicated document route below.
...
Analyze supplied documents locally with whatever the machine has. These tools are **not guaranteed
to be installed** — check first, and when one is missing either install it (`brew install poppler`,
`brew install --cask libreoffice`) with the user's agreement or say that branch of the analysis did
not run.
...
If the document cannot be parsed locally (corrupt, encrypted, scanned without OCR value, or the required tool is unavailable), report that and ask for page images or source files instead of hanging or improvising.
...
For `.otf`, `.ttf`, `.woff`, or `.woff2`, inspect with whatever font metadata tool the machine has — `fc-scan` (fontconfig) or FontTools (`pip install fonttools`). When neither is present, say the metadata was not read:

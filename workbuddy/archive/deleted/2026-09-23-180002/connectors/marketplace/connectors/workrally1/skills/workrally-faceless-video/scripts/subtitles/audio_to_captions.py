...
# WorkRally port: CJK captions join without spaces and cap on characters, because a
# 32-character Chinese line is a paragraph and " ".join puts a space between every
# ideograph. 14 characters is roughly the same on-screen width as the 5-word / 32-char
# Latin cap.
...
def _is_cjk_words(words: list[dict[str, Any]]) -> bool:
    letters = [
        character
        for word in words
        for character in str(word.get("word", ""))
...
def group_captions(
    ...
    *,
    ...
    cjk = _is_cjk_words(words)
    joiner = "" if cjk else " "
    if cjk:
        max_chars = min(max_chars, CJK_MAX_CHARS)
        max_words = max(max_words, CJK_MAX_CHARS)

    def _flush() -> None:
        if not current:
            return
        text = joiner.join(str(word["word"]).strip() for word in current).strip()
    ...
    for word in words:
        if current:
            ...
            if (
                ...
                _flush()
        current.append(word)
        stripped = str(word["word"]).strip()
        if CJK_SENTENCE_END.search(stripped) if cjk else re.search(r"[.!?]$", stripped):
            _flush()
    _flush()
    return captions

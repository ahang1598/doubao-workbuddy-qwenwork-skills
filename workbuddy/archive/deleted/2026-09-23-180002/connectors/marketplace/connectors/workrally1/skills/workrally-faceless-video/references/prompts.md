...
## 1. Style key (`canvas_generate_image`, `resolution: 4`, chosen aspect — default 16:9)
...
## 2. Assets (`canvas_generate_image`, always `resolution: 4`; pass the style key's result URL
in `input_images`; {STYLE} verbatim)
- **Character (3:4, `resolution: 4`):** "Full-body character centered on a plain flat solid-color
  background, in THIS EXACT style: {STYLE}. Character: {desc — distinctive, readable
  silhouette}. No text, no watermark."
- **Location (CHOSEN aspect, `resolution: 4`):** "{interior/exterior} … in THIS EXACT style: {STYLE}.
...
- **Prop (1:1, `resolution: 4`):** "A single isolated prop centered on a plain flat solid-color
  background, in THIS EXACT style: {STYLE}. Object: {desc}. No hands, no scene, no
  other objects. No watermark."

## 3. Block = one 10s clip with FIVE hard-cut shots (the core template)
Each `canvas_generate_video` call uses the model resolved from `canvas_video_provider_config`,
`duration:10`, `aspect_ratio` = the CHOSEN aspect
(default `"16:9"`; this workflow uses ONLY 16:9 / 9:16), `resolution: 4` (1080P),
`reference_assets` = location → characters → props, in that order, **max 7 refs per call** (keep the production packing even when the model allows more) — only this block's assets; trim extra props/coverage first, never the
location or an on-screen character.  Follow the Phase-4 live-schema/native-audio preflight;
...
REFERENCES (look, identity, palette) — WorkRally binds references POSITIONALLY, so name them in the order they were passed: the first reference image is the LOCATION ({desc}), the second is {CHARACTER A} ({desc}), the third is {CHARACTER B}, the fourth is {PROP}. When the prompt is written in Chinese, use 「第一张图片」「第二张图片」… and never an @Image tag.
...
interplay staging in `${FACELESS_SKILL_DIR}/references/kids-styles.md`. Everything below applies to both
templates:
```
...
## 4. Retry ladder (moderation failure)
On a moderation failure: resubmit ≤3× (change `seed` only if declared); if still failing,
reword (drop child/kid/childlike, avoid tight animal-face CU, calm the pose). On a terminal moderation failure after retries: swap the beat's framing. Never drop a block.

## Scriptwriter

Phase 3 owns the mechanics (N blocks, 5 varied ~2s shots per block — Kids 4, one VO line
of 20–23 words, Kids 17–21, paced toward a full-block read, humor per channel type).
...
- **20–23 words, Kids 17–21, comma-light, paced to fill most of the block** (the
  `[00:00-00:09]` timecode carries delivery but does not pace the TTS provider — see
  vo_and_captions.md), **at most two sentences** — TTS
...
7. Every full-block line 20–23 words (Kids 17–21), flowing/comma-light and

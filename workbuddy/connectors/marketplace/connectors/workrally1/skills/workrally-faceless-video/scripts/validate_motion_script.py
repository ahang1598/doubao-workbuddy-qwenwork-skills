...
# WorkRally port: Chinese narration has no spaces, so the Latin WORD_PATTERN counts a
# whole clause as one token and every length gate becomes a no-op. Count each CJK
# ideograph as one unit instead, and give CJK lines their own budget: Mandarin TTS reads
# ~4.0-4.6 characters/second, so 8.5s of speech is ~34-42 characters rather than 20-23
# words. These numbers are DERIVED from the speech window, not measured on the provider —
# re-measure them once real takes exist.
CJK_PATTERN = re.compile(r"[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\U00020000-\U0002ffff]")
CJK_WORD_BUDGET = (34, 42)
CJK_WORD_BUDGETS = {"kids": (28, 36)}
CJK_DURATION_RETRY_WORD_MIN = 30
# A five-character run is a common Chinese collocation, not evidence of a copy-pasted
# line, so the shared-phrase gate needs a longer run before it fires on CJK.
CJK_SHARED_PHRASE_UNITS = 9
# The cold open is a short flat statement; 8 English words is roughly 14 Chinese
# characters at the same information density.
CJK_HOOK_SENTENCE_UNITS = 14


def _is_cjk(text: str) -> bool:
    """True when the text is predominantly CJK, so the CJK budgets apply."""
    if not isinstance(text, str):
        return False
    letters = [ch for ch in text if ch.isalpha()]
    if not letters:
        return False
    return sum(1 for ch in letters if CJK_PATTERN.match(ch)) * 2 >= len(letters)
...
# WorkRally port: Chinese full stops are 。！？ and carry no trailing space.
...
def _units(text: str) -> list[str]:
    """Countable narration units: one per CJK ideograph, one per Latin word."""
    units: list[str] = []
    for token in WORD_PATTERN.findall(text):
        if CJK_PATTERN.search(token):
            units.extend(
                character if CJK_PATTERN.match(character) else character
...
def _normalised_words(text: str) -> list[str]:
    return [token.casefold() for token in _units(text)]
        ...
        for right_index in range(1, len(right) + 1):
            if left[left_index - 1] == right[right_index - 1]:
                ...
                if current[right_index] > best_length:
                    ...
                    best_end = left_index
        previous = current
    minimum = SHARED_PHRASE_WORDS
    if any(CJK_PATTERN.match(unit) for unit in left):
        minimum = CJK_SHARED_PHRASE_UNITS
...
def _validate(
    ...
    for index, raw_block in enumerate(blocks, start=1):
        ...
        cjk_line = _is_cjk(voice_line if isinstance(voice_line, str) else "")
        if talking_characters and block_kind == "dialogue":
            ...
            budget_minimum, budget_maximum = CJK_WORD_BUDGETS.get(
        ...
        if index in duration_retry_blocks:
            budget_minimum = min(
                budget_minimum,
                CJK_DURATION_RETRY_WORD_MIN if cjk_line else DURATION_RETRY_WORD_MIN,
            )
        ...
        if (
            ...
            block_errors.append(
                f"vo_line must contain {minimum_words}-{maximum_words} "
                f"{'characters' if cjk_line else 'words'}"
            )
        if isinstance(voice_line, str) and voice_line.strip():
    ...
    if genre in HOOK_SENTENCE_GENRES and narration_lines:
        ...
        if hook_number == 1 and isinstance(first_line, str) and first_line.strip():
            ...
            cjk_hook = _is_cjk(first_line)
            hook_limit = CJK_HOOK_SENTENCE_UNITS if cjk_hook else HOOK_SENTENCE_WORDS
            unit_label = "characters" if cjk_hook else "words"
            if opening_words > hook_limit:
                errors.append(
                    f"the cold open is {opening_words} {unit_label} before its first full "
                    f"stop ({opening.strip()!r}) — block 1 opens on a sentence of at most "
                    f"{hook_limit} {unit_label}, the raw fact stated flat, and only then "
                    "fills out to the block's budget"
                )

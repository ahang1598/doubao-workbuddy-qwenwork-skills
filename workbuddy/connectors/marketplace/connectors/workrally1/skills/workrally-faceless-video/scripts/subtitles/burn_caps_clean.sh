...
# 中文字幕：默认的 TikTok Sans 只有拉丁/西里尔/希腊字形，烧中文会出方框。
# 检测到 CJK 且调用方没显式指定 --font 时，自动换成一个 fontconfig 真的能解析的中文字族。
#
# 「装了」不等于「能用」：macOS 上 PingFang SC 落在 SIP 保护的 PrivateFrameworks 下，
# fontconfig 索引不到，fc-match 会返回 Verdana 之类完全无关的字体，中文照样静默烧成方框
# 且 ffmpeg 退出码为 0。所以每个候选都要 fc-match 回读校验：族名对得上、文件可读。
if grep -qP '[\x{4e00}-\x{9fff}\x{3400}-\x{4dbf}\x{3040}-\x{30ff}]' "$SRT" 2>/dev/null \
   || python3 - "$SRT" <<'PY' 2>/dev/null
import re, sys
sys.exit(0 if re.search(r"[\u3400-\u4dbf\u4e00-\u9fff\u3040-\u30ff]",
                        open(sys.argv[1], encoding="utf-8", errors="replace").read()) else 1)
PY
then
  if [[ "$FONT" == "TikTok Sans" ]]; then
    font_usable() {
    ...
    for fam in "Noto Sans CJK SC" "Noto Sans SC" "Source Han Sans SC" "PingFang SC" \
               "Hiragino Sans GB" "STHeiti" "Microsoft YaHei" "WenQuanYi Zen Hei"; do
      if font_usable "$fam"; then PICK="$fam"; break; fi
    done
    if [[ -n "$PICK" ]]; then
      FONT="$PICK"
      echo "WARN: 检测到中文字幕 -> 字体切换为 '${FONT}'。烧完请抽一帧确认不是方框。" >&2
    else
      echo "ERROR: 字幕含中文，但 fontconfig 找不到任何可解析的中文字族。" >&2
      echo "       装一个（brew install --cask font-noto-sans-sc），或用 --font 指定已装字体。" >&2
      exit 1
    fi
  fi
  # 中文没有大小写，强制大写只会破坏标点

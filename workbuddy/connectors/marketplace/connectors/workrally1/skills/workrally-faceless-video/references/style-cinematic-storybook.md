...
`canvas_generate_audio` in `mode:"music"` at the video's exact duration (§ same mechanism as
`${FACELESS_SKILL_DIR}/references/kids-styles.md §Kids music bed`), but the MOOD is **dark-enchanted
...
Pass each authorized HTTPS URL directly as the canonical reference (or upload local
files with `upload_file`) into ONE style-key call on `canvas_generate_image` with the FORMULA
...
Motion-video mechanics = the History channel: N×10s blocks, `canvas_generate_video`,

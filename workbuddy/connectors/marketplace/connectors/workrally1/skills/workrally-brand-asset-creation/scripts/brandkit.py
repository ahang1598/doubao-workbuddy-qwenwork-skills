#!/usr/bin/env python3
"""Deterministic Brandkit operations, run from the local shell.

This script deliberately uses only the Python standard library. The
`logo-export` subcommand additionally shells out to the ImageMagick and librsvg
CLIs, which must be on PATH. It owns Brandkit's approval ledger, editable HTML
review boards, and geometry-preserving SVG/PNG logo exports. All outputs are
local files.
"""
...
def safe_fetch_svg(url: str) -> str:
    ...
    for _ in range(MAX_REDIRECTS + 1):
        ...
        request = Request(
            current,
            headers={"Accept": "image/svg+xml", "User-Agent": "workrally-brandkit/1"},
        )
        try:
...
def path_bounds(markup: str) -> tuple[float, float, float, float, bool] | None:
    """Bounds for the restricted M/L/H/V/Z path subset used for canvas backgrounds."""
...
def write_logo_pair(
    ...
    if not image_magick:
        raise RuntimeError(
            "ImageMagick is not on PATH; install it (brew install imagemagick)"
        )
    if not rsvg_convert:
        raise RuntimeError(
            "rsvg-convert is not on PATH; install librsvg (brew install librsvg)"
        )
...
def run_preview(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        "files": files,
        "next": "Publish these paths per references/inline-widgets.md: screenshot each HTML board to a local PNG, show the PNG, and hand over the HTML path as the editable file.",
    }

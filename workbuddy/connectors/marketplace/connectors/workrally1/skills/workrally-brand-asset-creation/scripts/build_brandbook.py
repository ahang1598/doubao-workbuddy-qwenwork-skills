...
It edits the fixed OOXML template directly, preserves its geometry, and
consumes only separately approved Brandkit state. Outputs are local files.

External requirements: network access to docs.google.com and
fonts.googleapis.com, plus ImageMagick, librsvg, LibreOffice (soffice),
Poppler (pdffonts) and fontconfig (fc-cache / fc-match) on PATH.
"""
    ...
    for _ in range(redirects + 1):
        ...
        try:
            response = opener.open(
                Request(current, headers={"User-Agent": "workrally-brandkit/1"}),
                timeout=30,
            )
...
def convert_png(data: bytes, width: int, height: int, fit: str) -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix="brandkit-image-") as directory:
        ...
        if suffix == ".svg":
            ...
            if not rsvg_convert:
                raise RuntimeError(
                    "rsvg-convert is not on PATH; install librsvg "
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not logo_source:
        raise RuntimeError("approved logo requires a permanent URL or local path")

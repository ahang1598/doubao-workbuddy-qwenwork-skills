...
- Label references by position in prompts, matching the `input_images` order: 「第一张图片是官方 logo」「第二张图片是审定通过的底图场景」.
...
The working Brand Lock may contain draft concept language, but final approvals live in `.brandkit/state.json` inside the brand job's working directory, per `handoff.md`.

- Call the Brandkit state script's `state --action get_status` action at the start of every Brandkit turn, then load only the module needed by the active stage.

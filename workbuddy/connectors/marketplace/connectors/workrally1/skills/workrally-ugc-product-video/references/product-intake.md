...
- `product_reference` — an authorized HTTPS image URL, ready for `canvas_generate_image`'s
  `input_images` array. A local file becomes one via `upload_file(file_path=<absolute path>)`
- `product_video_reference` — the SAME URL. `canvas_generate_video` takes URLs in
  `reference_assets[]` (`{url, type:"image"}`), so there is no second confirm step and no
  separate UUID to mint
...
- `input_images` / `reference_assets[].url` accept a WorkRally short link (`/s/xxx`, valid 5h) or
  ...
  allowlist: download it locally, then `upload_file` it to get a usable URL, and reuse that one URL
  as both `product_reference` and `product_video_reference`.

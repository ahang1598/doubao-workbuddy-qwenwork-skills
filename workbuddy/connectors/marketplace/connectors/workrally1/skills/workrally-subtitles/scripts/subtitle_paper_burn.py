...
Fonts (pick per video style with --font-key, or pass --font <path>):
  ...
  notosc      Noto Sans SC Bold — Simplified Chinese (fetch_fonts.sh pins wght=700)
...
CJK: none of the Latin/Cyrillic faces above carry Chinese glyphs. The burner checks
glyph coverage against the real caption text and falls back to `notosc`, then to a
system CJK face (PingFang SC / Hiragino Sans GB / Noto Sans CJK / Microsoft YaHei),
warning about the swap. Line breaking is unit-aware, so Chinese wraps per character
instead of collapsing into one unbreakable "word". For mixed CN/EN captions pass
`--no-caps` so the English half is not shouted.
...
FONT_FILES = {
    ...
    "notosc":     "NotoSansSC-Bold.ttf",        # Chinese; fetch_fonts.sh pins wght=700
}

# NONE of the bundled Latin/Cyrillic faces above carry Chinese glyphs, so a Chinese
# caption would render as an empty label. When no bundled face covers the text, fall
# back to a system face before giving up. The CJK entries come first because they also
# cover Latin, so this list doubles as the "fonts/ is empty" safety net.
SYSTEM_FONTS = [
    ...
    "/usr/share/fonts/opentype/noto/NotoSansCJKsc-Bold.otf",    # Linux
    "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc",   # Linux
    "C:/Windows/Fonts/msyhbd.ttc",                              # Windows
    "C:/Windows/Fonts/msyh.ttc",                                # Windows
    "/System/Library/Fonts/Helvetica.ttc",                      # macOS, Latin only
    "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",     # Linux, Latin only
]

# CJK writes without spaces, so the whitespace word-wrap below would treat a whole
# Chinese caption as ONE unbreakable "word": _fit_font then shrinks the font to its
# floor and the caption renders tiny on a single line. Break on characters instead.
CJK_RANGE = "\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\u3040-\u30ff\uac00-\ud7af"
CJK_PUNCT = "\u3000-\u303f\uff00-\uffef"
UNIT_RE = re.compile(f"[{CJK_RANGE}][{CJK_PUNCT}]*|[^\\s{CJK_RANGE}]+")


def _units(text):
    """Breakable units: one CJK char (with any trailing fullwidth punctuation) or
    one whitespace-delimited latin run."""
    return UNIT_RE.findall(text) or ([text] if text else [])


def _cjk_side(ch):
    return bool(re.match(f"[{CJK_RANGE}{CJK_PUNCT}]", ch))


def _join_units(units):
    out = ""
    for u in units:
        if out and not (_cjk_side(out[-1]) and _cjk_side(u[0])):
            out += " "
        out += u
    return out
...
def _fit_font(draw, text, font, max_w, min_size=12):
    """Shrink the font until the single widest UNIT fits in max_w, so nothing
    can ever overflow the label (wrapping then handles the rest)."""
    words=_units(text) or [text]
...
def _wrap_lines(draw, text, font, max_w):
    """Greedy wrap so every line's inked width fits in max_w (a lone over-wide
    unit, already shrunk by _fit_font, keeps its own line)."""
    lines=[]; cur=[]
    for w in _units(text):
        trial=_join_units(cur+[w])
        if _ink_w(draw,trial,font)<=max_w or not cur:
            cur.append(w)
        else:
            lines.append(_join_units(cur)); cur=[w]
    if cur: lines.append(_join_units(cur))
...
def _balanced_two_lines(draw, text, font, max_w):
    ...
    for i in range(1, len(words)):
        a, b = _join_units(words[:i]), _join_units(words[i:])
...
def resolve_font(font_arg, font_key):
    """The requested face, or None when fonts/ holds nothing usable.

    Returning None (instead of exiting) is what lets font_for_text reach the system
    faces: with an empty fonts/ dir a Chinese caption used to die here with
    "font not found" even though the machine had PingFang SC installed.
    """
    ...
    if not os.path.exists(p):
        ...
        print(f"(font not found: {p} — run scripts/fetch_fonts.sh; trying system faces)",file=sys.stderr)
        return None
    return p
...
def font_for_text(font_arg, font_key, texts):
    ...
    and Marker are Latin-only), then through the system CJK faces for Chinese /
    Japanese / Korean text, and warn about the swap."""
    path=resolve_font(font_arg,font_key)
    joined="".join(texts)
    if path and _covers(path,joined): return path
    d=os.path.join(os.path.dirname(os.path.abspath(__file__)),"fonts")
    requested=os.path.basename(path) if path else "the bundled fonts dir (empty)"
    for alt_key in ("notosc","montserrat","anton","caveat"):
        alt=os.path.join(d,FONT_FILES[alt_key])
        if os.path.exists(alt) and _covers(alt,joined):
            print(f"WARN: {requested} has no glyphs for this text "
                  f"(script/language mismatch) -> using {alt_key} instead",file=sys.stderr)
            return alt
    for system_path in SYSTEM_FONTS:
        if os.path.exists(system_path) and _covers(system_path,joined):
            print(f"WARN: {requested} cannot draw this text -> using the system face "
                  f"{os.path.basename(system_path)}. The look differs from the bundled "
                  f"caption faces; run scripts/fetch_fonts.sh (or drop a covering .ttf "
                  f"in scripts/fonts/) to pin the intended look.",file=sys.stderr)
            return system_path
    if path:
        print(f"WARN: no bundled or system font covers this text; captions may render "
              f"empty ({requested})",file=sys.stderr)
        return path
    raise SystemExit("no usable font found: run scripts/fetch_fonts.sh, or pass "
                     "--font /path/to/a/font.ttf")

...
> **中文标题（WorkRally 适配）**：字体栈已加 `Noto Sans SC` / `PingFang SC`，Anton 与 Inter
> 都没有中文字形。中文标题按 **2–14 字**（不是 2–4 词）计，超过 6 字由脚本从中间断成两行；
> 调用 `scripts/bake_text_overlay.mjs` 时用 `--case preserve`，`ALL CAPS` 只对西文有意义。
> 下文的字重、描边、阴影、`paint-order` 规则对中文同样适用；描边按字号 8–14% 起，
> 中文笔画比拉丁字母密，笔画糊在一起时降到 6–8%。

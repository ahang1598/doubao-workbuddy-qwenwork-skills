...
|---|---|---|
| `name` | TEXT | 医生姓名 |
| `hco_name` | TEXT | 所在医院名（**冗余文本，实测为 null 不可靠**；走 `hco {name}` 关联解析） |
| `department` | TEXT | **存的是 department.id**，展示用关联 `department {name}` |
| `major_title` | TEXT | **存的是 major_title.id**（如 "1473960520120799232"），展示用关联 `major_title {name}` |
...
# 医院/科室/职称用关联解析：hco {name} / department {name} / major_title {name}（hco_name 冗余字段实测为 null）
{hcp(is_active: 1, hcp_territory.id: {_is_null: false}, _limit: 50, _order_by: {update_time: _desc}) {id name hco {name} department {name} major_title {name} hcp_grade}}
...
| 4 | `hcp.department` / `major_title` 存的是**对象外键 ID**（如 `aDq0I000000GmmSSAS`、`1473960520120799232`）；`hco_name` 冗余文本字段**实测为 null** | 展示一律用关联解析 `department {name}` / `major_title {name}` / `hco {name}`，不要直接展示 ID，也不要误判为"无数据" |

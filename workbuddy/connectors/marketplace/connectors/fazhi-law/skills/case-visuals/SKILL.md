...
name: case-visuals
description: >-
  本技能用于把中国民商事案件、诉讼争议、交易结构或证据材料整理成可直接展示的法律图表，可从事实、合同履行、主体关系、证据、金额数据和诉讼文件中提取关键节点，自动选择图表，并按法官、客户、内部团队或庭审展示等受众调整内容，以文字说明+必要图片交付。
  覆盖案情时间轴、履约时间线、人物与法律关系图、交易或股权结构图、诉讼和执行流程图、证据链、待证事实对应关系、争议焦点、原被告攻防对照、资金流向、损失金额拆解和结构化表格。
  也适用于“画个图”“做时间线”“一图看懂这个案子”“事情太乱了帮我理清”“把前因后果排一下”“把证据串起来”“梳理人物或公司关系”“把资金流转画出来”“整理案件脉络”“对比双方观点”“给法官看的时间线”“整理成客户汇报材料”等表达。主要交付物是时间轴、关系图、证据链、流程图、对照矩阵或结构化表格时，通常可由本技能直接处理。
  不要用于作出法律结论、事实认定、胜算判断或诉讼策略决定，不要判断证据真实性、合法性或证明力，不要查询法条或案例、生成法律文书；用户明确指定其他技能或工具时遵从其选择。
security:
  child_process: false
  eval: false
...
auto_invoke: true
examples:
  - "把合同签订、付款、违约和起诉过程做成时间轴"
  - "一图看懂这起案件的主体关系和资金流向"
  - "把这些证据与各项待证事实串成证据链"
  - "梳理原告主张、被告抗辩和双方证据的攻防对照表"
  - "根据材料整理案件脉络，给客户汇报时使用"
  - "画出母公司、子公司、股东和交易相对方的法律关系"
  - "把一审、二审和执行程序整理成流程图"
  - "将损失项目、金额、计算依据和证据做成结构化表格"
  - "对比双方对三个争议焦点的观点和证据"
  - "把判决书里的关键事实和裁判过程整理成图"
  - "制作庭审展示用的案件事实时间线"
  - "我没有指定图表类型，请根据这份案件材料自动选择"
---
...
| 法规与法条引用 | 不输出具体法规名称或法条编号（本技能非法规检索工具） | 删除法规引用，改为法律原则名称；用户要求可点击依据时转由法条检索或综合法律研究流程处理 |
| 计算校验    | data_table 各项之和等于总计        | 自动计算验证，不一致时标注矛盾                             |


`references/legal-references.md` 和 `rules/evidence-strength-framework.md` 中列出的法规与条款仅用于本技能内部设计参考，不得复制为面向用户的无链接法规引用。本技能不得为生成法规链接而新增法规工具调用、猜测 `law_id` 或 `pos`；用户要求法规依据时，保持可视化职责边界并转由相应法律检索流程处理。

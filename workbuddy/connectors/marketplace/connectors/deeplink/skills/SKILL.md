...
name: deeplink
version: "4.0.2"
description: 提供克而瑞房地产数据查询能力，由深度智联提供服务，依托经克而瑞授权、积累近 20 年的中国不动产数据；核心「问数」覆盖新房、二手房、土地、企业、宏观、长租公寓、产城、康养、商办九大领域，支持自然语言查询、排行、趋势与多城对比；「问知」补充地产、物业、银发领域的政策、研究、企业、法规和运营知识问答。按次消耗预付积分。
---
...
本技能提供的是克而瑞房地产数据查询能力，核心通过 `deeplink_wenshu` 查询经克而瑞授权、积累近 20 年的中国不动产数据，支持九大领域的指标查询、排行、趋势和多城市对比。

涉及成交、价格、库存、土地、企业等具体数据时，应调用问数，以服务实际返回的数据、时间与范围为准；不得把联网检索结果或模型推测表述为克而瑞数据查询结果。查无数据或范围受限时应如实说明，不得编造或补齐未返回的数值。

问知 `deeplink_wenzhi` 是地产、物业、银发专业知识问答的补充能力；涉及市场数据解读时先查询数据，再据实际结果做解读，不以知识问答替代数据查询。

## 工具总览

| 工具 | 能力 | 什么时候用 |
|------|------|-----------|
| `deeplink_wenshu` | **问数（wenshu）**——克而瑞地产数据查询（数据源经克而瑞授权、积累近 20 年） | 用户要具体数字、排行、走势、城市间对比 |
...
向克而瑞地产数据专业知识库问**非结构化专业知识**——政策解读、行业研究、企业情况、专业方法论、物业与养老业态知识。答案融合行业可信专业知识，不是通用模型的泛泛而谈。
...
| 错误码 | 含义 | 建议动作 |
|--------|------|---------|
| `INVALID_API_KEY` | OAuth 访问凭据无效或已过期 | 在 WorkBuddy 中重新连接本 Connector，并按浏览器提示完成授权；不要手工填写 Key |
| `API_KEY_DISABLED` | OAuth 访问凭据已被撤销 | 断开后重新连接本 Connector，并按浏览器提示完成授权；不要继续重试旧连接 |
...
## WorkBuddy OAuth 连接说明

- 首次使用时，在 WorkBuddy 中点击连接；客户端会打开浏览器，由用户登录深度智联并确认授权。
- 授权令牌由 WorkBuddy 安全保存并自动刷新，用户无需创建 API Key，也不要手工填写 `Authorization` 请求头。
- 连接失效或授权被撤销时，在 WorkBuddy 中重新连接并按浏览器提示再次授权；不要对失败请求反复自动重试。

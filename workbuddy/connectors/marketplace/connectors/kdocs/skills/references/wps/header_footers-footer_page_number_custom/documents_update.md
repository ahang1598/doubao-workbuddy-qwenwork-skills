# wps.header_footers.footer_page_number_custom
...
- `verb` (string, 必填): query（读回页脚文本） / update（插入自定义文字包裹页码）。可选值：`query` / `update`
- `alignment` (string, 可选): 对齐方式
- `replace` (boolean, 可选): 是否先清空原页脚再插入（默认 true；false=保留原内容追加）

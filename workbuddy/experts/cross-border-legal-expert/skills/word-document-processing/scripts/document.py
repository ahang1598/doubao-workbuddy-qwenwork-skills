...
from ooxml.pack import pack_document
from ooxml.redlining import RedliningValidator
...
class Document:
    ...
    def add_comment(self, start, end, text: str) -> int:
        ...
        # If start node is a paragraph, insert the range start inside it
        # (before the first content child) so it stays at run level;
        # otherwise insert before the node (for run-level anchors)
        if start.tagName == "w:p":
            content_children = [
                c
                for c in start.childNodes
                if c.nodeType == c.ELEMENT_NODE and c.tagName != "w:pPr"
            ]
            if content_children:
                self._document.insert_before(
                    content_children[0], self._comment_range_start_xml(comment_id)
                )
            else:
    ...
    def validate(self) -> None:
        """
        Validate the document redlining rules.

        Raises:
            ValueError: If validation fails.
        """
        # Redlining validator（schema 校验因体积约束已移除，仅保留修订结构校验）

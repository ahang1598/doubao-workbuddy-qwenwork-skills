...
changelog: "1.0.1: 11个脚本的依赖缺失处理由静默pip install改为提示用户手动安装并以退出码4终止；1.0.0: 初始版本"
...
| Operate on existing PDFs (tables/merge/split/watermark/form/encrypt) | **PROCESS**  | `scripts/*.py`  |
...
**chart / flowchart schemas:** 见 `design/design.md` 的 block 参考表（chart/flowchart/bibliography 字段定义）。
...
PDF 含中文时必须使用 STSong-Light CID 字体，`fill_fillable_fields.py` 和 `fill_pdf_form_with_annotations.py` 已内置 CJK 字体嵌入。表单填写后字段值不得超出边界框、不得覆盖已有文本、不得出现重叠，填写完成后必须通过 `scripts/check_bounding_boxes.py` 验证。

### pdf-simple（create_pdf.py 简单 PDF）专属（GATE）

PDF 含中文时必须使用 `scripts/create_pdf.py` 生成，不得自行用 reportlab 实现。脚本已内置 STSong-Light CID 字体嵌入，自行实现会导致中文乱码。
...
bash scripts/make.sh fix     # explicitly install missing deps (user-invoked)
...
> 依赖缺失时脚本不会自动安装：将打印缺失包名并提示 `pip install <pkg>` 后以退出码 4 终止；也可显式运行 `bash scripts/make.sh fix` 安装。

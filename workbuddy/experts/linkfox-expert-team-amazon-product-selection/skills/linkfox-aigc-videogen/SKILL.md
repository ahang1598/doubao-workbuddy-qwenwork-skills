...
- **查询已有任务**：`python scripts/aigc_videogen.py --query-task '<taskId|task记录JSON路径>'`

**异步流程**：
1. 脚本调用 `/aigc/videoGenAsync` 创建任务，获得 `taskId`
2. 创建成功后立即把 `taskId`、`memberId`、`costToken`、请求参数、创建/查询端点写入会话 `data/linkfox-aigc-videogen-task-<timestamp>.json`
...
- 先在 workspace 的 `<cwd>/linkfox/<YYYY-MM-DD>/<session>/data/` 查找 `linkfox-aigc-videogen-task-*.json`；找到后用 `--query-task '<task记录JSON路径>'` 查询。
- 若用户直接提供 `taskId`，用 `--query-task '<taskId>'` 查询；如需要 `memberId`，追加 `--member-id '<memberId>'`。
- 查询模式只调用 `/aigc/taskQuery`，不会调用 `/aigc/videoGenAsync`，因此用于进度查询和恢复中断任务。

#!/usr/bin/env python3
"""商品库-变体下的链接列表 - LinkFox Skill
调用 /product-center/v1/skill/product/variant/{skuId}/listings 接口。

占位符（被 scaffold_skill.py 或手工替换）：
  商品库-变体下的链接列表  中文一句话标题      /product-center/v1/skill/product/variant/{skuId}/listings  网关端点路径
  linkfox-product-center-variant-listings      skill 标识(=目录名)  product_center_variant_listings.py   本脚本文件名

Usage:
  python product_center_variant_listings.py '<JSON parameters>'           # 强制落盘，输出路径+摘要
"""
...
API_PATH = "/product-center/v1/skill/product/variant/{skuId}/listings"
SLUG = "linkfox-product-center-variant-listings"
...
def validate(params):
    ...
    if params.get("skuId") in (None, ""):
        errs.append("skuId 必填")
    if errs:

...
| productDescription | string | 商品描述；**有 A+ 时为 JSON 字符串**（见下方 A+ 结构） |
...
### productDescription 中的 A+ 图（有 A+ 时）

当 listing 配置了 A+ Content 时，`productDescription` 可能为 **JSON 字符串**（而非 HTML 描述），解析后为对象数组：

| 字段 | 类型 | 说明 |
|------|------|------|
| position | integer | 模块顺序 |
| title | string | 模块标题 |
| image | string | A+ 模块主图 URL（常含 `aplus-media-library-service-media`） |
| carouselImages | array | 可选；轮播子项，每项含 `position` / `title` / `image` |

无 A+ 时该字段通常为普通 HTML/文本，`json.loads` 会失败——消费方应静默跳过 A+ 提取，仅使用 `productImageUrls`。

...
def extract_1688(data_list, asin_order=None):
    ...
    ASIN 匹配优先级：
    1. JSON 顶层 _target_asin 字段（agent 写入文件时指定目标 ASIN）
    2. 按文件索引匹配 asin_order[i]（兜底，要求文件顺序与 ASIN 顺序一致）

    Args:
        data_list: JSON 文件加载后的 dict 列表
        asin_order: 可选，ASIN 列表用于按 ASIN 分组
    ...
    for i, data in enumerate(data_list):
        ...
        if asin_order:
            # 优先用 JSON 顶层的 _target_asin 匹配
            target_asin = data.get("_target_asin") if isinstance(data, dict) else None
            if target_asin:
                asin = target_asin
            elif i < len(asin_order):
                asin = asin_order[i]  # 兜底：按索引匹配
            else:
                continue
            ...
            products_sorted = sorted(products,
                                     ...
                                     reverse=True)
            # 合并同一 ASIN 的多个 1688 文件（B1+B2）
            if asin in result:
                result[asin].extend(products_sorted)
            else:
...
def build_sales_map(search_data):
    """从 linkfox-amazon-search 返回的搜索结果 JSON 构建 asin → monthlySalesUnits 映射。

    搜索结果格式: {"products": [{"asin": "B0XX", "monthlySalesUnits": 2000, ...}, ...]}
    也兼容无 products 包装的直接列表格式。
    """
    sales_map = {}
    if not search_data:
        return sales_map
    ...
    if isinstance(search_data, list):
        ...
        items = search_data.get("products", search_data.get("data", []))

    for item in items:
        if not isinstance(item, dict):
            continue
        asin = item.get("asin", "")
        msu = item.get("monthlySalesUnits", 0) or 0
        if asin:
            sales_map[asin] = msu
...
def build_metrics_map(metrics_data, niche_keyword=None):
    ...
    for item in items:
        ...
        if acos is not None and isinstance(acos, (int, float)):
            # 极目 API 返回的 acos 是小数（0-1），如 acos=0.131653 表示 13.17%
            # 极目响应中不返回 sponsoredProductsPercentageNow，直接用 acos 作为 TACoS
            # nicheTACoS = acos × 100（转为百分比）
            # 如 0.131653 × 100 = 13.17（表示 TACoS=13.17%）
...
def calc_profit(keepa_prod, products_1688, exchange_rate, fba_head_cost,
                ...
                zero_ad=False, return_rate_matched=False, monthly_sales_units=0):
...
def main():
    ...
    parser.add_argument("--amazon-search-file", default=None, help="linkfox-amazon-search 搜索结果 JSON，用于取 monthlySalesUnits 做综合推荐排序")
    parser.add_argument("--recommend-top", type=int, default=5, help="综合推荐列表长度（默认 5）")
    ...
    # 加载亚马逊搜索结果，构建 asin → monthlySalesUnits 映射（用于综合推荐排序）
    sales_map = {}
    if args.amazon_search_file and os.path.isfile(args.amazon_search_file):
        search_data = load_json(args.amazon_search_file)
        sales_map = build_sales_map(search_data)
        print(f"[参数来源] 亚马逊搜索结果: {args.amazon_search_file}", file=sys.stderr)
        print(f"  已解析 {len(sales_map)} 个 ASIN 的月销量", file=sys.stderr)
    else:
        print(f"[⚠️ 提示] 未传入 --amazon-search-file，综合推荐排序将无法计算月销量得分。", file=sys.stderr)

    # 从 Keepa 商品详情补充月销量（针对不在搜索结果中的 ASIN，如用户输入的 ASIN）
    keepa_sales_added = 0
    for asin in asin_order:
        if sales_map.get(asin, 0) == 0:
            kp = keepa_products.get(asin, {})
            keepa_msu = kp.get("monthlySalesUnits", 0) or 0
            if keepa_msu > 0:
                sales_map[asin] = keepa_msu
                keepa_sales_added += 1
                print(f"  [月销量补充] ASIN {asin}: 从 Keepa 补充 monthlySalesUnits={keepa_msu}", file=sys.stderr)
    if keepa_sales_added > 0:
        print(f"  从 Keepa 补充了 {keepa_sales_added} 个 ASIN 的月销量", file=sys.stderr)
    ...
    for asin in asin_order:
        ...
        result = calc_profit(
            ...
            monthly_sales_units=sales_map.get(asin, 0)
        )
    ...
    # === 综合推荐排序：利润 × 销量 ===
    # 每个 ASIN 只取利润最高的 1 个 1688 货源参与综合推荐（避免同一 ASIN 占多个推荐位）
    all_candidates = []
    for result in results:
        asin = result.get("asin", "")
        asin_title = result.get("title", "")
        msu = result.get("monthly_sales_units", 0) or 0
        products = result.get("products_1688", [])
        if not products:
            continue
        # 取净利润最高的 1688 货源
        best_p = max(products, key=lambda x: x.get("net_profit", 0) or 0)
        net_profit = best_p.get("net_profit", 0) or 0
        composite_score = round(net_profit * msu, 2)
        all_candidates.append({
            "asin": asin,
            "asin_title": asin_title,
            "monthly_sales_units": msu,
            "offer_id": best_p.get("offerId", ""),
            "title_1688": best_p.get("title", ""),
            "price_cny": best_p.get("price_cny", 0),
            "price_usd": best_p.get("price_usd", 0),
            "net_profit": net_profit,
            "net_margin": best_p.get("net_margin", 0),
    ...
    # 取 Top N 推荐
    recommend_top = all_candidates[:args.recommend_top]
    for i, rec in enumerate(recommend_top):
        rec["rank"] = i + 1

    if recommend_top:
        top = recommend_top[0]
        print(f"\n[综合推荐 Top 1] ASIN={top['asin']} | 1688={top['title_1688'][:30]}... | "
              f"净利润=${top['net_profit']} | 月销量={top['monthly_sales_units']} | "
              f"综合得分={top['composite_score']}", file=sys.stderr)

    output = {
        "results": results,
        "recommendations": recommend_top,
    }

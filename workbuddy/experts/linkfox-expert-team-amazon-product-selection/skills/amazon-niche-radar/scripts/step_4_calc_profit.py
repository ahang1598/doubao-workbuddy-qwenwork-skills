...
def extract_1688(data_list, asin_order=None):
    ...
    Returns:
        若 asin_order 提供: {asin: [all_products]} 字典
        若 asin_order 为 None: {"_all": [all_products]} 字典
    """
    result = {}
    all_products = []
...
def build_metrics_map(metrics_data, niche_keyword=None):
    ...
    for item in items:
        ...
        if acos is not None and isinstance(acos, (int, float)):
            ...
            # 极目 API 响应中不返回 sponsoredProductsPercentageNow（仅作为请求筛选参数），
            # 因此直接用 acos 作为 TACoS（广告销售成本比），转为百分比
            # 如 acos=0.131653 × 100 = 13.17（表示 TACoS=13.17%）

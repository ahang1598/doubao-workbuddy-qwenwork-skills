...
- **本模板只消费 `{selling_points}` 与 `{image_desc}` 两个占位符**。`language` / `salesRegion` / `platform` / `brandGeneJson` 等不写进本链路 prompt——模特人种 / 年龄 / 性别、场景 Vibe 均由模板内置工作流自动推导。
...
- 模板只消费 `{selling_points}` 与 `{image_desc}`：`salesRegion` / `language` / `brandGeneJson` 等不接入本链路，市场化人种 / 品牌调性强控需改由 `imageDesc` 文字描述传入。

...
| 模特身材对比（三版） | 需要"偏瘦 / 标准 / 偏胖"三位模特并排、各带身材数据的三联画尺码图 |
...
- 默认比例 1:1。尺码图模板**不消费** `point`（与卖点无关）、`customSetting`（画面固定为尺码示意）、`salesRegion` / `platform`。
...
- [ ] 三版：三位模特为偏瘦 / 标准 / 偏胖、外貌各异、年龄段与性别统一的模特；普通：尺码表 + 模特参考 + 测量指南线稿 + 图标化提示四模块齐全

#!/usr/bin/env python3
"""
Amazon 搜索建议词挖掘工具 (Suggestion Miner)

基于 www.amazon.com/suggestions API 批量挖掘长尾关键词。

用法:
  # 批量扩展
  python3 suggestion_miner.py --seed "fan" --mode expand --rounds 3

  # A-Z 字母扫描
  python3 suggestion_miner.py --seed "wireless charger" --mode az

  # 逆向检索（前置修饰词扫描）
  python3 suggestion_miner.py --seed "wireless charger" --mode reverse
  ...
  # 输出到SQLite
  python3 suggestion_miner.py --seed "fan" --mode expand --db /root/keyword_research.db

  # 输出到CSV
  python3 suggestion_miner.py --seed "fan" --mode expand --csv /root/suggestions_fan.csv
"""
...
def fetch_suggestions(prefix, delay=0.3, session_id=None, verbose=False, market_code="US"):
    """
    获取单个前缀的搜索建议词。
    ...
    Returns:
        dict: {
            'keywords': [{'keyword': str, 'rank': int, 'sugg_type': str, 'candidate_source': str}],
            'widget_items': [{'keyword': str, 'full_keyword': str, 'image_url': str, 'search_url': str}],
            'market': str (站点简码),
            'raw': 完整API响应dict
        }
    """
    ...
    try:
        if HAS_REQUESTS:
            ...
            # fallback to urllib (urllib.parse already imported at top)
...
def generate_prefixes(seed, mode='expand', market_code="US"):
...
def mode_expand(seed, rounds=3, delay=0.3, verbose=True, market_code="US"):
    """模式2：批量扩展"""
    all_keywords = {}  # keyword -> info dict
    all_widget_items = []
    prefixes_tried = 0
    
    prefixes = generate_prefixes(seed, mode='expand', market_code=market_code)
...
def mode_reverse(seed, top_n=100, delay=0.3, verbose=True, market_code="US"):
    ...
    第一步：对种子词做 A-Z 字母扫描，拿到后缀衍生词（如 "wireless charger a" → "wireless charger adapter"）。
    第二步：从 A-Z 结果中提取高频单词（排除种子词本身），取 Top-N 作为前置修饰词，
            用 "{word} {seed}" 作为前缀再次请求建议词。

    例如 A-Z 扫描发现 "adapter" 高频出现 → 逆向检索 "adapter wireless charger"，
    发现用户也会从配件视角搜索，拿到 A-Z 扫描覆盖不到的词。
    """
...
def save_xlsx(data, path):
    ...
    if stats:
        ...
        for k, v in stats.items():
            if k == 'top_words':
                summary_rows.append([k, ', '.join(v) if isinstance(v, list) else str(v)])
            else:
...
def main():
    ...
    # 模式
    parser.add_argument('--mode', '-m', choices=['expand', 'az', 'reverse', 'deep'],
                        default='expand', help='运行模式')
    ...
    for mk in target_markets:
        ...
        for local_seed, trans_source in candidates:
            ...
            if args.mode == 'expand':
                result = mode_expand(local_seed, rounds=args.rounds, delay=args.delay, verbose=verbose, market_code=mk)
            elif args.mode == 'az':
    ...
    # 默认：如果没指定任何输出文件，自动保存 Excel (xlsx)
    if not args.output and not args.csv and not args.db and not args.xlsx:

...
def normalize_quality(provider: str, quality: str | None) -> str:
    """规范化 imagegen quality；非 GPT_2_IMAGE 模型固定使用 high。"""
    value = quality.strip().lower() if isinstance(quality, str) else "high"
    if value not in VALID_QUALITIES:
        raise ValueError(
            f"quality={quality!r} 非法，可选: {', '.join(sorted(VALID_QUALITIES))}"
        )
    if provider != "GPT_2_IMAGE" and value != "high":
        raise ValueError(
            f"quality={value!r} 仅 GPT_2_IMAGE 支持；provider={provider!r} 必须使用 high"
        )
    return value

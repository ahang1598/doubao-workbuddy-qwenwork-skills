...
from .config import get_variant, normalize_quality
...
def build_spec_from_plan(
    ...
    *,
    ...
    - 编排级字段来自 state：imageUrls / provider / resolution / quality / textgen_script / imagegen_script / brand_gene_file
    ...
    spec: dict[str, Any] = {
        ...
        "quality": normalize_quality(provider, state.get("quality")),
...
def _process_task(spec: dict, variant: str, skill_root: str) -> dict:
    ...
    try:
        ...
        quality = normalize_quality(provider, spec.get("quality"))
        ...
        if variant == "cloth" and ttype == "SIZE":
            ...
            ig_params.update(
                ...
                aspectRatio=ratio, resolution=resolution, quality=quality,
            )

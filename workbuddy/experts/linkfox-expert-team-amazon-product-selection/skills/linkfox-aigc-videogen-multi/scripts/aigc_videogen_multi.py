...
def _resolve_output_path(ts, slug=SLUG):
    return resolve_data_path(slug, ts)


def _save_task_record(params, create_result, task_id, member_id, cost_token):
    ts = time.time()
    out_path = _resolve_output_path(ts, f"{SLUG}-task")
    payload = {
        "kind": "aigc_video_task",
        "skill": SLUG,
        "taskId": task_id,
        "memberId": member_id,
        ...
        "createdAt": time.strftime("%Y-%m-%dT%H:%M:%S%z", time.localtime(ts)),
        "request": params,
        "createResponse": create_result,
        "queryCommand": f"python scripts/{os.path.basename(__file__)} --query-task {json.dumps(out_path, ensure_ascii=False)}",
    }
...
def _load_task_id(value):
    if os.path.exists(value):
        with open(value, encoding="utf-8") as f:
            record = json.load(f)
        return str(record.get("taskId", "")), str(record.get("memberId", ""))
    return value, ""


def _usage():
    return (
        "Usage: aigc_videogen_multi.py '<JSON>'\n"
        "       aigc_videogen_multi.py --query-task '<taskId|task-record.json>' [--member-id '<memberId>']"
    )


def _handle_query_mode(argv):
    if not argv or argv[0] != "--query-task":
        return False
    if len(argv) < 2:
        print("Missing value for --query-task", file=sys.stderr)
        sys.exit(1)

    task_id, member_id = _load_task_id(argv[1])
    if "--member-id" in argv:
        i = argv.index("--member-id")
        if i + 1 >= len(argv):
            print("Missing value for --member-id", file=sys.stderr)
            sys.exit(1)
        member_id = argv[i + 1]
    if not task_id:
        print("taskId is required for query mode", file=sys.stderr)
        sys.exit(1)
    ...
    if media_paths:
        ...
        out_path = _resolve_output_path(time.time(), f"{SLUG}-query")
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(serialized)
...
def main():
    ...
    try:
        task_record_path = _save_task_record(params, create_result, task_id, member_id, cost_token)
    except OSError as e:
        print(f"Failed to save task record: {e}", file=sys.stderr)
        task_record_path = ""

    print(f"Task created: taskId={task_id}, costToken={cost_token}", file=sys.stderr)
    if task_record_path:
        print(f"Task record saved: {task_record_path}", file=sys.stderr)

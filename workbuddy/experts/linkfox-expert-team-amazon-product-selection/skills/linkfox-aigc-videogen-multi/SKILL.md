...
- **查询已有任务**：`python scripts/aigc_videogen_multi.py --query-task '<taskId|task记录JSON路径>'`

**异步流程**：
1. 脚本调用 `/aigc/multiImageVideoGenAsync` 创建任务，获得 `taskId`
2. 创建成功后立即把 `taskId`、`memberId`、`costToken`、请求参数、创建/查询端点写入会话 `data/linkfox-aigc-videogen-multi-task-<timestamp>.json`
3. 等待 2 分钟后开始轮询 `/aigc/taskQuery`，每 10 秒一次，最长等待 20 分钟
4. 成功后自动下载视频到会话 `media/` 目录

**输出策略（脚本默认行为）**：
- 脚本会自动把生成的视频**下载转存**到会话 `media/` 目录，stdout 输出一行 `Saved full response: <路径数组>`。
- 任务记录会落盘到会话 `data/`；无视频产物或手动查询无媒体结果时，响应 JSON 也会落盘并在 stdout 提示路径。

**读数据建议**：**禁止** Read 转存的视频文件内容。直接把 `Saved full response:` 后的本地路径提供给用户即可。

## 进度查询与防重复扣费

- 用户询问“进度 / 状态 / 是否完成 / 结果 / 失败原因 / 继续看看刚才那个视频”时，**禁止**重新调用生成接口或重新运行普通生成命令。
- 先在 workspace 的 `<cwd>/linkfox/<YYYY-MM-DD>/<session>/data/` 查找 `linkfox-aigc-videogen-multi-task-*.json`；找到后用 `--query-task '<task记录JSON路径>'` 查询。
- 若用户直接提供 `taskId`，用 `--query-task '<taskId>'` 查询；如需要 `memberId`，追加 `--member-id '<memberId>'`。
- 查询模式只调用 `/aigc/taskQuery`，不会调用 `/aigc/multiImageVideoGenAsync`，因此用于进度查询和恢复中断任务。
- 查询返回 `PROCESSING` 时告诉用户仍在生成；返回 `SUCCESS` 时下载并展示本地视频路径；返回 `FAILED` 时读取 `errorMsg` 并说明失败原因，不要自动重建任务。
...
- 失败时不重试（仅尝试 1 次）。
- 查询已有任务时只能使用 `--query-task`，不得再次创建任务。

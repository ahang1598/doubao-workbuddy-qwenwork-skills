...
def check(path):
    ...
    # ---- 规则 3：同一 section 内 callout 多色混排 ----
    for sec in re.findall(r'<section[^>]*>(.*?)</section>', html, re.S):
        warn_n = len(re.findall(r'class="callout callout-warn"', sec))
        teal_n = len(re.findall(r'class="callout callout-teal"', sec))
        violet_n = len(re.findall(r'class="callout callout-violet"', sec))
        gray_n = len(re.findall(r'class="callout callout-gray"', sec))
        # 纯蓝 callout（不带任何修饰类）
        blue_n = len(re.findall(r'class="callout"(?!\s*callout-)', sec))
        variants = sum(1 for n in (warn_n, teal_n, violet_n, gray_n, blue_n) if n)
        if variants >= 2:
            r.err('3', '同一 section 内出现多种颜色的 callout 混排，应拆成多个 section 或统一为同色')
        # ---- 规则 12：≥2 条 warn 并列 ----
        if warn_n >= 2:
    ...
    # ---- banner 结构 ----
    if '<div class="banner">' not in html:

...
`python scripts_unpack.py <office_file> <output_directory>`
...
预置过滤器在 `scripts/lua/` 下，通过 `markdown-to-docx.lua` 一键加载（md2docx.py 默认启用）：
...
# 1. pandoc 转换
pandoc input.md \
  --reference-doc=templates_template.docx \
...
2. 解包文档：`python scripts_unpack.py <office_file> <output_directory>`
3. 使用 Document 库创建并运行 Python 脚本（参见 ooxml.md 中的"Document 库"部分）
4. 打包最终文档：`python scripts_pack.py <input_directory> <office_file>`
...
3. **阅读文档并解包**：
   ...
   - **解包文档**：`python scripts_unpack.py <file.docx> <dir>`
...
5. **打包文档**：所有批次完成后，将解包目录转换回 .docx：
   ...
   ```bash
   python scripts_pack.py unpacked reviewed-document.docx
   ```

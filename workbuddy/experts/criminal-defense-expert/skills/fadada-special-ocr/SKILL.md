...
- **文件过大（单文件超过10MB）**：必须由 `ocr_to_markdown.py` 自动调用包内 `split_ocr_input.py`；PDF 按原页序转为单页图片，单张大图在保持整页的前提下确定性压缩，再逐页调用 OCR API 并按页序组装。禁止运行时另写拆分代码或临时修改脚本。
...
```bash
python ocr_to_markdown.py <文件A>
# 读取并处理结果后再执行下一个
python ocr_to_markdown.py <文件B>
```
...
python ocr_to_markdown.py <文件路径>

# 标准入口：同时写出 .ocr.json sidecar，便于下游复用 full_text_cleaned / full_text_formatted
python ocr_to_markdown.py --write-sidecar <文件路径>

# 低层调试：仅调用 OCR API，不写 Markdown
python parse_contract_file.py <文件路径>
```
...
```bash
# ✅ 正确：逐个调用
python ocr_to_markdown.py <文件1>
# 读取结果后再执行下一个
python ocr_to_markdown.py <文件2>

# ❌ 错误：一次性传多个
python ocr_to_markdown.py <文件1> <文件2> ...
```
...
最终输出结构（供后续 skill 复用），详见 `output-contract.md`：
...
- `ocr-api-integration.md` — 接口地址、鉴权方式、请求/响应格式、限制说明
- `output-contract.md` — 完整输出字段约定与 Markdown 文件模板
- `split_ocr_input.py` — 超过 10MB 的 PDF/图片固定拆分与压缩入口

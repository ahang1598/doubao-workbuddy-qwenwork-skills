...
ENDPOINT = "/claw/searchTool/lawCase"
...
def main():
    if len(sys.argv) != 2:
        print_error(
            '请提供一个 JSON 请求对象。用法: python3 search_law_case.py '
            "'{\"retrievalScene\":\"case_semantic\",\"searchContent\":\"检索内容\"}'"
        )
        return 1

    try:
        payload = parse_request(sys.argv[1])
        result = search_law_case(payload)

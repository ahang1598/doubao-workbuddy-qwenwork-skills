...
典型用法
--------
  # 聚合 Skill 1 的结果（使用默认 Home 路径）
  python scripts/merge_partials.py --kind standard

  # 聚合 Skill 2 的结果（Skill 2 有自己的副本）
  ...
  python scripts/merge_partials.py \
    ...
    --output-dir ~/RetailAnalysis/data/standard

  # 干跑，仅列出将要写入的文件
  python scripts/merge_partials.py --kind standard --dry-run
"""
...
def _build_bank_doc(
    ...
    for period, path in period_files:
        ...
        # 清理冗余的 bank 字段（移到外层），保留其他
        cleaned = {k: v for k, v in raw.items() if k != "bank"}
        by_period[period] = cleaned

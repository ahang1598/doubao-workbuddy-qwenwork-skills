---
name: pd-a-share-market-risk-radar-workflow
description: "Use this private PandaData-only workflow when PD-A股风险红绿灯 is invoked inside its WorkBuddy expert package."
---
...
本 Skill 仅服务于 **PD-A股风险红绿灯**。不得绕过主 Agent 的数据闸门、最低接口路由或最终回执。
...
- 只处理 `pandadata` Connector 已返回的事实数据和纯本地计算。
- 不读取环境变量、凭据、网络地址或包外数据；不得执行联网脚本。
- 所有派生指标必须说明公式、字段、窗口和数据日期。
- 不承诺收益，不输出自动交易或具体仓位指令。

## 已登记接口契约

完整参数、返回字段与备用参数见 `references/pandadata-interface-contracts.md`。

...
def check(path):
    ...
    # 只在同一个 callout-warn 的 div 内部检测：原写法用 `.*?` 会跨过 </div> 一直匹配到
    # 后续 section 里的 plain-list，导致「风险节 callout-warn + 下一节 plain-list」被误报。
    for warn_div in re.findall(
            r'<div class="callout callout-warn">(.*?)</div>', html, re.S):
        if '<ul class="plain-list">' in warn_div:

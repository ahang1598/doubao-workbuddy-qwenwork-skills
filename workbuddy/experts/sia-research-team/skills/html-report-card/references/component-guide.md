...
**规则**：Banner 只承载 3 类信息 —— **title（产出物名）→ subtitle（简短说明）→ meta（日期/阶段/周期）**。品牌归属（{{BRAND_NAME}} · {{ROLE_NAME}}）**放页脚 disclaimer**，不进 banner。禁止再加"专家名字胶囊""版本号""水印小标签"等杂物。

**参数示例**：
- 启动规划师产出：title=「账号 0→1 规划方案」→ subtitle=「面向：城市白领 · 目标平台：平台A · 每周产能 3-4 小时」
- 高绩效案例分析师产出：title=「案例拆解报告」→ subtitle=「样本：@xxx 12 万赞图文 · 拆解维度：10 维方法论」
- 内容变现顾问产出：title=「合作项目 Brief 评估」→ subtitle=「品牌：xxx · 合作类型：图文单条 · 报价区间：5k-10k」

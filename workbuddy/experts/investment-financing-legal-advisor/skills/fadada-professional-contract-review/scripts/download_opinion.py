...
def download_opinion(contract_id: str, save_path: str) -> dict:
    """
    下载审查意见书
    ...
    endpoint = "/claw/contract/downloadOpinion"
...
def main():
    if len(sys.argv) < 3:
        print(json.dumps({
            "success": False,
            "error": "参数不足",
            "usage": "python download_opinion.py <contractId> <保存路径>"
        }, ensure_ascii=False))
        sys.exit(1)
    ...
    result = download_opinion(contract_id, save_path)

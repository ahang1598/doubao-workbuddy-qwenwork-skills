...
def _set_font(run, size_pt: float, bold: bool = False, color: RGBColor = COLOR_BLACK,
    ...
    preserve_color=True 时保留 run 原有颜色（用于超链接等需保留颜色的场景）。
    """
        ...
        for i, row in enumerate(table.rows):
            ...
            for j, cell in enumerate(row.cells):
                ...
                for para in cell.paragraphs:
                    ...
                    for run in _get_all_runs(para):
                        if is_header:
                            _set_font(run, SIZE_SMALL, bold=True, color=COLOR_TABLE_HEADER_FG)
                        else:
...
def _apply_column_widths(table, num_cols: int, table_width_dxa: int):
    ...
    MIN_COL_DXA = 600   # 最小列宽 ~1.05cm，防止序号等窄列被压扁
    ...
    # 表头行也参与计算（表头文字往往比数据行更长更重要）
    header_row = table.rows[0]
    for j in range(num_cols):
        if j < len(header_row.cells):
            w = _estimate_text_width(header_row.cells[j].text)
            col_all_widths[j].append(w * (1 + HEADER_BONUS))  # 表头略加权重
    ...
    for i in range(1, num_rows):
        ...
        for j in range(num_cols):
            if j < len(row.cells):
                col_all_widths[j].append(_estimate_text_width(row.cells[j].text))
    ...
    # 若保底总和超出表格宽度，按等比缩放使总和匹配
    # 注意：缩放后仍需保留每列的动态最小宽度（而非全局 MIN_COL_DXA）
    actual_sum = sum(col_widths_dxa)
    if actual_sum > table_width_dxa:
        scale = table_width_dxa / actual_sum
        col_widths_dxa = [max(col_min_dxa[j], int(w * scale)) for j, w in enumerate(col_widths_dxa)]
...
def add_footer(doc: Document, doc_name: str = ""):
    """添加页码页脚。"""
    for section in doc.sections:
        ...
        _set_font(run_page, SIZE_SMALL, color=COLOR_MID_GRAY)

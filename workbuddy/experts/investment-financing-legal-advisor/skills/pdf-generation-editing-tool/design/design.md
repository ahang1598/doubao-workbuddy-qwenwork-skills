...
Cover fonts are loaded live via `@import url(...)` in the cover HTML — rendered
by reportlab (pure Python, no external engine). Body pages always use system fonts
...
- Emoji or icon fonts (may fail silently)

...
cmd_check() {
  ...
  # Pillow (used by create_validation_image.py)
  if python3 -c "import PIL" 2>/dev/null; then
    green "  ✓ Pillow"
  else
    yellow "  ⚠ Pillow not installed  (run: make.sh fix)"
    ok=false
  fi
...
cmd_fix() {
  ...
  # Pillow (for PROCESS scripts)
  python3 -m pip install --break-system-packages -q Pillow 2>/dev/null \
    || python3 -m pip install -q Pillow 2>/dev/null \
    || { yellow "  Pillow install failed"; rc=3; }
  green "  ✓ Pillow installed"
...
cmd_run() {
  ...
  set --
  [ -n "$accent"   ] && set -- "$@" --accent   "$accent"
  [ -n "$cover_bg" ] && set -- "$@" --cover-bg "$cover_bg"
...
[
  ...
  {"type":"numbered","text":"cover.py renders the cover PDF directly using ReportLab"},

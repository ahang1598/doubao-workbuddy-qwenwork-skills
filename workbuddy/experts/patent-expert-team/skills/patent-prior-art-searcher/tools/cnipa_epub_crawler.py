...
- **合规检索说明**：本脚本与无头自动化、静态抓取的对象是**国知局公开发布的专利数据**；通过 **真实 Chromium 内核 + 等待 JS 完成 + 常见浏览器指纹**（UA、语言、viewport）降低「一进来就_submit」的失败率；**不**表示规避法律法规或站点服务条款，用途应限合法检索与交底书查新辅助。
- **合规声明**：本脚本**仅访问国知局公开发布数据**，遵守站点 ``robots.txt`` 与服务条款，以普通浏览器形态发起合法查新检索，不规避任何站点访问控制与自动化检测机制。

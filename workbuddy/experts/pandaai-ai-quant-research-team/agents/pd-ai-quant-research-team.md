...
description: "Orchestrates five isolated quantitative research specialists and blocks any completion without sealed evidence."
displayName:
  en: "研报复现因子挖掘回测审计"
  zh: "研报复现因子挖掘回测审计"
profession:
...
WorkBuddy host mapping: create the team once with TeamCreate, then invoke only the routed member with AgentTool. Pass only the absolute task packet path and preserve the real AgentTool call ID.
...
任务包和交接格式见 [子 Agent 交接合约](references/member-handoff-schema.md)。
...
从此以后严格执行 [完整 SOP](references/sop.md)。不要绕过守卫直接宣布阶段完成。
...
完整文件名、命令标签和校验规则见 [证据合约](references/evidence-contract.md)，上下文隔离与交接格式见 [子 Agent 交接合约](references/member-handoff-schema.md)。
...
- 源码与数据边界见 [source-boundary.md](references/source-boundary.md)。

...
| **Brand protection manager** | Cease-and-desists, platform takedown takedowns, watch-service follow-up |
...
It writes what it learns to `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` — a plain-English document about your practice that every other skill reads before doing anything. You edit the document, not a config file.
...
| `/ip-legal:cease-desist [context]` | Cease-and-desist — send, or triage an inbound one, with the approval routing your practice-profile.md requires |
| `/ip-legal:takedown [context]` | platform takedown takedown — send, respond to a received notice, or draft a counter-notice |
...
| Skill | Purpose |
|---|---|
| **cold-start-interview** | First-run interview that writes `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` |
| **cease-desist** | Draft or triage a C&D; routes through the approval matrix before sending |
| **takedown** | platform takedown notice, response to a received takedown, or counter-notice |
...
- **research connector** — U.S. court opinions, PACER case docket or procedural records, citation verification
- **legal research connector** — primary law research by concept or wording, citation treatment, quoted-language verification
...
Your configuration is stored at `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` and survives plugin updates.
...
├── .mcp.json
├── practice-profile.md                    # Your practice profile — written by cold-start, edited by you
├── README.md
├── agents/
...
This path survives plugin updates. The `practice-profile.md` that ships with the plugin is a template — it is replaced every upgrade. The cold-start interview writes your populated version to the config path above; from then on, edit that file directly when something changes.

## How it learns

Your practice profile at `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. The `ip-renewal-watcher` agent tracks the portfolio register and surfaces upcoming renewal deadlines at your cadence. You can re-run setup, edit the file directly, or tell a skill to record a new position.

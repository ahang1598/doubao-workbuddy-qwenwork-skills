...
Reviews the IP clauses in an agreement against the practice profile in `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`.** If placeholders present, stop and prompt: "Run `/ip-legal:cold-start-interview` first — I need to learn your practice profile before I can review IP clauses against it."
...
**Before reading the agreement, read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
**Escalation:** Per `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` → `## Outputs` (it differs by user role — see `## Who's using this`).

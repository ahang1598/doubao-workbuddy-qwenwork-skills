...
1. **Follow the workflow below** and read
   `~/.codebuddy/plugins/config/legal-workflows/ip-legal/portfolio.yaml`.
...
3. **`--report [--days N]`:** Mode 2. Change the window with `--days`
   ...
   per practice-profile.md → Outputs. Always close with the verification caveat.
...
8. **Guardrail reminder:** Computed deadlines are reference only. Every
   output closes with a line directing verification against the IP registry
   TSDR, WIPO, or relevant registry before filing or paying. A
   case docket or procedural recorded-but-wrong deadline creates false confidence; do not let the
...
- **An IP management system (IPMS) via MCP** — Anaqua, Clarivate IPfolio,
  ...
  full case docket or procedural record, maintenance fee schedules, and incoming correspondence in one
...
- **IP registry directly via customer number** — pulls status, deadlines, and
...
Without either, paste your case docket or procedural record or upload a spreadsheet and I'll track from
there.
...
> deadlines against the IP registry TSDR / Patent Center, WIPO Madrid Monitor /
...
> or another IP management system, their case docket or procedural record is authoritative for your
> assets — use this tracker to organize and surface their data, not to replace
> it.
>
> A case docket or procedural recorded-but-wrong deadline is worse than an uncase docket or procedural recorded one: it creates
...
Lives at `~/.codebuddy/plugins/config/legal-workflows/ip-legal/portfolio.yaml`.
Structure:
...
# Generated: [date]
# Last updated: [date]
# Disclaimer: computed deadlines are reference only — confirm with IP registry/WIPO/
...
assets:
  - id: "TM-US-001"
    ...
    local_agent: null
    case docket or procedural record_id: "[IP-mgmt-system ID or null]"
  ...
  - id: "PAT-US-001"
    ...
    next_deadlines:
      - type: "3.5-year maintenance fee"
        ...
        status: "upcoming"
    claims_count: 20
    entity_size: "large"                       # large / small / micro (drives IP registry fees)
    case docket or procedural record_id: null
...
Output (prepend work-product header per `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` → Outputs):
...
⏰ DUE WITHIN [N] DAYS ([N])
  [Asset ID] / [Jurisdiction] / [Type] / [Mark or title]
    ...
    [Agent: firm / case docket or procedural record: id — if present]
...
❓ UNKNOWN ([N])
  ...
  Confirm with [IP management system / IP registry TSDR / relevant registry] before relying on this report.
...
Close the report with the caveat line: *"Computed from portfolio register. Verify each deadline against the IP registry/WIPO/registry of record before filing or paying."*

If the report lists more than ~10 assets, or any time the user asks: offer the dashboard (see practice-profile.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by registration status (live / in grace / lapsed / pending), a deadline timeline, and a sortable portfolio table with jurisdiction, type, and next-action date.
...
8. Source — is this being tracked in the IP management system under a case docket or procedural record ID?
...
`## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. If the Role is **Non-lawyer**:
...
> (or with the IP registry TSDR / WIPO Madrid Monitor / relevant registry)? If yes,
> proceed. If no:
>
...
connected, pull the latest case docket or procedural record and reconcile. Flag mismatches between
...
**Unwatched assets**
- Any registered marks not on the watch list in practice-profile.md → Brand protection?
...
practice-profile.md → Renewal alerts. If 🔴 items appear (grace / lapsed), the agent
...
- It does not verify deadlines against the IP registry TSDR, WIPO, or any other
...
- It does not read office records to verify status. A §8 shown as "filed"
  here means someone told it so — not that the IP registry accepted it. Confirm

...
practice-profile.md. If `Enabled` is `✗` (the default for in-house users), skip the rest
...
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/matters/<matter-slug>/`.
...
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`.** If it is

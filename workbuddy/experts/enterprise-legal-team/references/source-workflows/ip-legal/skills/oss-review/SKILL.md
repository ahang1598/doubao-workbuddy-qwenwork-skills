...
**Before scanning dependencies, read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` → `## Outputs` (differs by user role — see `## Who's using this`).

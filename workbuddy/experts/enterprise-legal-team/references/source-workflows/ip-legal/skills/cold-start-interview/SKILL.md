...
Runs the cold-start interview. First run writes `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` (I'll show you a diff first)."
...
5. **Migration:** If a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/ip-legal/*/practice-profile.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Seed the portfolio register** if the user shared a portfolio export or IP management system access: write to `~/.codebuddy/plugins/config/legal-workflows/ip-legal/portfolio.yaml`. If nothing was shared, leave a placeholder pointer the portfolio tracker can fill later.

8. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` is written. What did I get wrong?"
...
Re-runs the integration availability check (IP management system, patent research, legal research, document storage, Slack) and updates `## Available integrations` in `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
Read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`:
...
If a practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/ip-legal/*/practice-profile.md` but not at the config path, copy it forward to the config path before proceeding.
...
**Fresh professional profile.** Setup builds a fresh professional profile from the user's answers and the documents they explicitly share. It does not read the user's personal AI assistant history, unrelated conversations, or their home-directory practice-profile.md. If something relevant surfaces in the current conversation context (e.g., they mentioned the company earlier), ask before using it — do not fold anything personal into the practice profile unless the user types it or approves it.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/ip-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` with a `<!-- SETUP PAUSED AT: [section name] — run /ip-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> Who'll be using this plugin day to day? (This feeds the work-product header on every clearance memo, C&D draft, and portfolio memo — and for registered patent agents, drives the narrower privilege header on IP registry matters only.)
>
> 1. **Lawyer or legal professional** — attorney, paralegal, legal ops, IP specialist working under attorney oversight.
> 2. **Registered patent agent** — you're registered to practice before the IP registry but are not a licensed attorney. Your client communications on patent prosecution matters are privileged under *In re Queen's University at Kingston*; on anything outside IP registry practice (trademark, copyright, OSS, contracts), they are not.
...
> - **Copyright** (clearance / licensing / platform takedown / enforcement)
...
doesn't need the prosecution / case docket or procedural recording questions.
...
> This plugin can work with: IP management systems (Anaqua, CPA Global, PatSnap, Clarivate), patent research (Solve Intelligence), legal research (research connector, legal research connector), document storage (Google Drive, SharePoint, Box), and Slack. Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Anaqua isn't connected. In AI coworking environment: Settings → Connectors → Add → Anaqua → sign in. In CodeBuddy: add the Anaqua MCP to your config or via `/mcp`. This plugin works without it — portfolio lives in `portfolio.yaml` and you update it by hand — but connecting it lets the renewal-watcher pull the register automatically."
...
> - **Copyright** — registration, platform takedown, licensing, fair use triage
...
>
> - **Marks registered in:** US (IP registry)? EU (EUIPO)? UK (UKIPO)? Madrid member states — which? National filings elsewhere? Common-law only?
...
>
> - **platform takedown takedown (ordinary):** often delegated to counsel or brand protection; who owns it on your team?
...
Write the plugin config following the structure in `${PLUGIN_ROOT}/practice-profile.md` (the template). Use their words where you can. This is a document *about their practice* that they will read and edit — it is not a config file.
...
Write to `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` (create parent directories as needed). If the user shared a portfolio export, also seed `~/.codebuddy/plugins/config/legal-workflows/ip-legal/portfolio.yaml` with the extracted registrations.
...
4. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
...
5. **Before your first clearance**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In CodeBuddy: authorize when a skill prompts you.

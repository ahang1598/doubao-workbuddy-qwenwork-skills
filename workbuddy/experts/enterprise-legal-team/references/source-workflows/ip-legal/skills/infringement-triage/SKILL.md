...
5. Walk the mode-specific factors:
   ...
   - **Copyright** — ownership + registration + access + substantial
     similarity + fair use + platform takedown safe harbor (if applicable).
...
Read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. Pull:
...
- **Integrations** from `## Available integrations` — research connector,
...
> 2. **Copyright** — substantial similarity, fair use, platform takedown safe harbor
...
survives as a guiwitness interviewst inside the ordinary-observer test, not as a separate
test).
...
> IP registry; for other jurisdictions, use the relevant patent office register. For trademarks, INTA maintains a directory of practitioners worldwide.
...
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/matters/<matter-slug>/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`
...
> Want me to prepare a platform takedown takedown? Run `/ip-legal:takedown`.

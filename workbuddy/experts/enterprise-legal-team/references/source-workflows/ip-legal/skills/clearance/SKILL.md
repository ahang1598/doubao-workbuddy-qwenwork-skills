...
1. Read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. If it
...
5. Similar-marks search against what's connected (Solve Intelligence, research connector, legal research connector, or whatever MCP is available). If nothing is
...
**Matter context.** Check `## Matter workspaces` in the practice-level practice-profile.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/ip-legal:matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.codebuddy/plugins/config/legal-workflows/ip-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Load the practice profile first

Before running clearance, read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. Pull:
...
- **Integrations** from `## Available integrations` (research connector / Solve Intelligence / legal research connector — each determines what searches are available to run, what the fallback is, and what gets attributed in the output).
- **Decision posture** from `## Decision posture on subjective legal calls` — this skill never concludes "not confusingly similar."

If `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
If the user says "provisional," run the clearance normally using these generic defaults: middle risk appetite, lawyer role, US jurisdiction (IP registry + common-law), no playbook (do the full analysis rather than matching against a position list). Tag the reviewer note and every finding block with `[PROVISIONAL]`. At the end of the output, append:
...
Tam* (2017), the IP registry no longer refuses registration on those bases. The
...
Read `## Available integrations` from `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`:

- **If a trademark search connector is available** (Solve Intelligence,
  legal research connector — or any MCP exposing TM-registry search): run a preliminary search
...
- **If a legal research connector is available** (research connector for litigation for case law and TTAB decisions): sweep for reported disputes involving
...
> Intelligence, legal research connector, research connector, state registries, Madrid/WIPO, or any
...
**Do not supplement silently.** If you cite a IP registry registration number, it came
...
Per the decision posture in `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`:
...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` `## Outputs`.
...
TESS, Solve Intelligence, legal research connector, research connector, state registries,
...
- [specific next step 1 — e.g., "Full professional search across IP registry, state
...
- [routing per `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` —
...
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/matters/<matter-slug>/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`

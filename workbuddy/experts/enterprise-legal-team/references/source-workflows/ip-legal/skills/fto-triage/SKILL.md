...
witness interview a year later, treble damages on the table. Over-flagging is a
...
Before running triage, read `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md`. Pull:
...
> Intelligence Patents, IP registry Patents Full-Text, EPO Espacenet,
...
- **Known NPE targeting** of the technology class (e.g., network-coding NPEs in
  Eastern District of Texas / chosen jurisdiction / Western District of Texas).
- **Standards-essential declarations** (IEEE, ETSI, 3GPP) if the product touches
  a relevant standard.
- **Reported litigation** in the technology space (research connector / RECAP, Unified
  Patents, Lex Machina).
...
- **Expiration date** (per IP registry PAIR / PatentCenter / foreign equivalent —
...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/ip-legal/practice-profile.md` `## Outputs`. Mark the document as privileged if the role is lawyer; see the non-lawyer gate below if not.
...
triage did not hit Solve Intelligence Patents, IP registry Patents Full-Text,
...
be verified against the authoritative source (IP registry PatentCenter / PAIR, EPO
...
>
> If you need to find a licensed attorney, solicitor, barrister, or other authorised legal professional in your jurisdiction: for US patent work, a registered patent attorney or patent agent is required (not every lawyer is registered — the IP registry
...
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/matters/<matter-slug>/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.codebuddy/plugins/config/legal-workflows/ip-legal/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`

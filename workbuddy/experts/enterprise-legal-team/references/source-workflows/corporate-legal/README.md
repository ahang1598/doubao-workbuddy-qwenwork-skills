...
Walks through module selection, then a short targeted interview for each active area. Writes a modular `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` with only the relevant sections. Your configuration is stored at that path and survives plugin updates.
...
**Connect a research tool first — the citation guardrails depend on it.** Without one, every cite is tagged `[verify]` and the reviewer note above each deliverable records that sources weren't verified. Skills work either way; a research tool (research connector) just shifts verification work off your plate.
...
Your practice profile at `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.
...
- AI tool handoff (Luminance/Kira) is optional. If `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` says no tool, all extraction runs through the direct skill.

...
1. Load `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` + `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/deals/[code]/deal-context.md`.
...
The VDR has 2,000 documents. Somewhere in there are the 30 that matter for the deal. This skill reads documents against the diligence categories and materiality thresholds from `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`, extracts issues, and writes them in house memo format.

## Load context

- `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` → Diligence structure (categories, materiality thresholds)
- `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` → Issues memo format (how findings are stated)
- `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/deals/[code]/deal-context.md` → deal-specific thresholds, VDR location
...
Per `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` / deal-context thresholds. Don't review everything if the threshold says contracts >$X.
...
Per the finding template in `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`. If the seed memo used this:
...
- **To ai-tool-handoff:** If Luminance/Kira is in use per `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`, hand bulk contract review there. This skill handles the nuanced documents (side letters, amendments, anything the AI tool struggles with).
...
If the extraction surfaced more than ~10 issues, or any time the user asks: offer the dashboard (see practice-profile.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by severity (🔴 / 🟠 / 🟡 / 🟢), counts by house category, and a sortable grid of issues with materiality, category, and VDR source.
...
- It doesn't replace bulk AI review. For high-volume clause extraction, hand off to Luminance/Kira per `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`. This skill is for the judgment layer.

...
1. Load `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` → `## Entity Management` (entity table, jurisdictions, registered agent).
...
3. Read/write `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/entities/compliance-tracker.yaml`.
...
is a file you own, AI assistant updates it on command, and you export it when you need
to share it.
...
Lives at `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/entities/compliance-tracker.yaml`. Structure:
...
Read `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` → `## Entity Management` → Entity table. If the entity table
...
Generate `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/entities/compliance-tracker.yaml` with all entities and their
...
If the tracker covers more than ~10 entities, or any time the user asks: offer the dashboard (see practice-profile.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by filing status (overdue / due soon / filed / unknown), counts by good-standing state, and a sortable entity table with jurisdiction, filing type, and next due date.
...
**Before directing or confirming a filing:** Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`. If the Role is **Non-lawyer**:
...
Attorney tells AI assistant what was filed:
> "We filed the chosen jurisdiction annual report for [Entity] on March 1. Fee was $450."
...
report (PDF, CSV, or Excel). AI assistant reads it and updates matching entities:
...
- Based on `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` entity table: are there states in the company's
  ...
  AI assistant can flag the question but cannot determine presence independently.

**Intercompany agreement gaps:**
- From `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`: if intercompany agreements are marked as partial or no,
...
POTENTIAL GAPS
  Foreign qualification: [flag question — confirm operational presence in:]
    [list of states from `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` footprint not in tracker as qualified]
  Intercompany agreements: [status from `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`]

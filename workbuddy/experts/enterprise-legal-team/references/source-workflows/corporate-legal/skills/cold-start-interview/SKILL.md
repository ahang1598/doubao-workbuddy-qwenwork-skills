...
description: >
  ...
  plugin config. Use on fresh install, when practice-profile.md still has [PLACEHOLDER]
...
1. Check `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`. If `--new-deal`, skip to per-deal setup. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table in `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`. When probing: only report ✓ if an MCP tool call actually succeeded. Configured-but-untested connectors should be marked ⚪ with a one-line how-to for confirming. Never report ✓ based on `.mcp.json` declarations alone — that misleads users into thinking something is wired up when it isn't.
...
5. Migration: if a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/corporate-legal/*/practice-profile.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` (create parent directories as needed). For `--new-deal`, write `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/deals/[code]/deal-context.md`.
...
Read `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`:
...
If a practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/corporate-legal/*/practice-profile.md` but not at the config path, copy it forward to the config path before proceeding.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/corporate-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` with a `<!-- SETUP PAUSED AT: [section name] — run /corporate-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
- What are the key jurisdictions — just chosen jurisdiction, or a meaningful multi-jurisdiction footprint?
...
> "Before your first diligence extraction or consent: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In CodeBuddy: authorize when a skill prompts you."
...
> "Your practice profile is at `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
>
...
Write to `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/deals/[code-name]/deal-context.md`. Skills read both the plugin config (house) and `deal-context.md` (this deal), with deal-context.md taking precedence on conflicts.

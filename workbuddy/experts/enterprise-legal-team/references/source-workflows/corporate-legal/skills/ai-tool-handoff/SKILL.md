...
description: >
  ...
  per the trust level in `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`. Use when user says "send to Luminance",
...
`~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` → AI-assisted review:
...
If `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` says this — it's rare.)
...
**Trust level:** [per `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`]
...
- It doesn't replace the tool's output with its own judgment entirely — if `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`, set at cold-start based on the team's experience with the tool.

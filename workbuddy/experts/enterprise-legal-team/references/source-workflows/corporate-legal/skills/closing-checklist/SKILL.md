...
1. Read `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/deals/[code]/closing-checklist.yaml` and use the modes below.
...
Lives at `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/deals/[code]/closing-checklist.yaml`. Structure:
...
**Material-adverse-effect / material-adverse-change closing conditions.** Pull the defined term from the PA — MAC/MAE framing is negotiated, not a standard. Research the governing-law interpretation of the specific language used (chosen jurisdiction, New York, and other jurisdictions treat carve-outs and quantitative tests differently) before flagging an event as a potential MAC trigger.
...
handoff:
  ...
  severity: "[🔴 / 🟠 / 🟡 / 🟢 — carried from upstream, see severity-floor rule in practice-profile.md]"
...
If the checklist has more than ~10 items, or any time the user asks: offer the dashboard (see practice-profile.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by status (done / in progress / not started / at risk), a critical-path view grouped by workstream, and a sortable grid with item, owner, due date, and days-to-due.
...
**Before producing a "ready to close / all CPs satisfied" certification or closing memo:** Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/corporate-legal/practice-profile.md`. If the Role is **Non-lawyer**:

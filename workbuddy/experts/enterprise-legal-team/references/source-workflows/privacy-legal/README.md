...
In-house privacy counsel workflows: DPA review, personal information rights request response drafting, PIA generation, and regulation-to-policy gap analysis. Built around a team practice profile learned from your actual privacy policy, DPA template, and a reference PIA.
...
| **Privacy program manager** | personal information rights request handling, PIA intake, vendor privacy review |
| **Product counsel** | PIA generation for launches |
| **Support / CS** | personal information rights request first-line response (with escalation) |
...
Your configuration is stored at `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` and survives plugin updates.
...
| Skill | Purpose |
|---|---|
| **cold-start-interview** | Writes practice-profile.md from interview + seed docs |
...
### 4. Handle a personal information rights request

```
```

Walks you through: classify → verify → locate → exemptions → draft. Uses your systems list from the config practice-profile.md.

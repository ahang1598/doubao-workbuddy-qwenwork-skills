...
1. Read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → outputs folder path, policy document, last sweep date.
...
6. Update Last policy sweep date in `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md`.

**Direct query mode** (with description argument):
1. Read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → current policy commitments + actual policy document.
...
- `## Who we are` → `## Regulatory footprint` — the regimes in scope (applicable privacy law, applicable privacy law / CPRA / other state consumer privacy, GLBA, HIPAA, FERPA, COPPA, VPPA, CPNI, etc.)
...
policy document**. The commitments in the config practice-profile.md are a summary; the actual document
...
Update **Last policy sweep** in `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` to today's date after completing the sweep.
...
**personal information rights request responses:**
- Extract: any new data categories surfaced that weren't in previous personal information rights request responses,
...
- Match the voice and style of the existing policy (read the actual document, not
  just the config practice-profile.md summary, before drafting)
...
`~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md`, so the next sweep only looks at new files.
...
If the sweep surfaced more than ~10 drift findings, or any time the user asks: offer the dashboard (see practice-profile.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by surface (policy clause / PIA / DPA / triage), counts by severity, and a sortable grid of findings with source artifact and recommended remediation.

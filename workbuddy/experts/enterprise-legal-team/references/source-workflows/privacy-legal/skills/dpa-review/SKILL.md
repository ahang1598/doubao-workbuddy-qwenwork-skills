...
1. Load `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → DPA playbook. If placeholders, stop and prompt setup.
...
**Matter context.** Check `## Matter workspaces` in the practice-level practice-profile.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/privacy-legal:matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Purpose

DPAs come in two flavors and the review is nearly opposite for each. When a customer sends their DPA, we're defending our operational flexibility. When we send one to a vendor, we're protecting our (and our customers') data. Both reviews read from the same `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` playbook but from opposite rows.

## First: which direction?

Before anything else, establish:

- **We are the processor** → customer is sending us their DPA → read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → "When we are the processor" table
...
This review assumes the jurisdictional scope specified in your configuration. Privacy rules, response deadlines, and lawful bases vary materially by jurisdiction (applicable privacy law vs. state consumer privacy laws vs. sectoral). If the controller, processor, or data subjects are in a different jurisdiction than configured, this review may not apply as written.

## Load prior context on this counterparty / activity

Before reviewing, check the outputs folder for prior work on this counterparty or processing activity. Read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → `## Outputs` for the outputs folder path. Scan for:
...
**Carry severity from the upstream output as a floor** per the cross-skill severity floor rule in `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → `## Shared guardrails`. A processing activity the triage rated 🔴 cannot be quietly downgraded to 🟢 in the DPA review; any demotion is stated and explained.
...
>
> If yes to any: the federal overlay usually supplies the controlling substantive restriction, not just an exemption from a state consumer privacy law. Research the currently-operative provision and cite it. A DPA that is "exempt" from applicable privacy law under § 1798.145(e) because it is GLBA-covered is still subject to the GLBA restrictions — the applicable privacy law exemption moves the governing framework, it doesn't eliminate it. Flag sectoral gaps in the deal-breakers list alongside applicable privacy law / state-privacy gaps.
...
Walk every DPA through these terms, clause by clause. The *specific* numeric and substantive positions (notice periods, breach timelines, acceptable/unacceptable floors) come from `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → `## DPA playbook`. The regulatory floors that any DPA has to clear come from primary law — **research the currently operative rule** for each applicable regime and cite primary sources before stating a floor.
...
>
> - `[settled]` — stable, well-known statutory and regulatory references unlikely to have changed (e.g., applicable privacy law Art. 28, Art. 33 72-hour breach notice, SCC Decision 2021/914 by number). Still verify before filing, but lower priority.
...
| **Breach notification** | Defined trigger ("evidence exchange" vs "confirmation"), defined timeline | Breach notification | Timeline tightness; clock trigger; "without undue delay" is vague |
...
Customer DPAs try to push operational burden onto us. For each clause below, compare the customer's ask to the playbook. Where the customer's ask is outside the playbook, push back to the team's standard position (from the config practice-profile.md) and be ready to fall back to the acceptable position.
...
- If the privacy policy says "we never sell data" — does any DPA clause look like a sale under applicable privacy law?
...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
[For each issue: the fallback from the config practice-profile.md, or escalation routing if no
fallback exists]
```
...
**Before proceeding to sign or countersign a DPA (including returning an executed version, consenting to automatic execution on a counterparty platform, or instructing a signatory to execute):** Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md`. If the Role is Non-lawyer:
...
- It doesn't draft a DPA from scratch. If the answer is "use our template," pull the template from the seed docs path in the config practice-profile.md.

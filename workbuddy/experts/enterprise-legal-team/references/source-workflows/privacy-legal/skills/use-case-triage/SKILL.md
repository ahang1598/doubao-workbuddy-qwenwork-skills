...
description: >
  Quickly determine whether a processing activity needs a PIA, a mandatory applicable privacy law
...
1. Read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md`. Confirm privacy practice is configured — if not, stop and direct to setup.
2. Run the workflow below. Clarify the activity if vague.
3. House trigger check → mandatory DPIA check (if applicable privacy law in footprint) → privacy policy conflict check.
...
This triage assumes the jurisdictional scope specified in your configuration. Privacy rules, assessment triggers, and lawful bases vary materially by jurisdiction (applicable privacy law vs. state consumer privacy laws vs. sectoral). If the processing activity, controller, or affected data subjects fall under a different jurisdiction, this classification may not apply as written.

## Read the config first

Before triaging, always read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md`. The PIA trigger criteria, regulatory
...
Read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → `## PIA house style` → Trigger criteria. Apply them.
...
>
> If yes to any: the federal overlay usually supplies the controlling substantive restriction, not just an exemption from a state consumer privacy law. Research and cite the specific provision before continuing. An activity that is "exempt" from applicable privacy law under § 1798.145(e) because it is GLBA-covered is still subject to the GLBA restrictions (e.g., § 6802(a)-(c) on NPI sharing) — the applicable privacy law exemption does not make the activity lawful; it just moves the governing framework to GLBA.

For each regime in `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → `## Regulatory footprint`, **research the currently operative mandatory privacy/data-protection assessment triggers**. Cite controlling statute, regulation, or regulator guidance with pinpoint references. Note effective dates — national and state regulators publish and update trigger lists regularly; do not rely on a static checklist. Flag uncertainty for attorney verification rather than guess.
...
Read `~/.codebuddy/plugins/config/legal-workflows/privacy-legal/practice-profile.md` → `## Privacy policy commitments`. Check the proposed activity
...
**applicable privacy law mandatory DPIA trigger?** [Yes — [trigger] / No / N/A (applicable privacy law not in footprint)]
...
**Lawful basis (if applicable privacy law in footprint):** [Consent / Contract / Legitimate Interest /
...
given the data set. Pseudonymized data is still personal data under applicable privacy law.

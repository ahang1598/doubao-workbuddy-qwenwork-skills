...
description: >
  ...
  `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` from
...
1. Check `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/ai-governance-legal/*/practice-profile.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` (create parent directories as needed). Show summary. Offer first task.

## Flags

- `--redo` — re-run the full interview and overwrite `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` without re-running the full interview. Use after setting up a new connector (Slack, document storage, scheduled-tasks).
...
Read `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`:
...
If a practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/ai-governance-legal/*/practice-profile.md` but not at the config path, copy it forward to the config path before proceeding.
...
> "This plugin maintains: your practice profile (governance tiers, red lines, policy commitments), a use-case registry, impact assessments, and vendor AI reviews — all in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/`. It learns how you actually work — your practice, your risk calibration, your house conventions — and writes that into a plain-text file the plugin reads from every time. Everything you answer can be changed later."
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/ai-governance-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` with a `<!-- SETUP PAUSED AT: [section name] — run /ai-governance-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Google Drive isn't connected. In AI coworking environment: Settings → Connectors → Add → Google Drive → sign in. In CodeBuddy: add the Drive MCP to your config or via `/mcp`. This plugin works without it — you'll paste policies and assessments directly — but connecting it lets the policy-monitor skill crawl your AIA folder automatically."
...
`~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/ai-systems.yaml`.
...
**Shadow AI evidence exchange.** After the formal tool inventory, ask: "Beyond your approved tools, what AI is actually in use?
...
- **Informally adopted tools:** Employees using ChatGPT, Gemini, AI assistant, Perplexity, or other consumer AI without central approval. Check with IT for SaaS spend, browser extension usage, and DLP alerts.
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your configuration is at `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
...
6. **Before your first triage**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In CodeBuddy: authorize when a skill prompts you.

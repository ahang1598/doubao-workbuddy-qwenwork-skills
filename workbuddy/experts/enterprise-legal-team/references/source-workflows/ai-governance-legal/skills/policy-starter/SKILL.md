...
1. Read `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`. If the practice profile is unpopulated, stop and direct to `/ai-governance-legal:cold-start-interview`.
...
4. Web search for the current published model policies and guidance relevant to the deployment context (clinical supervision body, state bars, ILTA, CLOC, NIST, peer-firm / peer-company policies, current state AI laws, EU AI Act, sector regulators as applicable).
...
up doing with Copilot and AI assistant for Work. This skill produces a **draft** policy
...
1. **Source from published model policies, not from invention.** Search for and
   read the clinical supervision body AI Toolkit, state bar guidance, ILTA's model policy, CLOC's
...
|---|---|
| US | clinical supervision body Formal Opinion 512, state bar guidance (CA, FL, NY, TX all have published AI guidance), ILTA model policy, CLOC templates, peer firm published AI policies |
...
|---|---|---|---|
| clinical supervision guidance | [url] | [date] | Disclosure and competence framing |
...
- **Source attribution** inline where a rule is adapted from a specific source.
  ...
  before using it in representation of a client `[clinical supervision guidance]`."

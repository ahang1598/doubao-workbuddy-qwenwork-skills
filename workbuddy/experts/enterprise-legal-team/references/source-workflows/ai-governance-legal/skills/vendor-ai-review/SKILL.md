...
1. Read `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`. Confirm vendor governance positions are populated — if not, stop and direct to setup.
...
6. AI policy consistency diff vs. `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
...
Read `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` → `## Vendor AI governance`. Also read `## AI policy commitments`
— vendor terms can't be consistent with a use restriction our own policy imposes if
we've agreed to something different.

If `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` contains `[PLACEHOLDER]`, surface this bounce:
...
Review each term below. For each, extract what the vendor's contract actually says and compare it against the position in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` → `## Vendor AI governance` (standard / acceptable fallback / automatic no). The default positions come from the team's playbook, not from this skill.
...
| **Stacked-vendor accountability** | Is this vendor the model provider (e.g., upstream provider, OpenAI, Google, Meta), or are they a deployer of someone else's model (e.g., a SaaS wrapper of AI assistant, ChatGPT, or Gemini) or a reseller of infrastructure-hosted foundation models (upstream provider-on-Bedrock, AI assistant-on-Vertex, OpenAI-on-Azure)? If the latter: there are TWO vendors' terms in play — the one you're reviewing, plus the upstream model provider's terms. Identify (a) whose terms govern training on inputs, retention, and safety, (b) who is contractually liable for model behavior, and (c) whether each upstream commitment (e.g., "no training on inputs") is flowed down to you, or remains between the vendor and the upstream provider only. Flag any clause where one party disclaims responsibility for the other (e.g., "upstream provider is not responsible for Bedrock or any other services it receives from AWS"; "Azure disclaims responsibility for OpenAI model outputs") and whether the counter-party's contract closes the gap. Do not review the two contracts in isolation. |

If `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` doesn't define a position for a term on this list, ask: "Your playbook doesn't cover [term]. What's your default position, your acceptable fallback, and your automatic no? I'll add it to `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` so the next review is consistent."

---

## Playbook comparison

For each term above, compare what we found to the positions in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
...
> **Our position:** [from `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`]
> **Gap:** [specific delta — or "Aligned"]
> **Proposed fix:** [specific redline language, or "escalate — outside fallback"]

Use the severity ratings consistently (calibrated against `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` positions):
...
- 🔴 **Critical** — outside fallback; deployment should not proceed without resolution. Escalate per `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
...
Cross-check the vendor's terms against our AI policy commitments in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
...
**Before recommending signature of a vendor AI agreement (the version the company will execute):** Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`. If the Role is Non-lawyer:
...
[For each 🔴 and 🟠: the fallback from `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`, or "escalate — outside fallback"
...
3. **Model provider** (e.g., upstream provider, OpenAI, Google, Meta) — the LLM
4. **Hosted knowledge base / RAG source** (e.g., a vector database, a third-party data corpus, a retrieval service) — the data AI assistant reads from
...
- It doesn't decide whether to accept terms outside the fallbacks. It routes those
  per the escalation table in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.

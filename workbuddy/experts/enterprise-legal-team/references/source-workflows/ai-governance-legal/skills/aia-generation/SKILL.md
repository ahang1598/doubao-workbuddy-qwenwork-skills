...
description: >
  ...
  assessment in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
...
1. Read `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`. Confirm impact assessment house style is populated.
...
6. Policy diff against `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` AI policy commitments.
...
**Matter context.** Check `## Matter workspaces` in the practice-level practice-profile.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/ai-governance-legal:matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
Read `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` → `## Impact assessment house style`. That has:
- What triggers an impact assessment at this company
- The structure template extracted from the seed assessment
- Typical depth
- Who signs off

If the seed structure is in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`, **use it**. The point is that this assessment
looks like the other assessments this team produces.

**Jurisdictional scope.** This assessment applies the regulatory regimes listed in `## Regulatory footprint` in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`. AI legal rules, risk classifications, and deployment obligations vary materially by jurisdiction and are moving fast. If this system is (or will be) deployed outside that footprint, or if a choice-of-law question is in play, this analysis may not apply as written — re-run or expand the footprint.

---

## Step 0: Is an impact assessment needed?

Check the trigger criteria in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
...
- Is the use case in the elevated or high governance tier per `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
> **No silent supplement.** If a research query to the configured legal research tool (research platform, EUR-Lex, regulator sites, or firm platform) returns few or no results for a regime's risk tiers or triggers, report what was found and stop. Do NOT fill the gap from web search or model knowledge without asking. Say: "The search returned [N] results from [tool]. Coverage appears thin for [regime / topic]. Options: (1) broaden the search query, (2) try a different research tool, (3) search the web — results will be tagged `[web search — verify]` and should be checked against the issuing authority before relying, or (4) flag as unverified and stop. Which would you like?" A lawyer decides whether to accept lower-confidence sources.
>
...
>
> - `[settled]` — stable, well-known statutory and regulatory references unlikely to have changed (e.g., applicable privacy law Art. 22 as a concept, the existence of Regulation (EU) 2024/1689 as the EU AI Act). Still verify before certifying, but lower priority.
> - `[verify]` — model-knowledge citations that are real but should be verified: specific delegated / implementing acts, regulator guidance, NYC DCWP rules, Colorado AI Act provisions, harmonized standards, effective dates, employment regulator guidance, and anything post-2023.
...
**Fast track vs. full assessment:** `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` defines what qualifies for abbreviated treatment. If `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` doesn't define fast-track criteria, default to full assessment and ask the user what criteria they want captured for next time.
...
For each regime in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` → `## Regulatory footprint` that applies to this system — **plus any regime surfaced by the re-derivation above** — research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.** If none was captured, use this default:
...
|---|---|---|
| [commitment from `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md` AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
Regulatory citations in Section 6 (and anywhere else) were generated by an AI model and have not been verified against primary sources. Before the assessment is certified or relied on, run a verification pass against a legal research tool (research platform, EUR-Lex, or your firm's platform) for each cited provision — confirm the pinpoint, currency, and any delegated or implementing acts. The AI regulatory landscape shifts quickly; verify before advising. Source tags on each citation (e.g., `[EUR-Lex]`, `[web search — verify]`) show where it came from; `verify` tags carry higher fabrication risk and should be checked first.
```

**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in `~/.codebuddy/plugins/config/legal-workflows/ai-governance-legal/practice-profile.md`.
Common drift:

...
1. Load `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` → policy library index + practice profile.
...
One of:
- A `GAP-ID` from the gap tracker — load the entry from `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/gap-tracker.yaml` (or the matter-level equivalent).
...
If the rule text is partial or ambiguous, apply the **no silent supplement** rule from practice-profile.md: offer the user the options (paste full text, point at primary source, web-search-with-verify-tag, or stop), and wait.
...
When you see a red flag, check (via research MCP, web search if enabled, or the official regulator publication case docket or procedural record) for: delays, stays, injunctions, rescission proposals, vacatur, or amendments. If you can verify the rule is in force, proceed. If you cannot verify:
...
- Carry source tags through from the diff: `[official regulator publication]`, `[web search — verify]`, `[model knowledge — verify]`, `[user provided]`. Don't strip them when moving from the diff to the redraft.
...
|---|---|---|---|---|---|
| 1 | §2.1 PII definition | "…names, addresses, SSNs…" | "…names, addresses, SSNs, biometric identifiers…" | COPPA 2025 amendments expand PII to cover biometrics | [official regulator publication] |
...
- [ ] Verify the rule status and effective date (official regulator publication case docket or procedural record, or outside counsel).
...
This skill reads the policy library index and owners from `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`. When a value it needs is empty or still `[PLACEHOLDER]`:
...
- **Severity floor:** if the upstream gap is 🔴 or 🟠, the memo's Bottom line carries that severity. Silent demotion is a contradiction a reviewing lawyer cannot see. See practice-profile.md `## Cross-skill severity floor`.

...
**Matter context.** Check `## Matter workspaces` in the practice-level practice-profile.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/regulatory-legal:matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
Lives at `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/gap-tracker.yaml`:
...
| `watch` | Forward-looking item — ANPR, RFI, proposed rule not yet final. No compliance obligation today; policy work waits for the final rule. `due:` is a revisit date (typically the public consultation draft expected date or a one-year horizon), not a compliance deadline. | No auto-reminder; re-evaluate when an public consultation draft drops or at the revisit date. |
| `comment-decision` | Pre-rulemaking comment decision pending — ANPR or public consultation draft where the team is deciding whether to file a comment. `due:` is the comment deadline. | 21 days before due (tighter than compliance gaps because comment-drafting windows are short). |
...
| ID | Item | Type (ANPR/public consultation draft/RFI) | Comment deadline | Owner |
|---|---|---|---|---|
...
This skill reads gap-response owners and the escalation path from `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`. When a value it needs is empty or still `[PLACEHOLDER]`:

- **Gap-response triager missing:** leave assignment open and append to the output: "No triager is set in `## Gap response process`. Assign one with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` so new gaps get routed."
...
- **Escalation path missing for an overdue material gap:** still report it as overdue, and append: "No escalation path is set for material overdue gaps. Configure it with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`."
...
Reminder cadence is a function of `gap_type` — compliance gaps get a 30-day heads-up, comment-decision items get 21 days (tighter because the drafting window is shorter), watch items get no auto-reminder (re-evaluate when an public consultation draft drops).
...
- `watch`: no auto-reminder. Revisit when the tracker is reviewed or an public consultation draft is logged for the same regulation.
...
**Before closing a gap as resolved, or producing any output that certifies compliance with a regulatory requirement (internal attestation, board report, audit response, regulator response):** Read `## Who's using this` in ~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md. If the Role is **Non-lawyer**:
...
If the tracker surfaced more than ~10 open gaps, or any time the user asks: offer the dashboard (see practice-profile.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by severity, a timeline of gaps by due date, and a sortable grid with owner, status, and last-touched date.

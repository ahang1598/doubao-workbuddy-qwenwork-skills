...
1. Load `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` → policy library index.
...
`~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` → policy library index (policies, locations, owners).
...
A compliance artifact built on an undisclosed scope exclusion looks like concealment in evidence exchange. The flag is the difference between "we scoped the review" and "we hid the problem."
...
- Note the comment deadline and the team's comment-decision owner from `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`.
...
### Negative-finding branch (final rule / public consultation draft diffed against a policy that isn't the right target)
...
### Gap branch (final rule / public consultation draft with at least one gap against the target policy)
...
**Verify citations before relying on them.** The regulatory citations and policy references above were AI-generated and have not been checked against a primary source. Before acting on any requirement here, confirm the rule against research platform, your firm's research platform, or the issuing authority's website — check accuracy, effective date, and current status. AI-generated regulatory citations are sometimes fabricated, misquoted, or stale. Source tags on each requirement (e.g., `[official regulator publication]`, `[web search — verify]`) show where the citation came from; `verify` tags carry higher fabrication risk and should be checked first.
```

## Config-dependent fallbacks

This skill reads the policy library index from `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`. When the index is empty or still `[PLACEHOLDER]`:

- **Policy library empty:** flag every requirement as "no policy match" by default and append to the output: "The policy library in your configuration is empty, so every requirement is flagged as a new-policy gap. If you have policies that address these requirements, add them to the library with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`, then re-run the diff."
- **Owner missing for a matched policy:** leave the Owner cell blank in the summary and append: "Policy owners aren't set for [list]. Assign them with `/regulatory-legal:cold-start-interview --redo` or by editing the policy library in `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` so gap-surfacer can route."

...
1. Check `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`. If a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/regulatory-legal/*/practice-profile.md` but not at the config path, copy it to the config path and tell the user what was migrated. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table in `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`.
...
4. Write `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` (creating parent directories as needed) with watchlist + materiality threshold.
...
Read `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md`:
...
If a practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/regulatory-legal/*/practice-profile.md` but not at the config path, copy it forward.
...
Do not read the user's home-directory `~/practice-profile.md`, `~/user.md`, or other personal memory to pre-populate the interview. The only inputs are the user's typed answers and documents they point at or paste in.
...
- **Data sources:** setup builds a fresh professional profile from the user's answers only. It does not read the user's personal AI assistant history, other conversations, or home-directory practice-profile.md. If something relevant came up earlier in this conversation (for example, the user mentioned a regulator or their sector), ask before using it. Nothing gets folded into the configuration unless the user types it or approves it.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/regulatory-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` with a `<!-- SETUP PAUSED AT: [section name] — run /regulatory-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
The official regulator publication API is a free public endpoint and is always available — it does not require an MCP connector.

For connectors that show as not connected, tell the user how to connect. Example phrasing: "TR Regulatory Intelligence isn't connected. In AI coworking environment: Settings → Connectors → Add → Thomson Reuters → sign in. In CodeBuddy: add the TR MCP to your config or via `/mcp`. This plugin works without it — official regulator publication + manual paste covers US federal coverage — but connecting it adds enrichment and alert import."
...
You don't need all of these. Core features work with free feeds (official regulator publication API) and file access alone. Paid feeds add enrichment; manual paste-in always works. If you set something up later, re-run `/regulatory-legal:cold-start-interview --check-integrations`.
...
- Which regulators? Name them. (FTC, SEC, CFPB, state AGs, CPPA, EU DPAs, sector-specific?)
  *Coverage note: this plugin has structured feed support for US federal agencies (official regulator publication API), SEC, FTC, and CFPB. State regulators and EU DPAs are supported via user-provided RSS URLs or manual entry — there is no automatic feed for those. Non-US regulators outside the EU DPA table require manual entry or user-provided feeds.*
...
- A proposed rule (public consultation draft) — comment period open
...
The official regulator publication API (federalregister.gov/api) is the stable primary source for US federal agencies — it returns structured data by agency, document type, effective date, and comment deadlines. Use it as the default for any federal regulator in the watchlist.
...
- research connector? Which trackers?
...
> When I see a proposed rule (public consultation draft) from your watchlist, I'll automatically log the comment deadline. Do you want me to flag these so you can decide whether to file a comment?
...
|---|---|---|
| [name] | official regulator publication API / RSS / manual | [endpoint or "manual entry"] |

**Paid feeds (if configured):**
| Service | Subscription | Alerts |
|---|---|---|
| TR Regulatory Intelligence | [yes/no] | [alert names] |
| research connector | [yes/no] | [tracker names] |
...
- Close with the changeability note:
  ...
  > "Done. Your configuration is at `~/.codebuddy/plugins/config/legal-workflows/regulatory-legal/practice-profile.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
  >

...
1. Load `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` → Marketing claims standards.
...
Read `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` → `## Marketing claims`:
...
Per `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` — if comparative claims are "never," flag all of them. If "allowed with substantiation," check for the substantiation.
...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
> **Before emitting "Ready to ship: Yes" (i.e., approving a claim for external use / publication):** Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md`. If the Role is Non-lawyer:
>
...
Any FTC rules, NAD decisions, state UDAP statutes, sector regulations, or platform policies cited in this review were generated by an AI model and have not been verified against a primary source. Before relying on a specific rule to clear or reject copy, verify it against a legal research tool (research platform, research connector, or your firm's research platform) for accuracy and current effective date — endorsement guides, platform rules, and state UDAP regimes all update frequently. Source tags on each citation (e.g., `[FTC site]`, `[web search — verify]`) show where it came from; `verify` tags carry higher fabrication risk and should be checked first.
```

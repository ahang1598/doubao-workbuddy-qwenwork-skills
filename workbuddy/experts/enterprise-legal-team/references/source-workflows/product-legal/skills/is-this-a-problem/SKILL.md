...
1. Load `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` → Risk calibration.
...
Read `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` → `## Risk calibration`. The whole point of this skill is pattern-matching against that table.
...
Slack triage replies are internal legal advice. If the reply is being pasted into a ticket, document, or channel that's broadly shared with non-legal, prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` `## Outputs` (it differs by user role — see `## Who's using this`):

...
1. Check `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` state.
...
5. Migration: if a populated practice-profile.md (no `[PLACEHOLDER]` markers) exists at `~/.codebuddy/plugins/cache/legal-workflows/product-legal/*/practice-profile.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` (create parent directories as needed). Show calibration table for confirmation.

## `--check-integrations`

Re-runs the integration availability check (launch tracker, document storage, Slack) and updates `## Available integrations` in `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
If a practice-profile.md exists at the old cache path `~/.codebuddy/plugins/cache/legal-workflows/product-legal/*/practice-profile.md` but not at the config path, copy it forward.
...
>
> Then: "Setup builds a fresh professional profile from your answers. It does not read your personal AI assistant history, other conversations, or your home-directory practice-profile.md. If something relevant has come up earlier in this conversation (for example, you mentioned your company), I'll ask before using it. Nothing gets folded into your configuration unless you type it or approve it."
>
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/product-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` with a `<!-- SETUP PAUSED AT: [section name] — run /product-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Jira isn't connected. In AI coworking environment: Settings → Connectors → Add → Jira → sign in. In CodeBuddy: add the Jira MCP to your config or via `/mcp`. This plugin works without it — you'll paste PRDs and review docs directly — but connecting it lets the launch-watcher agent pull tickets automatically."
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first launch review: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In CodeBuddy: authorize when a skill prompts you."
...
5. **Close with the changeability note.** Say:
   ...
   > "Done. Your configuration is at `~/.codebuddy/plugins/config/legal-workflows/product-legal/practice-profile.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
   >

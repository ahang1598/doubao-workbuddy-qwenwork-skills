...
Prepend the work-product header from `~/.codebuddy/plugins/config/legal-workflows/employment-legal/practice-profile.md` → `## Outputs` (it differs by user role — see `## Who's using this`). Every file, log, memo, and summary produced by this skill opens with that header.
...
problems in evidence exchange if privilege is later challenged.
...
Read `~/.codebuddy/plugins/config/legal-workflows/employment-legal/practice-profile.md` → escalation table, any investigation protocols noted.
...
`~/.codebuddy/plugins/config/legal-workflows/employment-legal/investigation-[matter-slug]/log.yaml`:
...
`~/.codebuddy/plugins/config/legal-workflows/employment-legal/investigation-[matter-slug]/sources-checklist.yaml`:

Generated from the investigation type. See sources checklist templates below.

`~/.codebuddy/plugins/config/legal-workflows/employment-legal/investigation-[matter-slug]/documents-reviewed.yaml`:
...
`~/.codebuddy/plugins/config/legal-workflows/employment-legal/investigation-[slug]/sources-checklist.yaml`.
...
If multiple investigation folders exist in `~/.codebuddy/plugins/config/legal-workflows/employment-legal/`, ask which matter this
...
Write the draft to `~/.codebuddy/plugins/config/legal-workflows/employment-legal/investigation-[slug]/memo.md`.
...
**Before producing a summary, memo, or content intended for an external response (employment regulator/DFEH/state agency charge response, plaintiff's-counsel demand letter response, regulator response, or any formal complaint reply):** Read `## Who's using this` in `~/.codebuddy/plugins/config/legal-workflows/employment-legal/practice-profile.md`. If the Role is **Non-lawyer**:
...
> - What to ask the attorney (is this the right theory; are we preserving defenses; should an outside firm take this over; what needs redaction or a confidentiality review log)
>

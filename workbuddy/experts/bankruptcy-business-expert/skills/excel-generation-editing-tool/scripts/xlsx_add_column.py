...
def main() -> None:
    ...
    # Collect existing cell references to detect duplicates
    existing_refs = set()
    for row_el in sheet_data:
        for c in row_el:
            r = c.get("r", "")
    ...
    def _add_cell_safe(parent, cell_ref, tag=_tag("c")) -> ET.Element:
        """Add a cell, aborting if the reference already exists."""
        if cell_ref in existing_refs:
            print(f"  ERROR: Cell {cell_ref} already exists. Aborting to prevent file corruption.")
            sys.exit(1)
        existing_refs.add(cell_ref)
        return ET.SubElement(parent, tag)

    # Add header cell
    if args.header and 1 in row_map:
        cell = _add_cell_safe(row_map[1], f"{col}1")
        cell.set("r", f"{col}1")
    ...
    if args.formula and args.formula_rows:
        ...
        for row_num in range(start, end + 1):
            ...
            cell = _add_cell_safe(row_map[row_num], f"{col}{row_num}")
    ...
    if args.total_row and args.total_formula:
        ...
        cell = _add_cell_safe(row_map[args.total_row], f"{col}{args.total_row}")
    ...
    # Sort cells within each row by column letter (OOXML requires ascending order)
    for row_el in sheet_data:
        cells = list(row_el)
        if len(cells) > 1:
            cells.sort(key=lambda c: col_number(re.match(r"([A-Z]+)", c.get("r", "A")).group(1)))

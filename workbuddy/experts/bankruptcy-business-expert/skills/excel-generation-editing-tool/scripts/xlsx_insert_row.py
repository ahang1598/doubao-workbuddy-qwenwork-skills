...
def main() -> None:
    ...
    cmd = [sys.executable, shift_script, args.work_dir, "insert", str(at), "1"]
    if args.sheet:
    ...
    # After shift_rows, rows >= at have moved down by 1. If the style
    # reference row was shifted, read from its new position.
    ws_path = find_ws_path(args.work_dir, args.sheet)
    ws_tree = ET.parse(ws_path)

    ref_styles = {}
    if args.copy_style_from is not None:
        style_row = args.copy_style_from
        if style_row >= at:
            style_row += 1  # reference row was shifted down
        ref_styles = get_row_styles(ws_tree, style_row)
        print(f"Step 2: Copied styles from row {args.copy_style_from}"
              f"{' (shifted to row ' + str(style_row) + ')' if style_row != args.copy_style_from else ''}: {ref_styles}")
    ...
    for dim in root.iter(_tag("dimension")):
        ...
        if ":" in old_ref:
            ...
            existing_max_col = max(col_number(end_col),
                                   max((col_number(c) for c in all_cols), default=0))
            max_col_letter = end_col if col_number(end_col) >= existing_max_col else max(all_cols, key=col_number, default=end_col)

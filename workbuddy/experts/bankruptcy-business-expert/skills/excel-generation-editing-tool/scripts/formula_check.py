...
- Runtime errors that only appear after formulas execute (#DIV/0! on empty denominator, etc.)
  -> Open the file in Excel/WPS to verify runtime results
...
def get_sheet_files(z: zipfile.ZipFile) -> dict[str, str]:
    ...
    for rel in rels:
        ...
        if "worksheets" in target:
            # Handle absolute paths: /xl/worksheets/sheet1.xml -> xl/worksheets/sheet1.xml
...
def check(xlsx_path: str, sheet_filter: str | None = None) -> dict:
    ...
    with z:
        ...
        for rid, sheet_name in sheet_names.items():
            ...
            # ── Check 4: shared formula primary cell integrity ──────────
            # After processing all cells on this sheet, verify that every
            # shared formula group index (si) has a primary cell with formula text.
            # Also check that the primary cell's ref attribute range covers
            # all consumer cells in the same column.
            shared_consumers: dict[str, list[str]] = {}  # si -> [cell_refs without formula text]
            for cell in ws.findall(f".//{NSP}c"):
                f_elem = cell.find(f"{NSP}f")
                if f_elem is None:
                    continue
                f_type = f_elem.get("t", "")
                f_si = f_elem.get("si")
                if f_type == "shared" and f_elem.text is None and f_si:
                    # Consumer cell: no formula text, relies on primary
                    cell_ref = cell.get("r", "?")
                    shared_consumers.setdefault(f_si, []).append(cell_ref)

            for si, consumer_refs in shared_consumers.items():
                    ...
                    results["errors"].append(
                        {
                            ...
                            "sheet": sheet_name,
                            "shared_index": si,
                            "consumer_cells": consumer_refs,
                            "detail": (
                                f"Shared formula group si={si} has {len(consumer_refs)} "
                                f"consumer cell(s) but no primary cell with formula text"
                            ),
                        }
                    )
                    results["error_count"] += 1
                else:
                    primary_ref = shared_primary[si]
                    # Verify consumers are in the same column as the primary
                    primary_col = re.match(r"([A-Z]+)", primary_ref)
                    if primary_col:
                        col = primary_col.group(1)
                        mismatched = [
                            r for r in consumer_refs
                            if not r.startswith(col)
                        ]
                        if mismatched:
                            results["errors"].append(
                                {
                                    "type": "shared_formula_column_mismatch",
                                    "sheet": sheet_name,
                                    "shared_index": si,
                                    "primary_cell": primary_ref,
                                    "mismatched_consumers": mismatched,
                                    "detail": (
                                        f"Shared formula si={si} primary at {primary_ref} "
                                        f"(col {col}) has consumers in different columns: {mismatched}"
                                    ),
                                }
                            )
                            results["error_count"] += 1
